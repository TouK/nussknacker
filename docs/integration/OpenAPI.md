...
| secret                 | false    |         | Configuration for [authentication](https://swagger.io/docs/specification/authentication/) which matches all schemas `securitySchemas` of its type in the *OpenAPI interface definition*. This config entry has the same structure as values in `security` object (see above) |
