...
You have to configure database connection pool you will be using in your `Sql` enricher. It is performed at the root level of the configuration file (the same level as all other configs from Designer configuration area) - see extract of the [configuration file](../installation_configuration_guide/Common.md#configuration-areas).
