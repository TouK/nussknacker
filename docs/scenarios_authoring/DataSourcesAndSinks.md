...
Components which perform stateful computations in time windows (Flink engine only) are covered [here](./AggregatesInTimeWindows.md). The UnionMemo, PreviousValue, Delay components which are also specific to Flink engine are covered in this document. 
