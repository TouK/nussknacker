...
Also, can be used `#DATE_FORMAT` helper methods to parse or format certain data type from/to the String. It is not recommended using it
...
Can be used also `DateTimeFormatter` directly via e.g. `#DATE_FORMAT.formatter('EEEE').format(#date)`. Other formatter factory methods:
- `#DATE_FORMAT.formatter('EEEE', 'PL')` - creates `DateTimeFormatter` using given pattern (`EEEE`) and locale (`PL`)
- `#DATE_FORMAT.lenientFormatter('yyyy-MM-dd')` - creates lenient version `DateTimeFormatter`
- `#DATE_FORMAT.lenientFormatter('yyyy-MM-dd EEEE', 'PL')` - creates lenient version `DateTimeFormatter` using given pattern and locale

For full list of available format options take a look at https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/time/format/DateTimeFormatter.html api docs.
...
There are also available some useful constants
