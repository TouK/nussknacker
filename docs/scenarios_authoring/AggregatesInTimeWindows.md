...
If you need to count events in a window, use Sum aggregate function and aggregate by `1L` (1 of type Long) - see table with examples below. Subsequent sections use Count function on the diagrams as example for the **aggregator** - it is the easiest function to use in the examples. Please note that technically speaking we provide an indirect implementation of this aggregator.
