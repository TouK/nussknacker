...
To reduce resources consumption Sliding-window, Session-window and Single-side-join precompute aggregates in slices. [This video](https://www.youtube.com/watch?v=2bVC7sS1HVc) explains the concept of slices; please bear in mind that our implementation is slightly different. The slice length is a compromise between precision and resource requirements; please get in touch with us if you need different slice lenghts from the default ones.
