...
### Named outputs for custom components; deduplication `rejected` output

[#9450](https://github.com/TouK/nussknacker/pull/9450) Custom components declare their outputs as one ordered list (head = main output); `deduplication` declares `passed` and `rejected`. A custom node of such a component is stored in canonical JSON with an `outputs` field and connects only through named `CustomNodeOutput` edges. Migration `400` (`CustomNodeOutputsMigration`, parameterized so other components can reuse it) rewrites stored `deduplication` scenarios automatically; a setup replacing `DefaultModelMigrations`, or a component gaining an additional output after release, needs an equivalent migration - without it the stored scenarios stop validating. **An OLDER Nussknacker importing canonical JSON in the new shape silently drops everything downstream of such a node** (`ScenarioGraph` payloads fail loudly instead) - only exchange such scenarios with versions supporting this feature. Every custom node now registers a `dead_end` meter, counting records that leave through an unwired main output.
...
* [#7959](https://github.com/TouK/nussknacker/pull/7959)[#9012](https://github.com/TouK/nussknacker/pull/9012) Scenario testing API changes:
    ...
    * endpoints moved from `/processManagement` API to `/scenarioTesting` API
        * `/test` (test with form)
        * `/generateAndTest` (test with generated data) and merged into a single endpoints `/performTest`
        * `/test` to `/performTest`
...
* [#9450](https://github.com/TouK/nussknacker/pull/9450) New `EdgeType.CustomNodeOutput(name: String)` discriminator (JSON `"type": "CustomNodeOutput"`): every outgoing edge of a node whose component declares additional outputs is persisted with it, and `UINodeEdges` lists the declared outputs (main first) for such components. Clients decoding `EdgeType` exhaustively must handle the new case; an unrecognized discriminator in a `ScenarioGraph` payload fails decoding loudly.

### Code API changes

* [#9450](https://github.com/TouK/nussknacker/pull/9450) Named outputs for custom components: `CustomStreamTransformer` gained `def outputs: NonEmptyList[ComponentOutput]` (head = main output; the default keeps subclasses source-compatible), and `ComponentOutput` wraps a `String` refined as `NonEmpty And Trimmed` (`ComponentOutput.Name`), so a name may be neither empty nor padded with whitespace - a literal name needs `import eu.timepit.refined.auto._` (`eu.timepit:refined` is now a `nussknacker-components-api` dependency). `CustomComponentSpecificData` gained a third field (`outputs`), so a positional pattern match needs an extra `_`. Sealed `CanonicalNode`, `SubsequentNode` and `EdgeType` each gained a case for the multi-output shape (`CustomNodeWithOutputs` / `CustomNodeOutput`), so exhaustive matches - e.g. a custom `ProcessMigration` - must be extended. New save-blocking `ProcessCompilationError` cases: `UnknownCustomNodeOutput`, `MissingCustomNodeOutputName`, `UnsupportedEdgeNextToCustomNodeOutputs`, `DuplicateCustomNodeOutputNames` and `MultipleOutputsToSameJoin`; additionally a node other than `split` may now have at most one unnamed outgoing edge (previously such an API-submitted graph saved and got spliced into one chain).
    It is now rejected with `NonUniqueEdgeType("main output", ...)`.
  * `splittedgraph.part.CustomNodePart` was reshaped into a sealed trait with one case per shape: `MultiOutputCustomNodePart(node, outputs: NonEmptyList[SplittedOutput])` for a node connected through named edges - every wired continuation, the main one included, lives in `outputs` (each `SplittedOutput` carries its own `name` and the `nextParts`/`ends` of its output; an output the scenario leaves unwired has no entry) and `node` is a bare `EndingNode` - and `SingleOutputCustomNodePart(node, nextParts, ends)` for the released single-output form, where the continuation stays embedded in `node`.
    The multi-output case computes the inherited `nextParts`/`ends` members as the concatenation of every output's, so generic consumers still see every downstream part, but construction and pattern matches against the old single class break.
    Its counterpart `compiledgraph.part.CustomNodePart` folds every output into one type - `CustomNodePart(transformer, contextBefore, validationContext, outputs: NonEmptyList[CompiledOutput])` - where a `CompiledOutput` carries the output's identity (`output: ComponentOutput`) plus its own `node`, `nextParts` and `ends`. The list is built from the component declaration, so the head is always the main output (with a `DeadEnd` standing in when the scenario leaves it unwired) and the connected additional outputs follow in declaration order. `node` is no longer a field of the part: it is `outputs.head.node`, so it also leaves the extractor. `nextParts`/`ends` concatenate every output's, so positional construction and pattern matches break there too (`scenario-compiler/.../splittedgraph/part.scala`, `scenario-compiler/.../compiledgraph/part.scala`).
  * **`FlinkCustomStreamTransformation` is unchanged for a component with only the main output**: `transform` still returns a single `DataStream[ValueWithContext[AnyRef]]` and the `FlinkCustomStreamTransformation(...)` factories still take the same lambdas, so no such component needs migrating. A component with additional outputs implements the new, unrelated `FlinkMultiOutputStreamTransformation`, whose `transform` returns `NonEmptyList[(ComponentOutput, DataStream[ValueWithContext[AnyRef]])]` instead: the returned list restates the component's `outputs` declaration with a stream attached to each entry, keyed by the output it belongs to. Job registration looks the streams up by key, so their order does not matter. The main output always needs exactly one stream, whether or not the scenario wires it; an additional output needs one only when the scenario connects it, and the streams of the unconnected ones are never read. Omitting a stream that is needed, or returning two under the same key, fails job registration loudly. Its lambda form is `FlinkMultiOutputStreamTransformation { (start, ctx) => ... }`. A `FlinkCustomStreamTransformation` returns no key at all: registration wires its single stream to the main output whatever the component named it, so renaming the main output alone is no reason to reach for the multi-output trait.
    Registration fails there rather than letting an output the scenario wired silently emit nothing.
    Additional outputs are usually routed as Flink side outputs: `FlinkCustomNodeContext.createOutputTag(output, typeInfo)` gives a ready tag, the component emits under it and reads the stream back with `getSideOutput` on the emitting operator. `typeInfo` describes what that output emits and becomes the type of the stream `getSideOutput` returns, so pass the one the emitting operator uses for its own output, normally from `FlinkCustomNodeContext.valueWithContextInfo`; nothing verifies it, because `OutputTag` compares by name alone. That is one pass over the stream with each element going to exactly one output, and taking the tag from the same place on both ends keeps them identical. Nothing forces that shape though - any stream will do, so an output may have its own chain of operators.
    Every output has the same shape: the `Context` carried must be consistent with the node's declared output context (for typical components, the unchanged input context), and the accompanying value becomes the node's output variable on that output. A component with nothing to report on an output emits `ValueWithContext(null, ctx)`, which means the output variable is in scope downstream of that output holding `null` rather than being absent.
  * Joins (`canHaveManyInputs = true`) cannot declare additional outputs - the combination is rejected when the component definition is loaded; duplicate declared output names are rejected the same way. Like other component definition errors, the rejection is an `IllegalArgumentException` thrown at model load, so one invalid declaration prevents the whole processing type from starting.
  * A component declaring any additional outputs (a non-default `outputs` tail) has to return an implementation carrying the new `component.SupportsMultipleOutputs` marker, which on Flink only `FlinkMultiOutputStreamTransformation` carries. If the returned implementation does not carry it - another runtime's, or a Flink `FlinkCustomStreamTransformation`, which structurally cannot route more than one output - a scenario that connects one of the declared additional outputs fails compilation with `CustomNodeError`, so the designer reports it on save instead of letting the deployment fail. Only connected additional outputs are checked: a scenario that wires none of them - even one that wires only the main output - keeps compiling, because an unconnected output costs it nothing at runtime.
    This is also what rejects such a scenario on Lite: additional outputs target the Flink streaming runtime for now, so no lite implementation carries the marker. Adding them to another engine means marking that runtime's own transformation type.
* [#9399](https://github.com/TouK/nussknacker/pull/9399) `pl.touk.nussknacker.ui.process.migrate.RemoteEnvironment` changes, used to support comparing scenario versions against a remote (secondary) environment:
    * One new abstract method, `versionsWithDifferences`. Any external implementation of this trait must implement it.
    * `versionsWithDifferences` is backed by a new endpoint, `POST /processes/{scenarioName}/versions-with-differences`, which takes a scenario graph in the request body and answers with the subset of that environment's own versions that differ from it. The comparison deliberately runs on the environment that already stores those versions, so one graph is sent and a summary comes back, rather than full scenario graphs being pulled across the network to be compared locally. One request answers for as many of the scenario's most recent versions as the `limit` query parameter asks for (default 50, maximum 500) - the answering environment applies that limit before loading any graph, so it never walks its whole history, and reports `oldestComparedVersionId` when it stopped short. Version comments are returned separately, in `versionComments`, covering every version rather than only the compared ones - the answering environment reads them from its own activity list, so no second request is made for them. Comments recorded against scheduled or ad-hoc executions, and against deployments that failed, do not describe a version and are not included. The endpoint requires the `Read` permission on the scenario. Comparing scenario versions against a remote environment therefore requires that environment to run Nussknacker 1.19.0 or newer; against an older one the designer reports that the differences could not be determined.
    * `processVersions` now returns `Future[RemoteScenarioVersions]` instead of `Future[List[ScenarioVersion]]`. The new wrapper carries a `remoteUnavailable` flag, so implementations report "the remote environment could not be reached" without failing the `Future` - previously indistinguishable from "the remote has no such scenario". Implementations return `RemoteScenarioVersions(versions, remoteUnavailable = false)` for a successful fetch.
    * `GET /remoteEnvironment/{scenarioName}/versions` now responds `502 Bad Gateway` when the remote environment is unreachable or rejects our credentials, instead of returning an empty list. A remote environment that simply does not have the scenario yet still returns an empty list.
    * `GET /remoteEnvironment/{scenarioName}/versions` now also requires the `Read` permission on the scenario and responds `403 Forbidden` without it. Previously the endpoint had no permission check, so any authenticated user could read the remote environment's version history for any scenario; a client that relied on getting an empty list for an inaccessible scenario has to handle `403` now.
    * Comparing against a remote environment now fetches the remote scenario graph without validation and dictionary resolution, so both sides of the comparison are the raw stored graph. Scenarios using dictionaries previously reported differences on every node whose parameters referenced a dictionary.
    * `GET /processes/{scenarioName}` accepts a new optional `skipScenarioGraph` query parameter (default `false`). With it the response omits `scenarioGraph`, which is what `processVersions` uses when it needs only the version history. The parameter is additive - a client that does not send it, or an environment that does not recognise it, is unaffected.
...
* [#9391](https://github.com/TouK/nussknacker/pull/9391) Built-in parameter validators: runtime validation, `@NotNull`/`@NotEmpty` support and naming cleanup.
  * Affected validators: `NotNullValidator`, `NotBlankValidator`, `RegExpValidator`, `MinimalNumberValidator`, `MaximalNumberValidator`, and the new `NotEmptyCollectionValidator`. Each now also implements `RuntimeValidator`; when attached to a parameter, the evaluated value is validated at runtime after expression evaluation, and a violation throws `ParameterRuntimeValidationException` (a `NonTransientException`). Compile-time behavior is unchanged.
  * Behavior change: validators auto-attached via annotations (`@NotBlank` to `NotBlankValidator`, `@Min` to `MinimalNumberValidator`, `@Max` to `MaximalNumberValidator`) now also reject invalid values at runtime. A scenario that previously passed a dynamic blank or out-of-range value through such a parameter at runtime will now fail with `ParameterRuntimeValidationException`.
  * New annotation mappings: `@NotNull` now attaches `NotNullValidator` and `@NotEmpty` attaches `NotEmptyCollectionValidator`. Both annotations were previously ignored, so a parameter carrying them gains new validation at both compile time and runtime, and a scenario that compiled before may now fail compilation or at runtime. `@NotNull` is attached in addition to the `MandatoryExpressionValidator` that non-optional parameters already receive (the two checks are intentionally distinct). `NotEmptyCollectionValidator` only checks `java.util.Collection` and `java.util.Map` (by design), so `@NotEmpty` on a `String` or array is not validated; use `@NotBlank` for string parameters.
  * `NotBlankValidator`, `RegExpValidator`, and `NotEmptyCollectionValidator` pass `null` through to allow validator chaining; combine them with `NotNullValidator` to also reject `null`.
  * Other built-in validators (`MandatoryExpressionValidator`, `JsonExpressionValidator`, `FixedValuesExpressionValidator`, `LiteralIntegerExpressionValidator`, `MultiSelectFixedValuesExpressionValidator`) remain compile-time only.
  * Built-in validators were renamed to reflect when they run: `*ExpressionValidator` runs at compile time on the expression, `*ValueValidator` at runtime on the evaluated value, `*Validator` at both. Renames (Code API change): `MandatoryParameterValidator` → `MandatoryExpressionValidator`, `NotNullParameterValidator` → `NotNullValidator`, `NotBlankParameterValidator` → `NotBlankValidator`, `NotEmptyCollectionParameterValidator` → `NotEmptyCollectionValidator`, `RegExpParameterValidator` → `RegExpValidator`, `FixedValuesValidator` → `FixedValuesExpressionValidator`, `LiteralIntegerValidator` → `LiteralIntegerExpressionValidator`, `JsonValidator` → `JsonExpressionValidator`, `MultiSelectFixedValuesValidator` → `MultiSelectFixedValuesExpressionValidator`. `MinimalNumberValidator`, `MaximalNumberValidator`, and `CompileTimeEvaluableValueValidator` keep their names. The JSON serialization format (the `"type"` field) is unchanged, so saved scenarios and configurations are not affected.
  * New `CompileTimeAndRuntimeValidator` trait (extends `CompileTimeParameterValidator with RuntimeParameterValidator`) is the base for validators that run at both stages: an implementation provides `message`, `description`, `validate`, and `compileTimeError`, while both `isValid` methods and a default `runtimeError` (which uses `message`) are inherited. All built-in dual validators extend it.
...
* [#8778](https://github.com/TouK/nussknacker/pull/8778) [#8777](https://github.com/TouK/nussknacker/pull/8777) Legacy Flink 1.x APIs removal
  * Custom, Nussknacker's `TimestampWatermarkHandler` abstraction was replaced by Flink's `WatermarkStrategy`
    ...
    * Nussknacker's `SimpleSerializableTimestampAssigner` was removed - use Flink's `SimpleSerializableTimestampAssigner` instead
