...
Deployment of a scenario onto the [Engine](/docs/about/GLOSSARY.md#engine) is managed by the Designer's extension
called [Deployment Manager](/docs/about/GLOSSARY.md#deployment-manager).
To enable a given Deployment Manager its jar package has to be placed in the Designer's classpath. Nussknacker is distributed with three
default Deployment Managers: `flinkStreaming`, `lite-k8s`, `lite-embedded`.
...
check [configuration areas](./Overview.mdx#configuration-areas) to understand the structure of the configuration file.
...
- `type` parameter determines the type of the Deployment Manager. Possible
...
Please check high level [Lite engine description](/docs/about/GLOSSARY.md#deployment-manager) before
...
of [main configuration file](./Overview.mdx#configuration-areas).
...
| scenarioTesting.timeout                              | duration       | 55 seconds    | Timeout for scenario testing. When scenario test is not finished during this time, testing job will be canceled. This property should be configured along with `pekko.http.server.request-timeout` for proper effect.                       |
...
| scenarioStateVerification.timeout                    | duration       | 55 seconds    | Timeout for scenario state verification. When scenario test is not finished during this time, testing job will be canceled. This property should be configured along with `pekko.http.server.request-timeout` for proper effect.             |
