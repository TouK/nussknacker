...
this [configuration file snippet](../configuration/Overview.mdx#configuration-areas) to understand the
...
| discoveryCacheTtl        | false     | 30 seconds | TTL of in-memory cache for discovered OpenAPI definitions                                                                                                                                                                                                                                                         |
| cacheFileLocation        | false     |            | Path to file where last valid discovered OpenAPI definitions are persisted. When discovery refresh fails, Nussknacker falls back to cached definitions from memory and, if configured, from this file.                                                                                                            |
...
| security.*.type          | false     |            | Type of security configuration for a given security schema. Currently `apiKey`, `basicAuth` and `oauth2ClientCredentials` (alias: `oauth2`) are supported                                                                                                                                                         |
...
| security.*.clientId      | false     |            | OAuth2 client ID used when selected security type is `oauth2ClientCredentials`                                                                                                                                                                                                                                     |
| security.*.clientSecret  | false     |            | OAuth2 client secret used when selected security type is `oauth2ClientCredentials`                                                                                                                                                                                                                                |
...
```
`tokenUrl` and scopes are taken from the OpenAPI definition (`securitySchemes` + operation's `security` requirements).
