...
You have to configure a database connection pool you will be using in your `Sql` enricher. It is performed at the root level of the configuration file (the same level as all other configs from Designer configuration area) - see [configuration file](../configuration/Overview.mdx#configuration-file).
...
> It should be placed in Flink's /lib folder (/opt/flink/lib), more info can be found in [Flink Documentation](https://nightlies.apache.org/flink/flink-docs-master/docs/ops/debugging/debugging_classloading/#unloading-of-dynamically-loaded-classes-in-user-code).
