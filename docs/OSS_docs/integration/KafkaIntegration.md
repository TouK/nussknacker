...
* [Role of Schema Registry](/docs/integrations/kafka.md#schemas-and-schema-registry)
...
```json
{
  ...
  "schema.registry.url": "http://some.schema.registry.host:8080",
  "basic.auth.credentials.source": "USER_INFO",
  "basic.auth.user.info": "SchemaRegistryUsername:SchemaRegistryPassword"
}
```

Secured - simple login/password with network traffic encryption:
```json
{
  "bootstrap.servers": "kafka.some.domain-1:9092,kafka.some.domain-2:9092,kafka.some.domain-3:9092",
  ...
  "ssl.truststore.location": "path-to-PEM-file-containing-CA-certificates-required-to-validate-the-Kafka-broker-certificate-chain",
...
Kafka cluster has multiple options to configure Authentication. Take a look at [Kafka security documentation](https://kafka.apache.org/documentation#security)
...
If your Kafka cluster uses TLS/SSL, you typically need to provide a truststore.

A truststore defines which certificate authorities (CAs) are trusted when establishing a secure connection. It is used to verify the Kafka broker’s certificate during the TLS handshake.

### PEM format

Provide a PEM file containing the CA certificates required to validate the Kafka broker certificate chain. Typically, this includes:

- the intermediate CA certificate(s)
- the root CA certificate

The truststore should contain CA certificates (not the Kafka broker certificate itself), unless the broker uses a self-signed certificate.

At runtime, the truststore file will be available at `/certs/client.truststore`.Ensure that the required files are available on all Flink TaskManagers at the specified location.

Configure Kafka using:
```json
{
  "ssl.truststore.type": "PEM",
  "ssl.truststore.location": "path-to-PEM-file-containing-CA-certificates-required-to-validate-the-Kafka-broker-certificate-chain"
...
If your Kafka cluster uses TLS/SSL and requires client certificate authentication (mTLS), provide the client keystore here in PEM format. 

Kafka supports PEM keystores in two forms:

1. A single PEM file referenced with `ssl.keystore.location`, containing private key and certificate chain - (see [this blog](https://dttung2905.medium.com/kafka-internals-kip-651-using-pem-certificate-for-ssl-listener-679b56cc9c59) for the details). At runtime, the keystore file will be available at `/certs/client.keystore`. In the [Properties](/docs/integrations/kafka.md#properties) configuration, set [ssl.keystore.location](https://kafka.apache.org/documentation/#producerconfigs_ssl.keystore.location) to `/certs/client.keystore`. Ensure that the required files are available on all Flink TaskManagers at the specified location.
2. Separate configuration using `ssl.keystore.key` and `ssl.keystore.certificate.chain`.
...
In case you use your own CA and client+server certificates authentication, you should additionally provide:
...
### Common part
The Kafka configuration is part of the Model configuration. All the settings below should be placed relative to `scenarioTypes.ScenarioTypeName.modelConfig` key. You can find the high level structure of the configuration file [here](/docs/OSS_docs/configuration/#configuration-areas)
...
| kafkaEspProperties.defaultOffsetResetStrategy                     | Medium     | string   | None          | If scenario is restarted, should offsets of source consumers be reset to latest (can be useful in test environments) or earliest, or consumers continue reading where previously stopped. Possible values: `None`, `ToEarliest`, `ToLatest`.                                                                                             |
...
| allowNotSuggestedTopicUsage                                       | Low        | boolean  | false         | Topic parameter will have extra switch allowing to manually enter the topic.                                                                                                                                                                                                                                                             |
| kafkaAdminConfig.clientTimeout                                    | Low        | duration | 10 seconds    | Timeout for communicating with Kafka cluster to fetch metadata                                                                                                                                                                                                                                                                           |
| kafkaAdminConfig.cacheConfig.topicsExpirationTime                 | Low        | duration | 30 seconds    | How long fetch topics will be cached                                                                                                                                                                                                                                                                                                     |
| kafkaAdminConfig.cacheConfig.autoCreateTopicSettingExpirationTime | Low        | duration | 5 minutes     | How long fetched `auto.create.topics.enable` setting will be cached                                                                                                                                                                                                                                                                      |
...
Following properties can be configured (please look at correct engine page : [Lite](/docs/OSS_docs/configuration/model/Lite#exception-handling) or [Flink](/docs/OSS_docs/configuration/model/Flink#configuring-exception-handling), 
...
| Name                                                 | Importance | Type                                     | Default value      | Description                                                                                                                                                                                                          |
|------------------------------------------------------|------------|------------------------------------------|--------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| kafkaEspProperties.defaultMaxOutOfOrdernessMilli   s | Medium     | long                                     | 60000 (60 seconds) | Configuration of [bounded out of orderness watermark generator](https://nightlies.apache.org/flink/flink-docs-master/docs/dev/datastream/event-time/built_in/#fixed-amount-of-lateness) used by Kafka sources        |
...
| optimizedGenericRecordSerialization.enabled          | Low        | boolean                                  | true               | Should Avro messages from topics registered in a Schema Registry be serialized in an optimized way, by serializing only schema id, not the whole schema                                                              |
| optimizedGenericRecordSerialization.schemaRegistryId | Medium     | int                                      |                    | Identifier of a Schema Registry instances, distinct for each separate Schema Registry environment; defaults to a hash of Schema Registry's URL, must be set so that a URL change won't make saved state incompatible |
