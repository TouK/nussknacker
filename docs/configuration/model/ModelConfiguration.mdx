...
The location and name of the configuration files is defined by the `CONFIG_FILE` environment variable. Consult [Basic environment variables](../Common.mdx#basic-environment-variables) for information on how this variable is resolved.
...
* Scenario Types configuration, comprising of:
    * [Model](./ModelConfiguration.mdx) configuration.
...
It’s important to remember that model configuration is prepared a bit differently. Please read [model configuration](./ModelConfiguration.mdx) for the details.
