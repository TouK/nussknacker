...
This is generic API that is used by Streaming-Lite engine, but can be used to implement own engines, e.g. running in Request-Response way, or embedded in your application.
...
- `Input` - the type of data (e.g. events processed by given engine). In Streaming-Lite it's `ConsumerRecord[Array[Byte], Array[Byte]]` 
- `Result` - the type of data produced by sinks of give engine. In Streaming-Lite it's `ProducerRecord[Array[Byte], Array[Byte]]`
...
in Streaming-Lite the batch contains all the records that are read with single `consumer.poll`.
                
Processing of a single batch ends with a list of results, each of the results may by either `Result` in case of succes, 
...
like union or for-each. If custom component depends on specific effect 
(e.g. in Streaming-Lite you want to invoke custom service which returns `Future[_]`),
...
with different effect, error will be returned by scenarion compiler. 
