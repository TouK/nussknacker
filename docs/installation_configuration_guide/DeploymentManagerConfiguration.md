...
| prometheusMetrics        | {enabled: boolean, port: int, agentConfigPath: String } | { enabled: false }                 | If `enabled`: true runtime image is run with jmx prometheus agent attached and Promethues metrics are exposed under defined `port`. Agent is run with default configuration (wich exposes basic jvm metrics) unless configured `agentConfigPath` wich is then used. |
