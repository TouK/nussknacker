...
| waitForDuringDeployFinish.maxChecks | boolean  | 60            | After Flink job execution, we check if tasks were successfully started on TaskMangers. This parameter describe how many times we should check it before notifying about deployment failure.                                       |
| waitForDuringDeployFinish.delay     | boolean  | 1 second      | After Flink job execution, we check if tasks were successfully started on TaskMangers. This parameter describe how long should be delay between checks.                                                                           |
