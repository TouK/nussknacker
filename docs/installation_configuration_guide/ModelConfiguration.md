...
| componentLinks                                     | Low        | `list[{id: string, title: string, icon: uri, url: url, supportedComponentTypes: option[List[string]]}]` | `[]`                   | Component's links configuration which are displayed at list of components. Fields `title`, `icon`, `url` can contain templates: `$componentId` nad `$componentName` which are replaced by component data. Param `supportedComponentTypes` means component's types which can support links. |  
