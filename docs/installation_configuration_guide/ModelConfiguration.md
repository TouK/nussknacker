...
| rocksDB.checkpointDataUri                          | High       | string   |                        | URI for storing checkpoints. Note: This **has** to be accessible from Nussknacker designer (for snapshot validation) **and** from Flink cluster (jobmanager and taskmanagers) |
...
| nkGlobalParameters.useTypingResultTypeInformation  | Low        | boolean  | false                  | Enables using Nussknacker additional typing information for state serialization. It makes serialization much faster, currently consider it as experimental                    |
