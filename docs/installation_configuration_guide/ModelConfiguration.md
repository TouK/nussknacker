...
| componentsAction                                   | Low        | map[string, {title: string, icon: string, url: option[string], types: option[List[string]]}] | map("usages" -> {...})  | Component's actions configuration which are displayed at list of components.  Fields `title`, `icon`, `url` can contain templates: `$componentId` nad `$componentName` which are replaced by component data. Parameter `url` isn't required because some of actions like `usages` are defaults FE actions. Url is required whe you want to do redirect to another page. Param `types` means component types which can support actions.  |
