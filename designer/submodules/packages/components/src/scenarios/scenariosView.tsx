...
function ScenariosView({ children, table }: PropsWithChildren<{ table?: boolean }>): JSX.Element {
  ...
  return (
    <FiltersContextProvider<ScenariosFiltersModel> getValueLinker={valueLinker}>
      ...
      {table ? (
        <TablePart data={data} isLoading={isLoading} />
      ) : (
...
export const ScenariosWithActions = (props: { addScenario?: () => void; addFragment?: () => void; isTable?: boolean}) => (
  <ScenariosView table={!!(props.isTable)}>
    <Actions {...props} />
  </ScenariosView>
);
