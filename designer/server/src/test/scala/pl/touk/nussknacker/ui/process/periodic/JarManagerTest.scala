...
import pl.touk.nussknacker.engine.management.periodic.flink.FlinkPeriodicDeploymentService
...
class JarManagerTest extends AnyFunSuite with Matchers with ScalaFutures with PatientScalaFutures {
  ...
  test("prepareDeploymentWithJar - should copy to local dir") {
    val result = jarManager.prepareDeploymentWithRuntimeParams(processVersion, process)
  ...
  test("prepareDeploymentWithJar - should handle disappearing model JAR") {
    ...
    def verifyAndDeleteJar(result: Future[DeploymentWithRuntimeParams[CanonicalProcess]]): Unit = {
    ...
    verifyAndDeleteJar(jarManager.prepareDeploymentWithRuntimeParams(processVersion, process))
  ...
  test("deleteJar - should delete both local and Flink jar") {
    ...
    jarManager.cleanAfterDeployment(RuntimeParams(Map("jarFileName" -> jarFileName))).futureValue

    Files.exists(jarPath) shouldBe false
  }

  test("deleteJar - should handle not existing file") {
    val result = jarManager.cleanAfterDeployment(RuntimeParams(Map("jarFileName" -> "unknown.jar"))).futureValue
