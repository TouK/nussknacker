...
trait ScenarioTestingApiHttpServiceSpec
  ...
  "The endpoint for running tests from file should" - {
    ...
    "return error for test data containing variable that doesn't match expected type" in {
      ...
      val response = runTestsFromFile(s"""[
                                         |  {"sourceId":"$exampleScenarioSourceId","variables":{"input":false},"timestamp":123}
                                         |]""".stripMargin)

      response.code shouldEqual StatusCode.BadRequest
      response.body should startWith(
        s"""Problem in sample 1 detected: Variable [name=input, type=${exampleScenarioInputVariableType.display}, encoded value=false] decoding error for source [$exampleScenarioSourceId]: """
      )
    }
  }
