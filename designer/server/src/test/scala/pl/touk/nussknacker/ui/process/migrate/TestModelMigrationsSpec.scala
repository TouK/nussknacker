...
class TestModelMigrationsSpec extends AnyFunSuite with Matchers {
  ...
  test("should perform test migration") {
    ...
    val results = testMigration.testMigrations(List(process), List(), ec)
  ...
  test("should migrate fragment and its usage within scenario") {
    ...
    val results = testMigration.testMigrations(List(wrapWithDetails(process)), List(wrapWithDetails(fragment)), ec)
