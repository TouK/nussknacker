...
class UIProcessValidatorSpec extends AnyFunSuite with Matchers with TableDrivenPropertyChecks with OptionValues {
  ...
  test("check for not unique CustomNodeOutput edges") {
    ...
    val process = createGraph(
      List(
        Source("in", SourceRef(ProcessTestData.existingSourceFactory, List())),
        CustomNode("custom", None, multiOutputTransformer, List()),
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("out2", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "custom", None),
        Edge("custom", "out", Some(EdgeType.CustomNodeOutput("rejected"))),
        Edge("custom", "out2", Some(EdgeType.CustomNodeOutput("rejected")))
      )
    )
  ...
  test("check for not unique main output edges on a node with a single main output") {
    ...
    val process = createGraph(
      List(
        Source("in", SourceRef(ProcessTestData.existingSourceFactory, List())),
        CustomNode("custom", None, ProcessTestData.existingStreamTransformer, List()),
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("out2", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "custom", None),
        Edge("custom", "out", None),
        Edge("custom", "out2", None)
      )
    )

    val result = validateWithConfiguredProperties(process)

    result.errors.invalidNodes.getOrElse("custom", Nil) should contain(
      NodeValidationError(
        typ = "NonUniqueEdgeType",
        message = "Edges are not unique",
  ...
  test("a node with only unnamed edges (e.g. Split) is not affected by the main output uniqueness check") {
    val process = createGraph(
      ...
      List(
        Edge("in", "split", None),
        Edge("split", "out", None),
        Edge("split", "out2", None)
      )
    )

    val result = validateWithConfiguredProperties(process)

    result.errors.invalidNodes.getOrElse("split", Nil).map(_.typ) should not contain "NonUniqueEdgeType"
  }
  ...
  test("reject a CustomNodeOutput edge leaving a non-custom node, leaving its own typed edges alone") {
    val process = createGraph(
      List(
        ...
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("out2", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("strayOut", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "filter", None),
        Edge("filter", "out", Some(EdgeType.FilterTrue)),
        Edge("filter", "out2", Some(EdgeType.FilterFalse)),
        Edge("filter", "strayOut", Some(EdgeType.CustomNodeOutput("rejected")))
      )
    )

    val result = validateWithOutputAwareValidator(process)

    val filterErrors = result.errors.invalidNodes.get("filter").value
  ...
  test("reject a CustomNodeOutput edge leaving a join, whose named outputs the conversion cannot keep") {
    val process = createGraph(
      List(
        Source("in", SourceRef(ProcessTestData.existingSourceFactory, List())),
        Join("join", None, "someJoin", List.empty, List.empty),
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "join", None),
        Edge("join", "out", Some(EdgeType.CustomNodeOutput("rejected")))
      )
    )

    val result = validateWithOutputAwareValidator(process)

    val joinErrors = result.errors.invalidNodes.get("join").value
  ...
  test("accept a CustomNodeOutput edge for an output the source component declares") {
    val process = createGraph(
      List(
        ...
        CustomNode("custom", None, multiOutputTransformer, List()),
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("rejectedOut", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "custom", None),
        Edge("custom", "out", Some(EdgeType.CustomNodeOutput("main"))),
        Edge("custom", "rejectedOut", Some(EdgeType.CustomNodeOutput("rejected")))
      )
    )
  ...
  test("report only MissingCustomNodeExecutor for a node whose component does not resolve, whatever its named edges") {
    val process = createGraph(
      List(
        Source("in", SourceRef(ProcessTestData.existingSourceFactory, List())),
        CustomNode("custom", None, "nonExistentComponent", List()),
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("otherOut", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "custom", None),
        Edge("custom", "out", Some(EdgeType.CustomNodeOutput("main"))),
        Edge("custom", "otherOut", Some(EdgeType.CustomNodeOutput("rejected")))
      )
    )

    val result = validateWithOutputAwareValidator(process)

    val customNodeErrorTypes = result.errors.invalidNodes.getOrElse("custom", Nil).map(_.typ)
  ...
  test("reject an unnamed edge next to a CustomNodeOutput edge even when the component does not resolve") {
    ...
    val errorTypesAndSeverities =
      result.errors.invalidNodes.get("custom").value.map(e => (e.typ, e.errorType))
  ...
  test("reject a typed edge next to a CustomNodeOutput edge even when the component does not resolve") {
    val process = createGraph(
      ...
      List(
        Edge("in", "custom", None),
        Edge("custom", "out", Some(EdgeType.CustomNodeOutput("rejected"))),
        Edge("custom", "otherOut", Some(EdgeType.FilterTrue))
      )
    )
  ...
  test("report the unnamed-edge problem once per node, not once per offending edge") {
    ...
    val missingNameErrors =
      result.errors.invalidNodes.get("custom").value.filter(_.typ == "MissingCustomNodeOutputName")
  ...
  test("reject a CustomNodeOutput edge leaving a single-output custom node, even one naming its main output") {
    forAll(Table("outputName", "main", "someOutput")) { outputName =>
      val process = createGraph(
        List(
          Source("in", SourceRef(ProcessTestData.existingSourceFactory, List())),
          CustomNode("custom", None, singleOutputTransformer, List()),
          Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List()))
        ),
        List(
          Edge("in", "custom", None),
          Edge("custom", "out", Some(EdgeType.CustomNodeOutput(outputName)))
        )
      )
  ...
  test("skip the CustomNodeOutput guard for an edge whose source node does not exist") {
    ...
    val process = createGraph(
      ...
      List(
        Edge("in", "out", None),
        Edge("phantom", "out", Some(EdgeType.CustomNodeOutput("rejected")))
      )
    )
    ...
    result.errors.invalidNodes.keys should not contain "phantom"
  }
  ...
  test("report a node reachable only through an edge from a nonexistent source as loose") {
    ...
    val process = createGraph(
      List(
        Source("in", SourceRef(ProcessTestData.existingSourceFactory, List())),
        Sink("out", SinkRef(ProcessTestData.existingSinkFactory, List())),
        Sink("orphan", SinkRef(ProcessTestData.existingSinkFactory, List()))
      ),
      List(
        Edge("in", "out", None),
        Edge("phantom", "orphan", None)
      )
    )
    val result = validateWithConfiguredProperties(process)
