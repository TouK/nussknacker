...
class ExtraScriptsListingPreparerTest extends AnyFunSuite with Matchers {
  ...
  test("should prepare correct listing based on classpath files") {
    val listingPreparer = new ExtraScriptsListingPreparer(classLoader, Path.of("web-extra"), Path.of("/resources-root"))
  ...
  test("should return empty list for not existing directory") {
    val listingPreparer = new ExtraScriptsListingPreparer(classLoader, Path.of("not-existing"), Path.of("/"))

    listingPreparer.webResourcesListing shouldBe empty
  }

  test("should return empty list for root as which is not a directory") {
    val listingPreparer = new ExtraScriptsListingPreparer(classLoader, Path.of("a.js"), Path.of("/"))
  ...
  test("should use scripts.lst file when available") {
    ...
    val listingPreparer = new ExtraScriptsListingPreparer(classLoader, Path.of("web-extra"), Path.of("/"))
