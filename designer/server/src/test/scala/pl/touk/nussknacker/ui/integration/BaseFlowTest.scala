...
import pl.touk.nussknacker.engine.testing.FlinkStreamingPropertiesConfig
...
//  private val mainRoute = new AkkaHttpBasedRouteProvider(
...
//  implicit val timeout: RouteTestTimeout = RouteTestTimeout(1.minute)
  ...
  test("ensure nodes config is properly parsed") {
    // todo: address from NuIttest
    val response = httpClient.send(
      basicRequest
        .get(uri"http://localhost:8080/api/processDefinitionData/streaming?isFragment=false")
        .auth.basic("admin", "admin")
    )

    val settingsJson = io.circe.parser.parse(response.body.right.get).right.get.hcursor.downField("componentsConfig").focus.get
  ...
  test("ensure additional properties config is properly applied") {
    ...
    val settingsJson = io.circe.parser.parse(response.body.right.get).right.get.hcursor.downField("additionalPropertiesConfig").focus.get
...
//  }
  ...
  private def saveProcess(process: CanonicalProcess) = {
    val response = httpClient.send(
      basicRequest
        .auth.basic("admin", "admin")
        .post(uri"http://localhost:8080/api/processes/${process.id}/Category1?isFragment=false")
    )
  ...
  private def updateProcess(process: CanonicalProcess) = {
    ...
    val response = httpClient
      .send(
        basicRequest
          .auth.basic("admin", "admin")
          .put(uri"http://localhost:8080//api/processes/$processId")
