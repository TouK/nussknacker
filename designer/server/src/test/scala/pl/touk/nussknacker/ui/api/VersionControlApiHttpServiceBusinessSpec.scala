...
class VersionControlApiHttpServiceBusinessSpec
  ...
  "The endpoint for version control should" - {
    "return the latest version when the scenario is unchanged" in {
      given()
        ...
        .statusCode(200)
        .equalsJsonBody(
          prepareResponseData(processName = processName.value, isLatest = true, localVersion = 1, latestVersion = 1)
        )
    }
    ...
    "return an updated latest version when the scenario has changed" in {
      given()
        ...
        .statusCode(200)
        .equalsJsonBody(
          prepareResponseData(processName = processName.value, isLatest = false, localVersion = 1, latestVersion = 2)
        )
    }
  ...
  private def prepareResponseData(processName: String, isLatest: Boolean, localVersion: Int, latestVersion: Int) =
    s"""
      |{
