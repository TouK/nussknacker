...
import pl.touk.nussknacker.engine.api.deployment.{DataFreshnessPolicy, DeploymentManager, ProcessActionId, ProcessActionState, ProcessActionType, ProcessState, StateStatus, StatusDetails}
...
class DeploymentServiceSpec extends AnyFunSuite with Matchers with PatientScalaFutures with DBIOActionValues with NussknackerAssertions
  ...
  test("should return state correctly when state is deployed") {
    ...
    deploymentManager.withWaitForDeployFinish(processName) {
      ...
      deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue.status shouldBe SimpleStateStatus.DuringDeploy
    }

    eventually {
      deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue.status shouldBe SimpleStateStatus.Running
    }
  }
  ...
  test("should return state correctly when state is cancelled") {
    ...
    deploymentManager.withWaitForCancelFinish {
      ...
      eventually {
        deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue.status shouldBe SimpleStateStatus.DuringCancel
      }
    }
  }
  ...
  test("Should mark finished process as finished") {
    ...
    checkIsFollowingDeploy(deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue, expected = true)
    fetchingProcessRepository.fetchLatestProcessDetailsForProcessId[Unit](processId).dbioActionValues.value.lastStateAction should not be None
    ...
    deploymentManager.withProcessFinished(processName, DeploymentId.fromActionId(deployActionId)) {
      ...
      (1 to 5).foreach { _ =>
        checkIsFollowingDeploy(deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue, expected = false)
      }
    ...
    val processDetails = fetchingProcessRepository.fetchLatestProcessDetailsForProcessId[Unit](processId).dbioActionValues.value
  ...
  test("Should finish deployment only after DeploymentManager finishes") {
    ...
    def checkStatusAction(expectedStatus: StateStatus, expectedAction: Option[ProcessActionType]) = {
      fetchingProcessRepository.fetchLatestProcessDetailsForProcessId[Unit](processId).dbioActionValues.flatMap(_.lastStateAction).map(_.actionType) shouldBe expectedAction
  ...
  test("Should skip notifications and deployment on validation errors") {
    val processName: ProcessName = generateProcessName
    val processId: ProcessId = prepareProcess(processName, Some(MockDeploymentManager.maxParallelism + 1)).dbioActionValues
    val processIdName = ProcessIdWithName(processId, processName)
    ...
    deploymentManager.withEmptyProcessState(processName) {
      ...
      fetchingProcessRepository.fetchLatestProcessDetailsForProcessId[Unit](processId).dbioActionValues.flatMap(_.lastStateAction) shouldBe None
      listener.events shouldBe Symbol("empty")
  ...
  test("Should return properly state when state is canceled and process is canceled") {
    ...
    deploymentManager.withProcessStateStatus(processName, SimpleStateStatus.Canceled) {
      deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue.status shouldBe SimpleStateStatus.Canceled
    }
  }
  ...
  test("Should return not deployed status for process with empty state - not deployed state") {
    val processName: ProcessName = generateProcessName
    val processId = prepareProcess(processName).dbioActionValues
    fetchingProcessRepository.fetchLatestProcessDetailsForProcessId[Unit](processId).dbioActionValues.value.lastStateAction shouldBe None

    deploymentManager.withEmptyProcessState(processName) {
      deploymentService.getProcessState(ProcessIdWithName(processId, processName)).futureValue.status shouldBe SimpleStateStatus.NotDeployed
    }
  ...
  private def preparedUnArchivedProcess(processName: ProcessName, actionType: Option[ProcessActionType]): DB[(ProcessId, Option[ProcessActionId])] =
    for {
