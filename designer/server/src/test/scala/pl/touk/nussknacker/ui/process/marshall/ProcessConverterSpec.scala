...
class ProcessConverterSpec extends AnyFunSuite with Matchers with TableDrivenPropertyChecks {
      ...
      val process = ValidatedDisplayableProcess(
        ...
        ValidationResult.errors(
          Map(unexpectedEnd.id -> List(
            NodeValidationError("InvalidTailOfBranch", "Scenario must end with sink or processor", "Scenario branch can only end with sink, processor or ending custom transformer", None, errorType = NodeValidationErrorType.SaveAllowed))),
          List.empty,
          List.empty
        )
      )
  ...
  test("return variable type information for process that cannot be canonized") {
    ...
    val process = ValidatedDisplayableProcess(
      ...
      Some(TestCategories.Category1),
      ValidationResult.errors(
        Map("e" -> List(NodeValidationError("InvalidTailOfBranch", "Scenario must end with sink or processor", "Scenario branch can only end with sink, processor or ending custom transformer", None, errorType = NodeValidationErrorType.SaveAllowed))),
        List.empty,
