  ...
  it should "return components for each user" in {
    ...
    forAll(testingData) {
      (user: LoggedUser, expectedComponents: List[ComponentListElement], possibleCategories: List[String]) =>
        val returnedComponentsWithUsages    = componentService.getComponentsList(withUsages = true)(user).futureValue
        val returnedComponentsWithoutUsages = componentService.getComponentsList(withUsages = false)(user).futureValue
  ...
  it should "throws exception when components are wrong configured" in {
    ...
    inside {
      intercept[TestFailedException] {
        componentService.getComponentsList(withUsages = true)(admin).futureValue
      }.cause
