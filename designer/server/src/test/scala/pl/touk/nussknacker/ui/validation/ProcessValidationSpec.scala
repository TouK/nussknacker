...
private object ProcessValidationSpec {
  ...
  private def createProcess(nodes: List[NodeData],
    ...
    DisplayableProcess("test", ProcessProperties(StreamMetaData(), additionalFields = Some(ProcessAdditionalFields(None, additionalFields))), nodes, edges, `type`, Some(category))
  }
