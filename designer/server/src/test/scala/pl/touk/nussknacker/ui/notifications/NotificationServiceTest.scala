...
class NotificationServiceTest extends AnyFunSuite with Matchers with PatientScalaFutures with MockitoSugar with WithHsqlDbTesting with EitherValuesDetailedMessage with OptionValues with DBIOActionValues {
  ...
  private val expectedRefreshAfterSuccess = List(DataToRefresh.versions, DataToRefresh.activity)
  ...
  test("Should return only events for user in given time") {
    ...
    val userForSuccess = TestFactory.adminUser(id = "successUser")
    val userForFail = TestFactory.adminUser("failUser")
  ...
  private def createDeploymentService(listener: NotificationsListener, deploymentManager: DeploymentManager) = {
