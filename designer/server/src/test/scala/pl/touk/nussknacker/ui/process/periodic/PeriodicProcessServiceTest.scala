...
class PeriodicProcessServiceTest
  ...
  test("handleFinished - should reschedule scenario if Flink job is missing") {
    ...
    val finished :: scheduled :: Nil =
      f.repository.deploymentEntities.map(createPeriodicProcessDeployment(processEntity, _, canonicalProcess)).toList
  ...
  test("handle first schedule") {
    ...
    f.events.toList shouldBe List(
      ScheduledEvent(
        createPeriodicProcessDeployment(processEntity, deploymentEntity, canonicalProcess),
        firstSchedule = true
      )
    )
  }
  ...
  test("handleFinished - should mark as failed for failed Flink job") {
    ...
    val expectedDetails =
      createPeriodicProcessDeployment(processEntity, f.repository.deploymentEntities.head, canonicalProcess)
  ...
  test("deploy - should deploy and mark as so") {
    ...
    ConfigFactory
      .parseString(f.jarManagerStub.lastDeploymentWithRuntimeParams.value.inputConfigDuringExecutionJson)
      .getString("runAt") shouldBe deploymentEntity.runAt.toString

    val expectedDetails =
      createPeriodicProcessDeployment(f.repository.processEntities.loneElement, deploymentEntity, canonicalProcess)
