...
object V1_057__MigrateActionsAndCommentsToScenarioActivitiesDefinition extends LazyLogging {
  ...
  class ProcessActionsDefinitions(val profile: ProfileWithDbSchema) {
    import profile.jdbcProfile.api._

    val table: LTableQuery[ProcessActionEntity] = LTableQuery(new ProcessActionEntity(_))

    class ProcessActionEntity(tag: Tag)
        extends Table[ProcessActionEntityData](tag, Some(profile.schemaName), "process_actions") {
  ...
  class CommentsDefinitions(val profile: ProfileWithDbSchema) {
    import profile.jdbcProfile.api._
    val table: LTableQuery[CommentEntity] = LTableQuery(new CommentEntity(_))

    class CommentEntity(tag: Tag) extends Table[CommentEntityData](tag, Some(profile.schemaName), "process_comments") {
