...
case class ProcessIllegalAction(message: String) extends Exception(message) with ForbiddenError

object ProcessIllegalAction {

  def apply(actionName: String, processIdWithName: ProcessIdWithName, state: ProcessState): ProcessIllegalAction =
    ProcessIllegalAction(s"Action: $actionName is not allowed in scenario (${processIdWithName.name.value}) state: ${state.status.name}, allowed actions: ${state.allowedActions.mkString(",")}.")
