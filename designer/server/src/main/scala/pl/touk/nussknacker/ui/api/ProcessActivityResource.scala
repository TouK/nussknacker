...
class AttachmentResources(
  ...
  def securedRoute(implicit user: LoggedUser): Route = {
    ...
    } ~ path("processes" / ProcessNameSegment / VersionIdSegment / "activity" / "attachments" / LongNumber) {
      (processName, versionId, attachmentId) => // FIXME: version isn't used, remove it or figure out why is not used
