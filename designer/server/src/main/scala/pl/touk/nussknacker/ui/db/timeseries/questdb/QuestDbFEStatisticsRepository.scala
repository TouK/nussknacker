...
import pl.touk.nussknacker.ui.db.timeseries.questdb.QuestDbExtensions.{CairoEngineExtension, RecordCursorFactoryExtension}
import pl.touk.nussknacker.ui.db.timeseries.questdb.QuestDbFEStatisticsRepository.{createTableQuery, recover, selectQuery, tableName}
...
// todo: check how apply works for 1 mln stats
private class QuestDbFEStatisticsRepository(private val engine: AtomicReference[CairoEngine], private val clock: Clock)(
  ...
  override def read(): Future[Map[String, Long]] = withExceptionHandling(() => {
    recordCursorPool.get().fetch(sqlContext.get()) { record =>
