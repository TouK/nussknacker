  ...
  def computeCounts(canonicalProcess: CanonicalProcess, isFragment: Boolean, counts: NodeId => Option[RawCount])(
    ...
    def computeCountsForSingle(prefixes: List[NodeId])(nodes: Iterable[CanonicalNode]): Map[NodeId, NodeCount] = {
      ...
      nodes.flatMap {
        ...
        case FilterNode(node, nextFalse) => computeCountsSamePrefixes(nextFalse) + (NodeId(node.id) -> nodeCount(NodeId(node.id)))
        ...
        case SplitNode(node, nexts)  => computeCountsSamePrefixes(nexts.flatten) + (NodeId(node.id) -> nodeCount(NodeId(node.id)))
