...
class AutoRefreshableCache[KEY, VALUE](
  ...
  private val keysToRefresh: Cache[KEY, Unit] = Caffeine
    .newBuilder()
    .expireAfterWrite(java.time.Duration.ofMillis(config.autoRefreshDurationSinceLastUsage.toMillis))
    .build[KEY, Unit]()

  private val cache: Cache[KEY, (VALUE, VALUE_CREATOR)] = Caffeine
    .newBuilder()
