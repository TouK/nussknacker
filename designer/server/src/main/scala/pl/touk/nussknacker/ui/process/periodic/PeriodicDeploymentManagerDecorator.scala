...
import pl.touk.nussknacker.engine.api.definition.{MandatoryExpressionValidator, StaticStringParameterEditor}
...
object PeriodicDeploymentManagerDecorator extends LazyLogging {
  ...
  private val cronConfig = CronSchedulePropertyExtractor.CronPropertyDefaultName -> ScenarioPropertyConfig(
    ...
    validators = Some(List(MandatoryExpressionValidator, CronParameterValidator)),
    label = Some("Schedule"),
