...
object UIProcessObjectsFactory {
  ...
  def prepareUIProcessObjects(
    ...
    val uiProcessDefinition =
      createUIProcessDefinition(staticObjectsDefinition, fragmentInputs, uiClazzDefinitions, processCategoryService)
    ...
    val finalComponentsConfig = ComponentDefinitionPreparer.combineComponentsConfig(
    ...
    val defaultAsyncInterpretation: DefaultAsyncInterpretationValue =
      DefaultAsyncInterpretationValueDeterminer.determine(defaultUseAsyncInterpretationFromConfig)
  ...
  def createUIFragmentObjectDefinition(
