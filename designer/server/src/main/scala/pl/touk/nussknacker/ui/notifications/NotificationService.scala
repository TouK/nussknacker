...
class NotificationServiceImpl(
  ...
  override def notifications(implicit user: LoggedUser, ec: ExecutionContext): Future[List[Notification]] = {
    ...
    dbioRunner
      ...
      .map(_.map {
        case (
              ProcessActionEntityData(id, _, _, _, _, _, _, actionName, ProcessActionState.Finished, _, _, _),
              processName
            ) =>
          Notification.actionFinishedNotification(id.toString, actionName, processName)
        case (
              ProcessActionEntityData(id, _, _, _, _, _, _, actionName, ProcessActionState.ExecutionFinished, _, _, _),
              processName
            ) =>
