...
ALTER TABLE "processes"
    ADD COLUMN "latest_execution_finished_action_id" INTEGER;
...
SET "latest_version_id"                   = (SELECT pv."id"
    ...
    "latest_finished_action_id"           = (SELECT pa."id"
                                             ...
                                             WHERE pa."process_id" = p."id"
                                               AND pa."state" = 'FINISHED'
                                             ORDER BY pa."created_at" DESC
                                             LIMIT 1),
    "latest_execution_finished_action_id" = (SELECT pa."id"
                                             FROM "process_actions" pa
                                             WHERE pa."process_id" = p."id"
                                               AND pa."state" = 'EXECUTION_FINISHED'
                                             ORDER BY pa."created_at" DESC
                                             LIMIT 1);
...
ALTER TABLE "processes"
    ADD CONSTRAINT "fk_process_latest_execution_finished_action" FOREIGN KEY ("id", "latest_execution_finished_action_id") REFERENCES "process_actions" ("process_id", "id") ON DELETE CASCADE;
