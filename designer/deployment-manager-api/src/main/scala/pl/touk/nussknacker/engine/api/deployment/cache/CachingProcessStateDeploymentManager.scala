  ...
  override def getProcessState(name: ProcessName)(implicit freshnessPolicy: DataFreshnessPolicy): Future[WithDataFreshnessStatus[Option[StatusDetails]]] = {
    freshnessPolicy match {
      case DataFreshnessPolicy.Fresh =>
        ...
        cache.put(name, resultFuture.map(_.value.toList).toJava.toCompletableFuture)
        resultFuture
      case DataFreshnessPolicy.CanBeCached =>
        Option(cache.getIfPresent(name))
          .map(_.toScala.map(processStates => WithDataFreshnessStatus(processStates.headOption, cached = true)))
          .getOrElse {
            cache.get(name, (_, _) =>
              delegate.getProcessState(name).map(_.value.toList).toJava.toCompletableFuture
            ).toScala.map(processStates => WithDataFreshnessStatus(processStates.headOption, cached = false))
          }
    }
  }
