...
class HttpService {
            ...
            .catch((error) => {
                if (error?.response?.status != 400) {
                    return this.#addError(
                        i18next.t("notification.error.failedToDeploy", "Failed to deploy {{processName}}", { processName: processName }),
                        error,
                        true,
                    ).then((error) => {
    ...
    cancel(processName, comment?) {
        return api.post(`/processManagement/cancel/${encodeURIComponent(processName)}`, comment).catch((error) => {
            if (error?.response?.status != 400) {
                return this.#addError(
                    i18next.t("notification.error.failedToCancel", "Failed to cancel {{processName}}", { processName: processName }),
                    error,
                    true,
                ).then((error) => {
                    return { isSuccess: false };
    ...
    downloadAttachment(processName: ProcessName, processVersionId: ProcessVersionId, attachmentId, fileName: string) {
        return api
            .get(`/processes/${encodeURIComponent(processName)}/${processVersionId}/activity/attachments/${attachmentId}`, {
                responseType: "blob",
            })
