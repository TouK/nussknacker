...
describe("json utils", () => {
    ...
    it("should flatten object #2", () => {
        const obj = {
            service: {
                parameters: [{ expression: { expression: "'foo'" } }, { expression: { expression: "'bar'" } }],
            },
        };
        expect(JsonUtils.flattenObj(obj)).toEqual({
    ...
    it("should diff objects", () => {
        ...
        const obj2 = {
            service: {
                parameters: [{ expression: { expression: "'foo2'" } }, { expression: { expression: "'bar'" } }],
            },
        };

        expect(JsonUtils.objectDiff(obj1, obj2)).toEqual({
            service: {
                parameters: [{ expression: { expression: "'foo'" } }],
            },
        });
    });
