...
import pl.touk.nussknacker.ui.security.oauth2.OAuth2ErrorHandler.{OAuth2AccessTokenRejection, OAuth2CollectiveException, OAuth2ServerError}
...
(implicit backend: SttpBackend[Future, Nothing, NothingT]) extends LazyLogging {
  ...
  protected[security] def handlingResponse[T](response: Response[Either[ResponseError[Error], T]], clientErrorMessage: String): Future[Response[Either[ResponseError[Error], T]]] = {
    if (response.code.isClientError) {
      ...
      Future.failed(throw OAuth2CollectiveException(NonEmptyList.of(OAuth2AccessTokenRejection(clientErrorMessage))))
    } else if (response.isSuccess) {
      Future.successful(response)
    } else {
      logger.debug(s"Handling ServerError response: ${response}")
      Future.failed(throw OAuth2CollectiveException(NonEmptyList.of(OAuth2ServerError(s"OAuth2 Server error: ${response}"))))
    }
  }
}
