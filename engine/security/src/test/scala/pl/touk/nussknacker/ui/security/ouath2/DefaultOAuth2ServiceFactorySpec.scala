...
import pl.touk.nussknacker.ui.security.oauth2.{DefaultOAuth2Service, DefaultOAuth2ServiceFactory, OAuth2AuthenticateData, OAuth2Configuration, OAuth2ErrorHandler}
...
class DefaultOAuth2ServiceFactorySpec extends FlatSpec with Matchers with ScalaFutures with Suite  {
  ...
  val config: OAuth2Configuration = ExampleOAuth2ServiceFactory.testConfig

  def createErrorOAuth2Service(uri: URI, code: StatusCode): DefaultOAuth2Service = {
    implicit val testingBackend: SttpBackendStub[Future, Nothing] = SttpBackendStub
      .asynchronousFuture
