...
class AuthenticationOAuth2ResourcesSpec extends FunSpec with Matchers with ScalatestRouteTest with FailFastCirceSupport {
  ...
  def routes(route: AuthenticationOAuth2Resources): Route = route.route()

  protected lazy val errorAuthenticationResources: AuthenticationOAuth2Resources = {
  ...
  protected lazy val badAuthenticationResources: AuthenticationOAuth2Resources = {
    implicit val testingBackend = SttpBackendStub
      .asynchronousFuture
      .whenRequestMatches(_.uri.equals(Uri(config.accessTokenUri)))
  ...
  protected lazy val authenticationResources: AuthenticationOAuth2Resources = {
    implicit val testingBackend = SttpBackendStub
      .asynchronousFuture
      .whenRequestMatches(_.uri.equals(Uri(config.accessTokenUri)))
  ...
  def authenticationOauth2(resource: AuthenticationOAuth2Resources, authorizeToken: String): RouteTestResult = {
