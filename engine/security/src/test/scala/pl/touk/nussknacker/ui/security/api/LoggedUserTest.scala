...
class LoggedUserTest extends FunSuite with Matchers {
  ...
  test("Admin permission grants other permissions") {
    def admin(cp: Map[String, Set[Permission]]) = LoggedUser("1", Option.apply("admin"), cp, Nil, true)
    ...
    forAll(perms) { (user: LoggedUser, p: Permission, c: String) =>
      ...
      user.getName shouldBe "admin"
    }
  }

  test("check user permission in category") {
    def u(m: Map[String, Set[Permission]]) = LoggedUser("user", Option.empty, categoryPermissions = m)
