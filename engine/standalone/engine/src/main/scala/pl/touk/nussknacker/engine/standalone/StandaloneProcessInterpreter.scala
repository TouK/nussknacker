...
object StandaloneProcessInterpreter {
  ...
  : CompilationResult[StandaloneProcessInterpreter] = modelData.withThisAsContextClassLoader {
    ...
    compilerData.compile().andThen { compiledProcess =>
      ...
      val intepreterCompilation = StandaloneInvokerCompiler(compiledProcess, compilerData).compile
      val lifecycle = compilerData.lifecycle(intepreterCompilation.services)
      intepreterCompilation.map(_.run).map { case (sinkTypes, invoker) =>
  ...
  private case class StandaloneInvokerCompiler(compiledProcess: CompiledProcessParts, processCompilerData: ProcessCompilerData) {
    ...
    private def compileWithCompilationErrors(node: SplittedNode[_], validationContext: ValidationContext): CompilationResult[Node] =
      processCompilerData.subPartCompiler.compile(node, validationContext)(compiledProcess.metaData)
    ...
    private def compiledPartInvoker(processPart: ProcessPart): CompilationResult[WithSinkTypes[InterpreterType]] = processPart match {
      ...
      case CustomNodePart(transformerObj, node, _, validationContext, parts, _) =>
        ...
        CompilationResult(validatedTransformer).andThen { transformer =>
...
case class StandaloneProcessInterpreter(source: StandaloneSource[Any],
                                        ...
                                        private val lifecycle: LifecycleHandler,
  ...
  def open(jobData: JobData): Unit = modelData.withThisAsContextClassLoader {
    lifecycle.open(jobData) {
      case a:StandaloneContextLifecycle =>
        a.open(jobData, context)
    }
  }
