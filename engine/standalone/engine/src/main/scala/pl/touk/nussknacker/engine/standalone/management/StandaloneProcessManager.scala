...
class StandaloneProcessManager(modelData: ModelData, client: StandaloneProcessClient)
  ...
  override def deploy(processVersion: ProcessVersion, processDeploymentData: ProcessDeploymentData,
                      savepointPath: Option[String], user: String): Future[Unit] = {
    savepointPath match {
  ...
  override def stop(name: ProcessName, savepointDir: Option[String], user: String): Future[SavepointResult] = {
  ...
  override def cancel(name: ProcessName, user: String): Future[Unit] = {
    client.cancel(name)
  }
