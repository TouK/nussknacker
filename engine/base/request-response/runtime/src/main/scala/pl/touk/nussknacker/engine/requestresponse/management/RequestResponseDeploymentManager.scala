...
object RequestResponseDeploymentManager {
  def apply(modelData: ModelData, config: Config)(implicit ec: ExecutionContext, sttpBackend: SttpBackend[Future, Nothing, NothingT]): RequestResponseDeploymentManager =
    new RequestResponseDeploymentManager(modelData, StandaloneProcessClient(config))
}

class RequestResponseDeploymentManager(modelData: ModelData, client: StandaloneProcessClient)(implicit ec: ExecutionContext)
