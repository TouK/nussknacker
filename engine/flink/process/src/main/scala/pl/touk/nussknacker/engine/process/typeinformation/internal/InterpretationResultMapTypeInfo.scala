...
case class InterpretationResultMapTypeSerializer(ctx: Map[String, TypeSerializer[InterpretationResult]]) extends TypeSerializer[InterpretationResult] {

  override def isImmutableType: Boolean = ctx.forall(_._2.isImmutableType)

  override def duplicate(): TypeSerializer[InterpretationResult] = InterpretationResultMapTypeSerializer(ctx.mapValuesNow(_.duplicate()))
