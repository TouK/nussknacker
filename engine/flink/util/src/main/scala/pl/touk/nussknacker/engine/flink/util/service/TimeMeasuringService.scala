...
trait TimeMeasuringService extends GenericTimeMeasuringService with WithMetrics with LazyLogging {
  ...
  override def espTimer(tags: Map[String, String], name: String): EspTimer = {
    ...
    if (metricUtils == null) {
      ...
      val meter = metricUtils.gauge[Double, InstantRateMeter](NonEmptyList.of("serviceInstant", name), tags, new InstantRateMeter)
      ...
      val registered = metricUtils.histogram(NonEmptyList.of("serviceTimes", name), tags, histogram)
