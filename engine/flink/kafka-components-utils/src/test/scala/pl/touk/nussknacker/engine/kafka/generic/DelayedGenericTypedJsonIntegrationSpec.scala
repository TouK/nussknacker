...
class DelayedGenericTypedJsonIntegrationSpec
    extends AnyFunSuite
    with FlinkSpec
    with Matchers
    with KafkaSpec
    with KafkaSourceFactoryProcessMixin {
  ...
  private val serializationSchema: String => serialization.KafkaSerializationSchema[Any] =
    (topic: String) =>
      new JsonSerializationSchema[BasicEvent](topic).asInstanceOf[serialization.KafkaSerializationSchema[Any]]

  override protected lazy val creator: ProcessConfigCreator = new DelayedDefaultProcessConfigCreator

  private val now: Long = System.currentTimeMillis()

  private def givenObj(timestamp: Long = now) =
    BasicEvent(id = "123", name = "kafka-generic-delayed-test", timestamp = Some(timestamp))
  ...
  test("properly process data using kafka-generic-delayed source") {
    ...
    val process = createProcessWithDelayedSource(
      topic,
      BasicEvent.definition,
      s"'${BasicEvent.timestampFieldName}'",
  ...
  test("null timestamp should raise exception") {
    val recordOk =
      new ConsumerRecord[String, TypedMap]("dummy", 1, 1L, "", TypedMap(Map("msisdn" -> "abc", "ts" -> 1625751296000L)))
    TypedJsonTimestampFieldAssigner("ts").extractTimestamp(recordOk, 123L) shouldEqual 1625751296000L

    val recordWithNull =
      new ConsumerRecord[String, TypedMap]("dummy", 1, 1L, "", TypedMap(Map("msisdn" -> "abc", "ts" -> null)))
  ...
  test("handle invalid negative param") {
    ...
    val process =
      createProcessWithDelayedSource(topic, BasicEvent.definition, s"'${BasicEvent.timestampFieldName}'", "-10L")
    intercept[IllegalArgumentException] {
      runAndVerify(topic, process, givenObj())
  ...
  private def createProcessWithDelayedSource(
      topic: String,
      definition: String,
    ...
    ScenarioBuilder
      .streaming("kafka-generic-delayed-test")
      .parallelism(1)
      .source(
        "start",
...
class DelayedDefaultProcessConfigCreator extends EmptyProcessConfigCreator {
  ...
  override def sourceFactories(
    ...
    Map(
      "kafka-generic-delayed" -> defaultCategory(
        new DelayedGenericTypedJsonSourceFactory(
          RecordFormatterFactory.fixedRecordFormatter(JsonRecordFormatter),
