...
class InputMetaDeserializationSpec extends AnyFunSuite with Matchers with FlinkTypeInformationSerializationMixin {
  ...
  test("should serialize and deserialize input metadata with TypeInformation serializer") {
    ...
    val givenObj = InputMeta[SampleKey](
      SampleKey("one", 2),
      "dummy",
      3,
      4L,
  ...
  test(
    "should serialize and deserialize input metadata with TypingResultAwareTypeInformationDetection and customisations"
  ) {
    val inputMetaTypingResult = InputMeta.withType(Typed[SampleKey])
    val typeInformation: TypeInformation[InputMeta[SampleKey]] =
      TypingResultAwareTypeInformationDetection(getClass.getClassLoader).forType(inputMetaTypingResult)
    val givenObj = InputMeta[SampleKey](
      SampleKey("one", 2),
      "dummy",
      3,
