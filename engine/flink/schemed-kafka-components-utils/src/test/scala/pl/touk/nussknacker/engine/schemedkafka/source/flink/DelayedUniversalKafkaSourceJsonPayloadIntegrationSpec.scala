...
class DelayedUniversalKafkaSourceJsonPayloadIntegrationSpec extends DelayedUniversalKafkaSourceIntegrationMixinSpec {
  ...
  test("properly process data using kafka-generic-delayed source") {
    val inputTopic = TopicName.OfSource("simple-topic-with-long-field-input")
    registerJsonSchema(inputTopic.toUncategorizedTopicName, LongFieldV1.jsonSchema, isKey = false)
  ...
  test("handle timestamp field in Int format") {
    val inputTopic = TopicName.OfSource("simple-topic-with-int-field-input")
    registerJsonSchema(inputTopic.toUncategorizedTopicName, IntFieldV1.jsonSchema, isKey = false)
    val process = createProcessWithDelayedSource(inputTopic, ExistingSchemaVersion(1), "'field'", "1L")
    runAndVerify(inputTopic, process, IntFieldV1.exampleData)
  }

  test("handle timestamp field in ZonedDateTime format") {
    val inputTopic = TopicName.OfSource("simple-topic-with-zoned-date-time-field-input")
    registerJsonSchema(inputTopic.toUncategorizedTopicName, ZoneDateTimeFieldJsonV1.jsonSchema, isKey = false)
    val process = createProcessWithDelayedSource(inputTopic, ExistingSchemaVersion(1), "'field'", "1L")
    runAndVerify(inputTopic, process, ZoneDateTimeFieldJsonV1.exampleData)
  }

  test("handle timestamp field in OffsetDateTime format") {
    val inputTopic = TopicName.OfSource("simple-topic-with-offset-date-time-field-input")
    registerJsonSchema(inputTopic.toUncategorizedTopicName, OffsetDateTimeFieldJsonV1.jsonSchema, isKey = false)
    val process = createProcessWithDelayedSource(inputTopic, ExistingSchemaVersion(1), "'field'", "1L")
