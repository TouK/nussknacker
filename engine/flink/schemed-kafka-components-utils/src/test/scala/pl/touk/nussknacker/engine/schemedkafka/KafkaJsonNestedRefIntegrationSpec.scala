  ...
  private lazy val creator: KafkaAvroTestProcessConfigCreator = new KafkaAvroTestProcessConfigCreator {
    ...
    override def sinkFactories(modelDependencies: ProcessObjectDependencies): Map[String, WithCategories[SinkFactory]] =
      super.sinkFactories(modelDependencies) ++ Map(
        "sinkForAny" -> defaultCategory(SinkForAny.toSinkFactory),
      )
  ...
  protected def runAndVerify(
    ...
    run(process) {
      eventually {
        ...
        SinkForAny.data should have size givenObj.size
      }
      SinkForAny.data.toString() shouldEqual expectedObj.toString()
    }
  }
  ...
  protected def createProcess(topic: String): CanonicalProcess = {
    ...
    ScenarioBuilder
      ...
      .source(
        ...
        SchemaVersionParamName  -> asSpelExpression(formatVersionParam(ExistingSchemaVersion(1))),
        TimestampFieldParamName -> "null",
        DelayParameterName      -> "null"
      )
      .emptySink(
        "out",
        "sinkForAny",
