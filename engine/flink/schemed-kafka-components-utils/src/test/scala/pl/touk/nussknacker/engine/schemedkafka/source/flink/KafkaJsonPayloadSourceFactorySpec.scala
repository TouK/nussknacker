...
import pl.touk.nussknacker.engine.schemedkafka.helpers.{
  KafkaSchemaRegistryMixin,
  KafkaSourceSpecificTestDataMixin,
...
class KafkaJsonPayloadSourceFactorySpec
    extends AnyFunSuite
  ...
  override def resolveModelConfig(config: Config): Config =
    super
      ...
      .withValue(s"$kafkaComponentsConfigPrefix.avroAsJsonSerialization", fromAnyRef(true))
