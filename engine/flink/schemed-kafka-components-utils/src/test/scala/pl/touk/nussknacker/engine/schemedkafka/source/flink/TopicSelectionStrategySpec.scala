...
class TopicSelectionStrategySpec extends KafkaAvroSpecMixin with KafkaAvroSourceSpecMixin {
  ...
  test("show how to override topic selection strategy") {
    new UniversalKafkaSourceFactory(schemaRegistryClientFactory, UniversalSchemaBasedSerdeProvider.create(schemaRegistryClientFactory), testProcessObjectDependencies, new FlinkKafkaSourceImplFactory(None)) {
