...
class AggregatorFunctionWrapper[MapT[K,V]](aggregator: Aggregator, nodeId: NodeId, aggregateElementType: TypingResult)
  ...
  override def processElement(value: InTyp,
    ...
    val fun = new LatelyEvictableStateFunction[InTyp, ValueWithContext[AnyRef], MapT[Long, AnyRef]]
      with AggregatorFunctionMixin[MapT] {
      override val nodeId: NodeId = nodeId

      override protected val aggregator: Aggregator = aggregator

      override protected def timeWindowLengthMillis: Long = value.value.value._1.toMillis

      override protected def aggregateElementType: TypingResult = aggregateElementType

      override protected implicit def rangeMap: FlinkRangeMap[MapT] = ???

      override def processElement(value: InTyp, ctx: KeyedProcessFunction[String, InTyp, ValueWithContext[AnyRef]]#Context, out: Collector[ValueWithContext[AnyRef]]): Unit = ???
    }
