...
import pl.touk.nussknacker.engine.api.context.{DefinitionContext, SingleInputGenericNodeTransformation, TransformationResult, ValidationContext}
...
object LastVariableFilterTransformer extends CustomStreamTransformer with
  WithExplicitMethodToInvoke with SingleInputGenericNodeTransformation[FlinkCustomStreamTransformation] {

  override def parameterDefinition: List[Parameter] = contextTransformation(None).parameters
  ...
  override def contextTransformation(context: Option[DefinitionContext[ValidationContext]]): TransformationResult[FlinkCustomStreamTransformation] = {
    val params = context.map(_.params).getOrElse(Map.empty)
    val validationContext = context.map(_.context)
    ...
    TransformationResult(errors, List(valueParameter, conditionParameter), Nil, validationContext)
  }
