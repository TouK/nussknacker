...
class DeploymentActorTest extends AnyFunSuite
  ...
  private def shouldFindToBeDeployedScenarios(result: Future[Seq[PeriodicProcessDeployment]]): Unit = {
    val probe = TestProbe()
    var counter = 0
    ...
    val actor = system.actorOf(DeploymentActor.props(
      findToBeDeployed,
      deploy = _ => fail("should not be called"),
      interval))
