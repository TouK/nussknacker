...
import pl.touk.nussknacker.engine.management.periodic.model.{PeriodicProcessDeploymentId, PeriodicProcessDeploymentState, PeriodicProcessDeploymentStatus, PeriodicProcessId, ScheduleId, ScheduleName}
...
class PeriodicProcessStateDefinitionManagerTest extends AnyFunSuite with Matchers {
  ...
  test("display periodic deployment status for not named schedule") {
    val deploymentStatus = DeploymentStatus(generateDeploymentId, notNamedScheduleId, fooRunAt, PeriodicProcessDeploymentStatus.Scheduled, processActive = true, None)
  ...
  test("display periodic deployment status for named schedules") {
    ...
    val firstDeploymentStatus = DeploymentStatus(generateDeploymentId, firstScheduleId, fooRunAt, PeriodicProcessDeploymentStatus.Scheduled, processActive = true, None)
    val secScheduleId = generateScheduleId
    val secDeploymentStatus = DeploymentStatus(generateDeploymentId, secScheduleId, fooRunAt, PeriodicProcessDeploymentStatus.Deployed, processActive = true, None)
