  ...
  it should "run scenario in test mode" in {
    ...
    whenReady(deploymentManager.test(ProcessName(processId), process, scenarioTestData)) { r =>
      r.nodeResults shouldBe Map(
        "startProcess" -> List(ScenarioProcessingContext(s"$processId-startProcess-0-0", Map("input" -> "terefere"))),
        "nightFilter"  -> List(ScenarioProcessingContext(s"$processId-startProcess-0-0", Map("input" -> "terefere"))),
        "endSend"      -> List(ScenarioProcessingContext(s"$processId-startProcess-0-0", Map("input" -> "terefere")))
      )
    }
  }
