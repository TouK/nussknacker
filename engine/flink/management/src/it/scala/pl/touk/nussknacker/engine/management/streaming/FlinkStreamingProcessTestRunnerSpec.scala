...
class FlinkStreamingProcessTestRunnerSpec extends AnyFlatSpec with Matchers with VeryPatientScalaFutures {
  ...
  it should "run scenario in test mode" in {
    ...
    whenReady(deploymentManager.test(ProcessName(processId), process, TestData.asJsonStrings("terefere"), identity)) { r =>
  ...
  it should "return correct error messages" in {
    ...
    val caught = intercept[IllegalArgumentException] {
      Await.result(deploymentManager.test(ProcessName(processId), process, TestData.asJsonStrings("terefere"), _ => null), patienceConfig.timeout)
    }
