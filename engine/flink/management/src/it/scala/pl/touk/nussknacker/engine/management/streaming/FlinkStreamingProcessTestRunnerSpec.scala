...
class FlinkStreamingProcessTestRunnerSpec extends FlatSpec with Matchers with VeryPatientScalaFutures {
  ...
  it should "run scenario in test mode" in {
    ...
    whenReady(deploymentManager.test(ProcessName(processId), canonicalProcess , TestData.newLineSeparated("terefere"), identity)) { r =>
  ...
  it should "return correct error messages" in {
    ...
    val caught = intercept[IllegalArgumentException] {
      Await.result(deploymentManager.test(ProcessName(processId), canonicalProcess, TestData.newLineSeparated("terefere"), _ => null), patienceConfig.timeout)
    }
