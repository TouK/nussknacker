...
object DocumentationFunctions {
    ...
    override def computeResultType(
      ...
      case left :: right :: Nil =>
        (left.withoutValue, right.withoutValue) match {
          ...
          case (l, r) if List(l, r).forall(_.canBeImplicitlyConvertedTo(numberType)) =>
  ...
  private class GetFieldGenericFunction extends TypingFunction {
    ...
    override def computeResultType(
      ...
      case TypedObjectTypingResult(_, _, _) :: x :: Nil if x.canBeImplicitlyConvertedTo(stringType) =>
