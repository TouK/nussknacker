...
object DynamicMultipleParamsService
  ...
  override def contextTransformation(context: ValidationContext,
                                    ...
                                    (implicit nodeId: NodeId): DynamicMultipleParamsService.NodeTransformationDefinition = {
    case TransformationStep(Nil, _) =>
      val fooParam = Parameter("foo", Typed[String]).copy(editor = Some(FixedValuesParameterEditor(List(
    ...
    case TransformationStep(("foo", DefinedEagerParameter(fooValue, _)) :: ("bar", DefinedEagerParameter(barValue: String, _)) :: Nil, _) =>
      NextParameters(List(Parameter("baz", Typed[String]).copy(defaultValue = Some(Expression.spel(s"'$fooValue' + '-' + '$barValue'")))))
    case TransformationStep(("foo", DefinedEagerParameter(fooValue, _)) :: ("bar", DefinedEagerParameter(_, _)) :: ("baz", DefinedEagerParameter(_, _)) :: Nil, _) =>
