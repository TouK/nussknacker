...
object ProcessTestHelpers extends Serializable {
  ...
  def prepareComponents(data: List[SimpleRecord]): List[ComponentDefinition] = List(
    ...
    ComponentDefinition("skewMonitor", SinkFactory.noParam(SkewedMonitorEmptySink)),
...
case object SkewedMonitorEmptySink extends EmptySink with Serializable {

  val invocationsCount = new AtomicInteger(0)

  private var skewValue: Int = _
