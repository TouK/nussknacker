...
class MetricsSpec
  ...
  test("does not report a custom node as a scenario end because of its additional output") { implicit scenarioName =>
    ...
    counter("nodeId.multi1.nodeCount") shouldBe 3L
    counter("end.nodeId.multi1.count") shouldBe 0L
  }
  ...
  test("counts records taking an unwired main output as dead ends") { implicit scenarioName =>
    ...
    counter("nodeId.multi1.nodeCount") shouldBe 3L
    counter("nodeId.rejected-out.nodeCount") shouldBe 2L
    counter("dead_end.nodeId.multi1.count") shouldBe 1L
    counter("end.nodeId.multi1.count") shouldBe 0L
  }
