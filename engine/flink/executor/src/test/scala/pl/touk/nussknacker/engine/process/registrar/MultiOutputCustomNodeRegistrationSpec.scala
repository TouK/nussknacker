...
class MultiOutputCustomNodeRegistrationSpec extends FlinkStreamGraphSpec with LoneElement with OptionValues {
  ...
  test("should reject a scenario where main output and an additional output feed the same join") {
    ...
    val exception = the[ScenarioCompilationErrors] thrownBy streamGraph(scenario)

    exception.errors.loneElement match {
      case ProcessCompilationError.MultipleOutputsToSameJoin(nodeIds) =>
        nodeIds.map(_.value) shouldBe Set("multi1")
      case other => fail(s"Unexpected compilation error: $other")
    }
  }
  ...
  test("should reject at compilation a connected additional output served by a single-output transformation") {
    ...
    exception.errors.loneElement match {
      case ProcessCompilationError.CustomNodeError(nodeId, message, _) =>
        nodeId.value shouldBe "multi1"
        message should include("Additional outputs of this component are not supported on this engine")
      case other => fail(s"Unexpected compilation error: $other")
    }
  }
