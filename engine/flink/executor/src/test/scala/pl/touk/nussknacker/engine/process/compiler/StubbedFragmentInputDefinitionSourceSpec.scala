...
class StubbedFragmentInputDefinitionSourceSpec extends AnyFunSuite with Matchers  {
  ...
  test("should generate test parameters for fragment input definition") {
    ...
    val stubbedSource = new StubbedFragmentInputDefinitionSource(ConfigFactory.empty, getClass.getClassLoader)
    val parameters: Seq[Parameter] = stubbedSource.createSourceDefinition(fragmentInputDefinition)
      .implementationInvoker.invokeMethod(Map.empty, None, Seq.empty).asInstanceOf[TestWithParametersSupport[Map[String, Any]]].testParametersDefinition
