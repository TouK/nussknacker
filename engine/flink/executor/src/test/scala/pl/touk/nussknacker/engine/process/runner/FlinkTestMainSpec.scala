...
class FlinkTestMainSpec extends AnyFunSuite with Matchers with Inside with BeforeAndAfterEach {
  ...
  test("be able to return test results") {
    ...
    results.externalInvocation("proc2") shouldBe List(ExternalInvocation("proc1-id-0-1", "logService", "0-collectedDuringServiceInvocation"))
    results.externalInvocation("out") shouldBe List(ExternalInvocation("proc1-id-0-1", "valueMonitor", 11))
    results.externalInvocation("eager1") shouldBe List(ExternalInvocation("proc1-id-0-1", "collectingEager", "static-s-dynamic-0"))
  ...
  test("return correct result for custom node") {
    ...
    results.externalInvocation("out") shouldBe
      List(
        ExternalInvocation("proc1-id-0-0", "valueMonitor", "1 0"),
        ExternalInvocation("proc1-id-0-1", "valueMonitor", "11 1")
      )
  }
  ...
  test("handle custom multiline source input") {
    ...
    results.externalInvocation("out") shouldBe
      List(
        ExternalInvocation("proc1-id-0-0", "valueMonitor", SimpleJsonRecord("1", "11")),
        ExternalInvocation("proc1-id-0-1", "valueMonitor", SimpleJsonRecord("2", "22")),
        ExternalInvocation("proc1-id-0-2", "valueMonitor", SimpleJsonRecord("3", "33"))
      )
  }
  ...
  test("handle custom variables in source") {
    ...
    results.externalInvocation("out") shouldBe
      List(
        ExternalInvocation("proc1-id-0-0", "valueMonitor", "transformed:abc|3")
      )
  }
  ...
  test("should handle joins for one input (diamond-like) ") {
    ...
    results.externalInvocation("proc2").map(_.value.asInstanceOf[String]).sorted shouldBe List("a", "b", "c", "c").map(_ + "-collectedDuringServiceInvocation")
  }
