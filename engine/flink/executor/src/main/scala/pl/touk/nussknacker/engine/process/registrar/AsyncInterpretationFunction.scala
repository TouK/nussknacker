  ...
  private def invokeInterpreter(
    ...
    if (useIOMonad) {
      ...
      compilerData.interpreter
        .interpret[IO](compiledNode, compilerData.jobRuntimeData, input, serviceExecutionContext)
        .unsafeRunAsync(callback)
    } else {
      implicit val executionContext: ExecutionContext = SynchronousExecutionContextAndIORuntime.syncEc
      compilerData.interpreter
        .interpret[Future](compiledNode, compilerData.jobRuntimeData, input, serviceExecutionContext)
