...
class TestFlinkProcessCompiler(creator: ProcessConfigCreator,
  ...
  override protected def prepareSourceFactory(sourceFactory: ObjectWithMethodDef,
                                              context: ComponentDefinitionContext): ObjectWithMethodDef = {
  ...
  override protected def prepareService(service: ObjectWithMethodDef,
                                        context: ComponentDefinitionContext): ObjectWithMethodDef = service
...
class StubbedSourcePreparer(classloader: ClassLoader,
                            ...
                            metaData: MetaData,
                            scenarioTestData: ScenarioTestData) {
  def prepareStubbedSource(originalSource: Source with FlinkSourceTestSupport[Object],
                           returnTypeOpt: Option[typing.TypingResult],
                           nodeId: NodeId): FlinkSource = {
    val samples: List[Object] = collectSamples(originalSource, nodeId)
    val returnType = returnTypeOpt.getOrElse(throw new IllegalStateException(s"${originalSource.getClass} extends FlinkSourceTestSupport and has no return type"))
    originalSource match {
      case providerWithTransformation: FlinkIntermediateRawSource[Object@unchecked] =>
        new CollectionSource[Object](samples, originalSource.timestampAssignerForTest, returnType)(providerWithTransformation.typeInformation) {
          override val contextInitializer: ContextInitializer[Object] = providerWithTransformation.contextInitializer
        }
      case _ =>
        new CollectionSource[Object](samples, originalSource.timestampAssignerForTest, returnType)(originalSource.typeInformation)
    }
  }

  private def collectSamples(originalSource: Source,
                             nodeId: NodeId): List[Object] = {
    val testDataPreparer = new TestDataPreparer(classloader, expressionConfig, dictRegistry, typeDefinitionSet, metaData)
