...
object CommonTestDataFormatStubbedSourcePreparer {
  ...
  private case class CommonTestDataFormatStubbedSource(
    ...
    override final def contextStream(
      ...
      // FIXME abr: set up the correct type information
      val typeInformation = TypeInformationDetection.instance.forType[DataRecord](Unknown)
      val streamOfRaw     = env.fromCollection(list.asJava, typeInformation)
