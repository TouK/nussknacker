...
case class TypedMapTypeInformation(informations: Map[String, TypeInformation[_]])
    extends TypedObjectBasedTypeInformation[TypedMap](informations) {
  override def createSerializer(serializers: Array[(String, TypeSerializer[_])]): TypeSerializer[TypedMap] =
    TypedMapSerializer(serializers)
}

@SerialVersionUID(1L)
case class TypedMapSerializer(override val serializers: Array[(String, TypeSerializer[_])])
    extends TypedObjectBasedTypeSerializer[TypedMap](serializers)
    with BaseJavaMapBasedSerializer[Any, TypedMap] {
