...
import pl.touk.nussknacker.engine.api.component.{ComponentAdditionalConfig, ComponentDependencies, DesignerWideComponentId, NodesDeploymentData}
...
class FlinkProcessCompilerDataFactory(
  ...
  private def prepareDefaultListeners(usedNodes: UsedNodes) = {
    ...
    List(
      LoggingListener,
      new NodeCountingListener(usedNodes.nodes.filterNot(nodesHandledInNextParts).map(n => NodeId(n.id)) ++ usedNodes.nextParts),
