...
class EventGeneratorSourceFactory(customTimestampAssigner: TimestampWatermarkHandler[AnyRef])
  ...
  def create(
    ...
    new StandardFlinkSource[AnyRef]
      ...
      // Temporary version for discussion - two variants are implemented at the same time, with temporary switch between them

      // private val useAutoGenerated   = ParameterName("useAutoGenerated")
      // private val valueParameterName = ParameterName("value")

      override def testParametersDefinition: List[Parameter] = List(
//        Parameter(
        //          name = useAutoGenerated,
        //          typ = Typed[Boolean],
        //          editor = None,
        ...
        //          typ = value.returnType.withoutValue,
        //          editor = None,
        //          validators = List.empty,
        //          defaultValue = None,
        //          additionalVariables = Map.empty,
...
//        params.get(useAutoGenerated).map(_.asInstanceOf[Boolean]) match {
//          case None | Some(true) =>
//            value.evaluate(Context("dummy"))
//          case Some(false) =>
//            params(valueParameterName)
//        }
        value.evaluate(Context("dummy"))
      }
