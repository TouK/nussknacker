...
object RowConversions {
  ...
  def rowToContext(row: Row): Context = {
    ...
    Context(
      id = deserializeContextId(row.getField(0).asInstanceOf[String]),
      variables = rowToScalaMap(row.getField(1).asInstanceOf[Row]),
      // Order of serialization traceId and parent are replaced, because parent can contain many fields
      parentContext = Option(row).filter(_.getArity >= 4).map(_.getField(3).asInstanceOf[Row]).map(rowToContext),
      traceId = Option(row.getField(2)).map(_.asInstanceOf[String]).map(TraceId(_))
    )
  }
  ...
  implicit class TypeInformationDetectionExtension(typeInformationDetection: TypeInformationDetection) {
    ...
    def contextRowTypeInfo(validationContext: ValidationContext): TypeInformation[_] = {
      ...
      val parentTypeInfo = validationContext.parent.map(contextRowTypeInfo).toList

      Types.ROW(Types.STRING :: variablesRow :: Types.STRING :: parentTypeInfo: _*)
    }
