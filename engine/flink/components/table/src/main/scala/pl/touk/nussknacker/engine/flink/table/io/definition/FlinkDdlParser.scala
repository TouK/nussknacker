...
object FlinkDdlParser {
  ...
  def parseUnsafe(sqlStatements: String): List[FlinkSqlDdlStatement] =
    parse(sqlStatements).fold(e => throw new IllegalArgumentException(e.head.message, e.head.cause.orNull), identity)
  ...
  private def extractDefinitions(sql: List[SqlNode]): ValidatedNel[FlinkDdlParseError, List[FlinkSqlDdlStatement]] = {
    sql.traverse {
      ...
      case table: SqlCreateTable => {
        ...
        // Here there is an assumption that table from CREATE TABLE gets registered in the default database and
        // default catalog.
        val tableName = TableName(table.getTableName.getSimple)
        extractOptions(table.getPropertyList)
          .find(_.key == "connector")
          .toValidNel(MissingConnectorOption(sqlString))
          .map(_.value)
          .map { connector =>
            CreateTable(tableName, sqlString, Connector(connector))
          }
      }
