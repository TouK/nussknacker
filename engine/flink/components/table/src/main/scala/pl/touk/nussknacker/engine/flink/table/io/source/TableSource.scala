...
import pl.touk.nussknacker.engine.flink.table.typing.SchemaExtensions._
...
class TableSource(
  ...
  override def contextStream(
    ...
    val finalQuery = flinkNodeContext.componentUseContext.deploymentData
      .flatMap(_.get(SQL_FILTERING_EXPRESSION_PARAMETER_NAME))
  ...
  override def generateTestData(maxNumberOfRecords: Int): TestData = {
    ...
    testDataGenerationMode match {
      ...
      case TestDataGenerationMode.Live =>
        flinkDataDefinition.registerIn(environmentForTestingPurposes).orFail
...
object TableSource {
  ...
  val SQL_FILTERING_EXPRESSION_PARAMETER_NAME = "sqlExpression"
