...
class FullOuterJoinTransformer(timestampAssigner: Option[TimestampWatermarkHandler[TimestampedValue[ValueWithContext[AnyRef]]]])
  ...
  override def contextTransformation(contexts: Map[String, ValidationContext], dependencies: List[NodeDependencyValue])(implicit nodeId: NodeId): NodeTransformationDefinition = {
    case TransformationStep(Nil, _) =>
      ...
      NextParameters(
        List(KeyParam, AggregatorParam, AggregateByParam, WindowLengthParam).map(_.parameter), errors_names ++ errors_key)
    ...
    (`KeyParamName`, _) ::
      ...
      val validatedOutputType: ValidatedNel[ProcessCompilationError, TypingResult] = aggregateByParam match {
        case DefinedLazyBranchParameter(aggregateByByBranchId) =>
          val validatedAggregatorReturnTypes = aggregatorByBranchId.map{case (id, agg) =>
            agg.computeOutputType(aggregateByByBranchId(id).returnType)
              .leftMap(x => {
  ...
  override def implementation(params: Map[String, Any], dependencies: List[NodeDependencyValue], finalState: Option[State]): FlinkCustomJoinTransformation = {
    ...
    (inputs: Map[String, DataStream[Context]], context: FlinkCustomNodeContext) => {
      val keyedStreams = inputs.map {
        case (id, stream) =>
          stream.flatMap(new StringKeyedValueMapper(context, keyByBranchId(id), aggregateByByBranchId(id)))
            ...
            .returns(ValueWithContextType.infoBranch[StringKeyedValue[AnyRef]](context, id, Typed.fromDetailedType[StringKeyedValue[AnyRef]]))
      }
