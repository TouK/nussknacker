...
import pl.touk.nussknacker.defaultmodel.MockSchemaRegistry.{RecordSchemaStringV1, RecordSchemaV1}
...
class StateCompatibilityTest extends FlinkWithKafkaSuite with Eventually with LazyLogging {
  ...
  val outputSchema: Schema = AvroUtils.parseSchema(outputSchemaString)
  ...
  ignore("should create savepoint and save to disk") {
    val inputConfig = createAndRegisterTopicConfig(inTopic, RecordSchemaV1)
    val outputConfig = createAndRegisterTopicConfig(outTopic, RecordSchemaV1)

    val clusterClient = flinkMiniCluster.asInstanceOf[FlinkMiniClusterHolderImpl].getClusterClient
    sendAvro(givenMatchingAvroObj, inputConfig.input)

    run(stateCompatibilityProcess(inputConfig.input, outputConfig.input), { jobExecutionResult =>
  ...
  test("should restore from snapshot") {
    val inputConfig = createAndRegisterTopicConfig(inTopic, RecordSchemaV1)
    val outputConfig = createAndRegisterTopicConfig(outTopic, outputSchema)

    val existingSavepointLocation = Files.list(savepointDir).iterator().asScala.toList.head
    val env = flinkMiniCluster.createExecutionEnvironment()
    val process1 = stateCompatibilityProcess(inputConfig.input, outputConfig.input)
    ...
    sendAvro(givenNotMatchingAvroObj, inputConfig.input)
