  ...
  test("should handle exceptions in kafka sinks") {
    ...
    val configCreator = new EmptyProcessConfigCreator {
      ...
      override def sinkFactories(processObjectDependencies: ProcessObjectDependencies): Map[String, WithCategories[SinkFactory]] = {
        ...
        Map(
          "kafka-avro" -> WithCategories(new BaseKafkaAvroSinkFactoryWithEditor(provider, processObjectDependencies) with FlinkKafkaAvroSinkFactory),
          "kafka-avro-raw" -> WithCategories(new BaseKafkaAvroSinkFactory(provider, processObjectDependencies) with FlinkKafkaAvroSinkFactory),
        )
      }
    }
