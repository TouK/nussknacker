...
import pl.touk.nussknacker.engine.avro.{KafkaAvroSpec, TestSchemaRegistryClientFactory, TopicConfig}
...
class ConfluentKafkaAvroDeserializationSpec extends KafkaAvroSpec with TableDrivenPropertyChecks {
  ...
  test("trying to deserialize generated wrong type record") {
    val schemas = List(PaymentV1.schema, PaymentV2.schema)
    val fromRecordTopic = TopicConfig("wrong.from-record", schemas)
    val fromSubjectVersionTopic = TopicConfig("from-subject-version", schemas)
