...
import pl.touk.nussknacker.engine.kafka.generic.KafkaScheduledSourceFactory._
import pl.touk.nussknacker.engine.kafka.generic.{BaseKafkaDelayedSourceFactory, Schedule}
...
// TODO: try merging this source factory with DelayedKafkaAvroSourceFactory
class ScheduledKafkaAvroSourceFactory[K: ClassTag, V: ClassTag](schemaRegistryProvider: SchemaRegistryProvider,
  ...
  override def paramsDeterminedAfterSchema: List[Parameter] = super.paramsDeterminedAfterSchema ++ List(
    TimestampFieldParameter, DaysParameter, HourFromParameter, HourToParameter
  )
  ...
  override protected def nextSteps(context: ValidationContext, dependencies: List[NodeDependencyValue])
    ...
    (`topicParamName`, DefinedEagerParameter(topic: String, _)) ::
      ...
      (DaysParameterName, DefinedEagerParameter(delay, _)) ::
      (HourFromParameterName, DefinedEagerParameter(hourFrom, _)) ::
      (HourToParameterName, DefinedEagerParameter(hourTo, _)) :: Nil, _) =>
      ...
      valueValidationResult match {
        case Valid((valueRuntimeSchema, typingResult)) =>
          ...
          val errors = timestampValidationErrors
    ...
    case step@TransformationStep((`topicParamName`, _) :: (SchemaVersionParamName, _) :: (TimestampFieldParamName, _) ::
      (DaysParameterName, _) :: (HourFromParameterName, _) :: (HourToParameterName, _) :: Nil, _) =>
  ...
  override protected def createSource(params: Map[String, Any],
    ...
    val days = extractDays(params)
    val hourFrom = extractHourFrom(params)
    val hourTo = extractHourTo(params)
    val schedule = Schedule(days, hourFrom, hourTo)
    ...
    createScheduledKafkaSource[K, V](preparedTopics, kafkaConfig, deserializationSchema, timestampAssignerWithExtract, formatter, flinkContextInitializer, schedule)
  }
