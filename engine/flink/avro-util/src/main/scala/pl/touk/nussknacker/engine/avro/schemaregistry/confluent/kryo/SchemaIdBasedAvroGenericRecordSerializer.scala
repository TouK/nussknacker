...
class SchemaIdBasedAvroGenericRecordSerializer(schemaRegistryClientFactory: ConfluentSchemaRegistryClientFactory, kafkaConfig: KafkaConfig)
  ...
  private lazy val schemaRegistry = schemaRegistryClientFactory.createSchemaRegistryClient(kafkaConfig).client

  protected lazy val encoderFactory: EncoderFactory = EncoderFactory.get
  ...
  private def writeDataBytes(record: GenericRecordWithSchemaId, bos: ByteArrayOutputStream): Unit = {
    val writer = createDatumWriter(record, record.getSchema, useSchemaReflection = false, useStringForStringSchema = false)
  ...
  private def readRecord(lengthOfData: Int, schemaId: Int, dataBuffer: Array[Byte]) = {
    ...
    val reader = createDatumReader(writerSchema, writerSchema, useSchemaReflection = false, useSpecificAvroReader = false, useStringForStringSchema = false)
