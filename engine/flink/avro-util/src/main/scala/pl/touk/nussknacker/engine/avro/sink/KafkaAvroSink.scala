...
/**
  ...
  * @param clientId
  */
class KafkaAvroSink(topic: String, output: LazyParameter[Any], kafkaConfig: KafkaConfig, kafkaAvroSchemaProvider: KafkaAvroSchemaProvider[_], clientId: String)
    ...
    dataStream
      ...
      .map(ctx => ctx.value match {
          ...
          case _ => {
            ...
            logger.error(s"Invalid output type error for topic: $topic.", ctx)
            null
          }
        }
      )
  ...
  private def toFlinkFunction: SinkFunction[Any] =
    PartitionByKeyFlinkKafkaProducer(kafkaConfig, topic, kafkaAvroSchemaProvider.serializationSchema, clientId)
}
