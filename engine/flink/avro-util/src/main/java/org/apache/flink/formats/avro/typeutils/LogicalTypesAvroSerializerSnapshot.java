 ...
 */
public class LogicalTypesAvroSerializerSnapshot<T> implements TypeSerializerSnapshot<T> {
	...
	LogicalTypesAvroSerializerSnapshot(Schema schema, Class<T> runtimeType) {
		this.schema = schema;
	...
	resolveSchemaCompatibility(TypeSerializer<T> newSerializer) {
		if (!(newSerializer instanceof LogicalTypesAvroSerializer)) {
			return TypeSerializerSchemaCompatibility.incompatible();
		}
		LogicalTypesAvroSerializer<?> newAvroSerializer = (LogicalTypesAvroSerializer<?>) newSerializer;
	...
	public TypeSerializer<T> restoreSerializer() {
		...
		if (runtimeSchema != null) {
			return new LogicalTypesAvroSerializer<>(runtimeType, new SerializableAvroSchema(runtimeSchema), new SerializableAvroSchema(schema));
		}
		else {
			return new LogicalTypesAvroSerializer<>(runtimeType, new SerializableAvroSchema(schema), new SerializableAvroSchema(schema));
		}
	}
