...
import static org.apache.flink.formats.avro.typeutils.LogicalTypesSchemaIdAvroSerializer.isGenericRecord;
 ...
 */
public class LogicalTypesSchemaIdAvroSerializerSnapshot<T> implements TypeSerializerSnapshot<T> {
	...
	public LogicalTypesSchemaIdAvroSerializerSnapshot() {
		// this constructor is used when restoring from a checkpoint.
	}

	LogicalTypesSchemaIdAvroSerializerSnapshot(Schema schema, Class<T> runtimeType, int schemaId) {
		this.schema = schema;
	...
	resolveSchemaCompatibility(TypeSerializer<T> newSerializer) {
		if (!(newSerializer instanceof LogicalTypesSchemaIdAvroSerializer)) {
			return TypeSerializerSchemaCompatibility.incompatible();
		}
		LogicalTypesSchemaIdAvroSerializer<?> newAvroSerializer = (LogicalTypesSchemaIdAvroSerializer<?>) newSerializer;
	...
	public TypeSerializer<T> restoreSerializer() {
		...
		if (runtimeSchema != null) {
			return new LogicalTypesSchemaIdAvroSerializer<>(runtimeType, new NkSerializableAvroSchema(runtimeSchema), new NkSerializableAvroSchema(schema), schemaId);
		}
		else {
			return new LogicalTypesSchemaIdAvroSerializer<>(runtimeType, new NkSerializableAvroSchema(schema), new NkSerializableAvroSchema(schema), schemaId);
		}
	}
