...
import pl.touk.nussknacker.engine.kafka.source.{KafkaGenericContextInitializer, KafkaGenericNodeSourceFactory}
...
class KafkaGenericNodeSourceFactorySpec extends FunSuite with Matchers with KafkaSpec with PatientScalaFutures with KafkaGenericNodeMixin {
  ...
  protected lazy val StringSourceFactory: KafkaGenericNodeSourceFactory[Any, Any] = {
    val processObjectDependencies = ProcessObjectDependencies(config, ObjectNamingProvider(getClass.getClassLoader))
    val deserializationSchemaFactory = new ConsumerRecordDeserializationSchemaFactory(stringDeserializationSchema, stringDeserializationSchema)
    val testDataRecordFormatter = new ConsumerRecordToJsonFormatter
    val sourceFactory = new KafkaGenericNodeSourceFactory(deserializationSchemaFactory, None, testDataRecordFormatter, processObjectDependencies)
    sourceFactory.asInstanceOf[KafkaGenericNodeSourceFactory[Any, Any]]
  }

  protected lazy val SampleEventSourceFactory: KafkaGenericNodeSourceFactory[Any, Any] = {
    val processObjectDependencies = ProcessObjectDependencies(config, ObjectNamingProvider(getClass.getClassLoader))
    val deserializationSchemaFactory = new ConsumerRecordDeserializationSchemaFactory(stringDeserializationSchema, sampleValueDeserializationSchema)
    val testDataRecordFormatter = new ConsumerRecordToJsonFormatter
    val sourceFactory = new KafkaGenericNodeSourceFactory(deserializationSchemaFactory, None, testDataRecordFormatter, processObjectDependencies)
    sourceFactory.asInstanceOf[KafkaGenericNodeSourceFactory[Any, Any]]
  }

  protected lazy val ConsumerRecordValueSourceFactory: KafkaGenericNodeSourceFactory[Any, Any] = {
    val processObjectDependencies = ProcessObjectDependencies(config, ObjectNamingProvider(getClass.getClassLoader))
    val deserializationSchemaFactory = new ConsumerRecordDeserializationSchemaFactory(stringDeserializationSchema, sampleValueDeserializationSchema)
    val testDataRecordFormatter = new ConsumerRecordToJsonFormatter
    val sourceFactory = new KafkaGenericNodeSourceFactory(deserializationSchemaFactory, None, testDataRecordFormatter, processObjectDependencies)
  ...
  protected lazy val ConsumerRecordKeyValueSourceFactory: KafkaGenericNodeSourceFactory[Any, Any] = {
    val processObjectDependencies = ProcessObjectDependencies(config, ObjectNamingProvider(getClass.getClassLoader))
  ...
  private def readLastMessage(sourceFactory: KafkaGenericNodeSourceFactory[Any, Any], topic: String, numberOfMessages: Int = 1): List[AnyRef] = {
  ...
  private def createSource(sourceFactory: KafkaGenericNodeSourceFactory[Any, Any], topic: String): Source[AnyRef] with TestDataGenerator with FlinkSourceTestSupport[AnyRef] with ReturningType = {
  ...
  test("read and deserialize from simple string source") {
    ...
    val expectedObj = new ConsumerRecord[String, String](topic, 0, 0L, constTimestamp, TimestampType.CREATE_TIME, 0L, -1, 11, null, givenObj, ConsumerRecordUtils.emptyHeaders)
