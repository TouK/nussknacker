...
class ConsumerRecordToJsonFormatterSpec extends FunSuite with Matchers with KafkaSpec with BeforeAndAfterAll with KafkaSourceFactoryMixin {
  ...
  private lazy val sampleKeyValueFormatterFactory = new ConsumerRecordToJsonFormatterFactory[SampleKey, SampleValue]
  private lazy val sampleKeyValueFormatter = sampleKeyValueFormatterFactory.create(kafkaConfig, deserializationSchemaFactory.create(List(topic), kafkaConfig))
  ...
  test("prepare and parse test data from ConsumerRecord with key, with headers") {
    ...
    val sampleKeyBytes = Encoder[SampleKey].apply(sampleKey).noSpaces.getBytes(cs)
    val sampleValueBytes = Encoder[SampleValue].apply(sampleValue).noSpaces.getBytes(cs)
  ...
  test("should raise exception when key is expected and empty") {
    ...
    intercept[Exception] {
      val resultBytes = sampleKeyValueFormatter.prepareGeneratedTestData(List(givenObj))
      sampleKeyValueFormatter.parseDataForTest(topic, resultBytes).head
