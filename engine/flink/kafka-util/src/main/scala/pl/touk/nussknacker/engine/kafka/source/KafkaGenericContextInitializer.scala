...
/**
  * KafkaGenericContextInitializer initializes Context variables with data provided in raw kafka event (see [[org.apache.kafka.clients.consumer.ConsumerRecord]]).
  ...
  */
class KafkaGenericContextInitializer[K: ClassTag, V: ClassTag, DefinedParameter <: BaseDefinedParameter, State] extends BasicFlinkGenericContextInitializer[ConsumerRecord[K, V], DefinedParameter, State] {
  ...
  override def validationContext(context: ValidationContext, dependencies: List[NodeDependencyValue], parameters: List[(String, DefinedParameter)], state: Option[State])
    ...
    contextWithInput.withVariable(VariableConstants.InputMetaVariableName, InputMeta.withType[K], None).getOrElse(contextWithInput)
  }
  ...
  override protected def outputVariableType(context: ValidationContext, dependencies: List[NodeDependencyValue], parameters: List[(String, DefinedParameter)], state: Option[State])
                                           (implicit nodeId: NodeId): TypingResult = Typed[V]
