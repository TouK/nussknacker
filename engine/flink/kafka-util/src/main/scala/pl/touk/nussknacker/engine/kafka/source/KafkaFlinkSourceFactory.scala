...
/**
  ...
  *     Fetching data is defined in [[pl.touk.nussknacker.engine.kafka.source.KafkaFlinkSource]] which
  ...
  *
  * @param deserializationSchemaFactory - produces KafkaDeserializationSchema for raw [[pl.touk.nussknacker.engine.kafka.source.KafkaFlinkSource]]
  ...
  * */
class KafkaFlinkSourceFactory[K: ClassTag, V: ClassTag](deserializationSchemaFactory: KafkaDeserializationSchemaFactory[ConsumerRecord[K, V]],
  ...
  protected def topicsValidationErrors(topic: String)(implicit nodeId: ProcessCompilationError.NodeId): List[ProcessCompilationError.CustomNodeError] = {
      ...
      validateTopics(preparedTopics).swap.toList.map(_.toCustomNodeError(nodeId.id, Some(KafkaFlinkSourceFactory.TopicParamName)))
  }
  ...
  protected def nextSteps(context: ValidationContext, dependencies: List[NodeDependencyValue])(implicit nodeId: ProcessCompilationError.NodeId): NodeTransformationDefinition = {
    case step@TransformationStep((KafkaFlinkSourceFactory.TopicParamName, DefinedEagerParameter(topic: String, _)) :: _, None) =>
    ...
    case step@TransformationStep((KafkaFlinkSourceFactory.TopicParamName, _) :: _, None) =>
  ...
  protected val topicParameter: ParameterWithExtractor[String] =
    ParameterWithExtractor.mandatory[String](
      KafkaFlinkSourceFactory.TopicParamName,
...
object KafkaFlinkSourceFactory {
