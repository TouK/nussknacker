...
import pl.touk.nussknacker.engine.testmode.Collectors.CollectorMode.CollectorMode
...
object Collectors {
  ...
  private[testmode] object CollectorMode extends Enumeration {
    type CollectorMode = Value
    val Test, Query, Production = Value
  }

  trait BaseServiceInvocationCollector extends ServiceInvocationCollector {
  ...
  case class QueryServiceInvocationCollector(runIdOpt: Option[TestRunId], serviceName: String) extends BaseServiceInvocationCollector {
  ...
  case class TestServiceInvocationCollector(runIdOpt: Option[TestRunId],
                                            contextId: ContextId,
                                            nodeId: NodeId,
                                            serviceRef: String) extends BaseServiceInvocationCollector {
