...
object DictTyper {
  ...
  val typingDictKeys: DictTyper = new DictTyperImpl {
    override protected def possibleValuesFromDict(dict: TypedDict): Iterable[String] = dict.labelById.keys
  }

  val typingDictLabels: DictTyper = new DictTyperImpl {
    override protected def possibleValuesFromDict(dict: TypedDict): Iterable[String] = dict.labelById.values
  }
