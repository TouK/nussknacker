...
class TyperSpec extends FunSuite with Matchers {
  ...
  private val disableDynamicPropertyAccess = true
  ...
  private val typer = new Typer(this.getClass.getClassLoader, commonSupertypeFinder, new KeysDictTyper(dict), strictMethodsChecking,
    staticMethodInvocationsChecking, TypeDefinitionSet.empty, evaluationContextPreparer = null,
    disableMethodExecutionForUnknown, disableDynamicPropertyAccess)(ClassExtractionSettings.Default)
