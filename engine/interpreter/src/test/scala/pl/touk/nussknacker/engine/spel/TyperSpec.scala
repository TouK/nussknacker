...
class TyperSpec extends FunSuite with Matchers {
  ...
  private val referenceTypeValidating = false
  ...
  private val typer = new Typer(this.getClass.getClassLoader, commonSupertypeFinder, new KeysDictTyper(dict), strictMethodsChecking, typeDefinitionSet = TypeDefinitionSet(), referenceTypeValidating)(ClassExtractionSettings.Default)
