...
class TypedClassDisplaySpec extends FunSuite with Matchers with EitherValues {
  ...
  test("parsing nested arrays display") {
    Typed.typedClass(classOf[Array[Array[String]]], List(Typed(classOf[String]))).display should equal("Array[Array[String]]")
  }
