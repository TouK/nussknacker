...
class ProcessValidatorSpec extends FunSuite with Matchers with Inside {
  ...
  private val baseDefinition = ProcessDefinition[ObjectDefinition](
    ...
    ExpressionDefinition(
      ...
      List.empty, LanguageConfiguration.default, optimizeCompilation = false, strictTypeChecking = true
    ),
  ...
  test("find usage of unresolved plain variables") {
    ...
    validate(process, baseDefinition).result should matchPattern {
      ...
      ExpressionParseError("Unresolved reference 'doesNotExist1'", "sampleFilter", Some("Expression"), _),
      List(ExpressionParseError("Unresolved reference 'doesNotExist2'", "sampleFilter", Some("Expression"), _)))) =>
    }
  }
  ...
  test("find usage of non references") {
    val process = EspProcessBuilder
      ...
      .emptySink("id2", "sink")
    validate(process, baseDefinition).result should matchPattern {
      case Invalid(NonEmptyList(ExpressionParseError("Non reference 'input' occurred. Maybe you missed '#' in front of it?", "sampleFilter2", Some("Expression"), _), _)) =>
    }
  }
  ...
  test("find usage of fields that does not exist in object") {
    ...
    validate(process, definitionWithTypedSource).result should matchPattern {
      case Invalid(NonEmptyList(ExpressionParseError("There is no property 'value3' in type: pl.touk.nussknacker.engine.compile.ProcessValidatorSpec$AnotherSimpleRecord", "sampleFilter2", Some("Expression"), _), _)) =>
    }
  }
  ...
  test("validate custom node return type") {
    ...
    validate(process, definitionWithCustomNode).result should matchPattern {
      ...
      "sampleFilter2", Some("Expression"), "#out1.terefere"), _)) =>
    }
  }
  ...
  test("find usage of fields that does not exist in option object") {
    val process = EspProcessBuilder
      ...
      .emptySink("id2", "sink")
    validate(process, definitionWithTypedSource).result should matchPattern {
      case Invalid(NonEmptyList(ExpressionParseError("There is no property 'terefere' in type: scala.math.BigDecimal", "sampleFilter1", Some("Expression"), _), _)) =>
    }
  }
  ...
  test("validate variable builder fields usage") {
    ...
    validate(process, definitionWithTypedSource).result should matchPattern {
      case Invalid(NonEmptyList(ExpressionParseError("Wrong part types", "notWorking", Some("Expression"), "#var1.a > 10"), _)) =>
    }
  }
  ...
  test("not allow to use vars from different branches") {
    ...
    validate(process, definitionWithTypedSource).result should matchPattern {
      case Invalid(NonEmptyList(ExpressionParseError("Unresolved reference 'var3'", "id3", Some("Expression"), "#var3"), _)) =>
    }
  }
  ...
  test("detect clearing context in custom transformer") {
    ...
    validate(processWithInvalidExpresssion, baseDefinition).result should matchPattern {
      case Invalid(NonEmptyList(ExpressionParseError("Unresolved reference 'input'", "id2", Some("Expression"), "#input.toString()"), _)) =>
    }
  }
