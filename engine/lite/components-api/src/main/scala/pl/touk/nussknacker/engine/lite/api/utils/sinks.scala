...
import pl.touk.nussknacker.engine.api.component.{ComponentInfo, RealComponentType}
      ...
      (
        ...
        ctx =>
          implicitly[Monad[F]].pure(
            ...
            withErrors(context, Some(ComponentInfo("unknown", RealComponentType.Sink)), ctx) {
              interpreter(ctx)
            }
          )
      )
