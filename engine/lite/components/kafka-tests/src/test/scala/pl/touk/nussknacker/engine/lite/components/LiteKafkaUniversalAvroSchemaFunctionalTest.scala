...
class LiteKafkaUniversalAvroSchemaFunctionalTest extends AnyFunSuite with Matchers with ScalaCheckDrivenPropertyChecks with Inside
  ...
  test("should test end to end kafka avro record data at sink / source with union field validations") {
    testEnd2End(Table(
      ...
      (rConfig(sampleUnionRecordLongAndString, recordUnionRecordLongAndStringSchema, recordUnionRecordIntAndStringSchema, Input), invalidTypes("path 'field' actual: 'Record{field: Long} | String' expected: 'Record{field: Integer} | String'")),
      ...
      (sConfig(sampleString, stringSchema, recordUnionStringAndIntegerSchema, Input), invalidTypes("actual: 'String' expected: 'Record{field: String | Integer}'")),
  ...
  test("should test end to end kafka avro record data at sink / source with input as output with arrays") {
    testEnd2End(Table(
      ...
      (rConfig(sampleInteger, recordIntegerSchema, recordOptionalArrayOfRecordsSchema, sampleInteger), invalidTypes(s"""path 'field' actual: '${typedInt.display}' expected: 'Null | List[Null | Record{price: Null | Double}]'""")),
      (rConfig(sampleInteger, recordIntegerSchema, recordOptionalArrayOfRecordsSchema, List(sampleInteger)), invalidTypes(s"""path 'field[]' actual: '${typedInt.display}' expected: 'Null | Record{price: Null | Double}'""")),
    ))
  }
  ...
  test("should test end to end kafka avro record data at sink / source with input as output with maps") {
    testEnd2End(Table(
      ...
      (rConfig(Map("price" -> sampleDouble.toString), fakeMapRecordWithStringPrice, recordMapOfStringsSchema, Input), invalidTypes("path 'field' actual: 'Record{price: String}' expected: 'Map[String,Null | String]'")),
      ...
      (rConfig(sampleInteger, recordIntegerSchema, recordOptionalMapOfRecordsSchema, sampleInteger), invalidTypes(s"path 'field' actual: '${typedInt.display}' expected: 'Null | Map[String,Null | Record{price: Null | Double}]'")),
      (rConfig(sampleInteger, recordIntegerSchema, recordOptionalMapOfRecordsSchema, Map("first" -> sampleInteger)), invalidTypes(s"path 'field.first' actual: '${typedInt.display}' expected: 'Null | Record{price: Null | Double}'")),
      (rConfig(sampleInteger, recordIntegerSchema, recordOptionalMapOfRecordsSchema, Nil), invalidTypes("path 'field' actual: 'List[Unknown]' expected: 'Null | Map[String,Null | Record{price: Null | Double}]'")),
  ...
  test("should test end to end kafka avro record data at sink / source with input as output with records") {
    testEnd2End(Table(
      ...
      (rConfig(sampleInteger, recordIntegerSchema, nestedRecordSchema, Input), invalidTypes("path 'field' actual: 'Integer' expected: 'Null | Record{sub: Null | Record{price: Null | Double}}'")),
      (rConfig(sampleNestedRecord, nestedRecordSchema, nestedRecordSchema, sampleInteger), invalidTypes(s"path 'field' actual: '${typedInt.display}' expected: 'Null | Record{sub: Null | Record{price: Null | Double}}'")),
      (rConfig(sampleInteger, recordIntegerSchema, nestedRecordSchema, Map("sub" -> sampleInteger)), invalidTypes(s"path 'field.sub' actual: '${typedInt.display}' expected: 'Null | Record{price: Null | Double}'")),
      (rConfig(sampleInteger, recordIntegerSchema, nestedRecordSchema, Nil), invalidTypes("path 'field' actual: 'List[Unknown]' expected: 'Null | Record{sub: Null | Record{price: Null | Double}}'")),
      ...
      (rConfig(sampleString, recordStringSchema, recordWithBigUnionSchema, Input), invalidTypes("path 'field' actual: 'String' expected: 'Null | Boolean | Record{sub: Null | Record{price: Null | String}} | Record{sub: Null | Record{price: Null | Double}}'")),
    ))
  }
