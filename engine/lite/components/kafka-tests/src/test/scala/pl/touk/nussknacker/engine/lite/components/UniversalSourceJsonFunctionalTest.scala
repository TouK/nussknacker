...
class UniversalSourceJsonFunctionalTest extends AnyFunSuite with Matchers with ScalaCheckDrivenPropertyChecks with Inside
  ...
  test("should test end to end kafka json data at sink and source / handling nulls and empty json" ) {
    val testData = Table(
      ...
      (oConfig(sampleInteger, schemaObjInteger, schemaObjNull, null), oValid(Null)),
      ...
      (oConfig(InputEmptyObject, schemaObjString, schemaObjUnionNullString, OutputField), oValid(Null)),
      ...
      (oConfig(sampleInteger, schemaObjInteger, schemaObjUnionNullString, null), oValid(Null)),
    )
  ...
  test("should test end to end kafka json data at sink and source / handling objects..") {
    val testData = Table(
      ("config", "result"),
      (oConfig(sampleObjFirstLastName, schemaObjFirstLastName, schemaObjFirstLastName, Input), oValid(sampleObjFirstLastName)),
      (oConfig(sampleInteger, schemaObjInteger, schemaObjFirstLastName, sampleSpELFirstLastName), oValid(sampleObjFirstLastName)),

      //Object with additionalProperties=true => it should be valid after: https://github.com/TouK/nussknacker/pull/3709
//      (oConfig(sampleObjWithAdditional, schemaObjAdditionalProperties, schemaObjAdditionalProperties, Input), oValid(sampleObjWithAdditional)),
//      (oConfig(sampleInteger, schemaObjInteger, schemaObjAdditionalProperties, sampleSpELWithAdditional), oValid(sampleObjWithAdditional)),
//      (oConfig(sampleObjFirstLastName, schemaObjFirstLastName, schemaObjAdditionalProperties, Input), oValid(sampleObjFirstLastName)),
//      (oConfig(sampleInteger, schemaObjInteger, schemaObjAdditionalProperties, sampleSpELFirstLastName), oValid(sampleObjFirstLastName))
    )
  ...
  test("should catch runtime errors") {
    val testData = Table(
      ("config", "result"),
      (oConfig(Json.Null, schemaObjFirstLastName, schemaObjFirstLastName), "#/field: expected type: JSONObject, found: Null"),
      (oConfig(obj("first" -> fromString("")), schemaObjFirstLastName, schemaObjFirstLastName), "#/field: required key [last] not found"),
      (oConfig(fromString("invalid"), schemaObjFirstLastName, schemaObjFirstLastName), "#/field: expected type: JSONObject, found: String"),
      (oConfig(InputEmptyObject, schemaObjString, schemaObjString, OutputField), "#/field: expected type: String, found: Null"),
      (oConfig(InputEmptyObject, schemaObjNull, schemaObjRequiredUnionNullString), "required key [field] not found."),
    )
  ...
  private def runWithValueResults(config: ScenarioConfig): Validated[NonEmptyList[ProcessCompilationError], RunListResult[Json]] =
