...
class LiteKafkaComponentProvider(schemaRegistryClientFactory: ConfluentSchemaRegistryClientFactory) extends ComponentProvider {
  ...
  protected def createAvroSchemaRegistryProvider: SchemaRegistryProvider[AvroSchema] =
    ConfluentSchemaRegistryProvider.avroSchemaAvroPayload(schemaRegistryClientFactory)

  protected def createJsonSchemaRegistryProvider: SchemaRegistryProvider[AvroSchema] =
    ConfluentSchemaRegistryProvider.avroSchemaJsonPayload(schemaRegistryClientFactory)
  ...
  override def create(config: Config, dependencies: ProcessObjectDependencies): List[ComponentDefinition] = {
    ...
    val universalKafkaComponents = List(
      ComponentDefinition(KafkaUniversalName, new UniversalKafkaSourceFactory(CachedConfluentSchemaRegistryClientFactory, dependencies, new LiteKafkaSourceImplFactory)).withRelativeDocs(avro),
