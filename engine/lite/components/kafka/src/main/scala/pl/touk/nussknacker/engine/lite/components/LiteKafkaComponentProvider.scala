...
class LiteKafkaComponentProvider(schemaRegistryClientFactory: SchemaRegistryClientFactory) extends ComponentProvider {
  ...
  override def create(config: Config, dependencies: ProcessObjectDependencies): List[ComponentDefinition] = {
    ...
    lazy val lowLevelKafkaComponents = List(
      ComponentDefinition(
        KafkaJsonName,
        new KafkaSinkFactory(GenericJsonSerialization(_), dependencies, LiteKafkaSinkImplFactory)
      ).withRelativeDocs(noTypeInfo),
      ComponentDefinition(
        KafkaJsonName,
        new KafkaSourceFactory[String, util.Map[_, _]](
          ConsumerRecordDeserializationSchemaFactory.fixedValueDeserialization(deserializeToMap),
          jsonFormatterFactory,
          dependencies,
      ...
      ComponentDefinition(
        KafkaTypedJsonName,
        new KafkaSourceFactory[String, TypedMap](
      ...
      ComponentDefinition(
        ...
        new KafkaAvroSinkFactoryWithEditor(
          ...
          avroPayloadSerdeProvider,
          dependencies,
          LiteKafkaAvroSinkImplFactory
        )
      ).withRelativeDocs(avro),
      ComponentDefinition(
        KafkaRegistryTypedJsonName,
        new KafkaAvroSourceFactory[Any, Any](
          schemaRegistryClientFactory,
      ...
      ComponentDefinition(
        KafkaSinkRegistryTypedRawJsonName,
    ...
    val lowLevelComponentsEnabled = dependencies.config
      .getAs[Boolean]("kafka.lowLevelComponentsEnabled")
      .getOrElse(KafkaConfig.lowLevelComponentsEnabled)
