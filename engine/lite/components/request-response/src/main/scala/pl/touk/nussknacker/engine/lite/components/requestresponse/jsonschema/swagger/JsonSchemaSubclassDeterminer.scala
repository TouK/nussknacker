...
class JsonSchemaSubclassDeterminer(parentSchema: Schema) extends LazyLogging {
  ...
  def validateTypingResultToSchema(typingResult: TypingResult): ValidatedNel[String, Unit] = validateTypingResultToSchema(typingResult, parentSchema, None)
  ...
  private def validateObjectSchema(e: TypedObjectTypingResult, objectSchema: ObjectSchema): ValidatedNel[String, Unit] = {
    implicit val location = objectSchema.getLocation.toString
    ...
    val additionalPropertiesValidation = {
        ...
        condNel(objectSchema.permitsAdditionalProperties || additionalProperties.isEmpty, (), msgWithLocation(s"The object has redundant fields: $additionalProperties"))
    }
  ...
  private def validateTypingResultToSchema(typingResult: TypingResult, schema: Schema, fieldName: Option[String]): ValidatedNel[String, Unit] = {
    implicit val location = schema.getLocation.toString
