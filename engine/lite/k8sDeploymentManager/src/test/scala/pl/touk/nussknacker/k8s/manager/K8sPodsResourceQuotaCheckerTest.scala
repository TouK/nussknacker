...
class K8sPodsResourceQuotaCheckerTest extends AnyFunSuite {
  ...
  private val recreate = Strategy.Recreate
  ...
  test("should not exceed limit when no quotas defined") {
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(None, ListResource("", "", None, List[Resource.Quota]()), 1, recreate)
  ...
  test("should not exceed limit when quotas defined but number of replicas is lower") {
    ...
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(None, quotaList(quota), 1, recreate)
    quotaExceeded shouldEqual Valid(())
  }
  ...
  test("should exceed quota limit when number of replicas is higher") {
    ...
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(None, quotaList(quota), 5, recreate)
  ...
  test("should not exceed quota limit when redeploying same number of replicas") {
    ...
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(oldDeploymentReplicasCount, quotaList(quota), 2, recreate)
    quotaExceeded shouldEqual Valid(())
  }
  ...
  test("should exceed quota limit when redeploying with bigger number of replicas") {
    val quota = Resource.Quota(status = Some(Quota.Status(hard = Map(podsResourceQuota -> 3), used = Map(podsResourceQuota -> 2))))
    val oldDeploymentReplicasCount = Some(2)
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(oldDeploymentReplicasCount, quotaList(quota), 4, recreate)
    quotaExceeded shouldBe Invalid(ResourceQuotaExceededException.notEnoughResources)
  }
  ...
  test("should exceed quota limit when redeploying with bigger number of replicas and bigger surge as int") {
    ...
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(oldDeploymentReplicasCount, quotaList(quota), 1, Strategy(RollingUpdate(maxSurge = Left(1))))
  ...
  test("should exceed quota limit when redeploying with bigger number of replicas and bigger surge as percent") {
    val quota = Resource.Quota(status = Some(Quota.Status(hard = Map(podsResourceQuota -> 1), used = Map(podsResourceQuota -> 1))))
    val oldDeploymentReplicasCount = Some(1)
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(oldDeploymentReplicasCount, quotaList(quota), 1, Strategy(RollingUpdate(maxSurge = Right("25 %"))))
    quotaExceeded shouldBe Invalid(ResourceQuotaExceededException.fullCluster)
  }
  ...
  test("should exceed quota limit when deploying with parallelism = 1 and cluster is full") {
    ...
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(oldDeploymentReplicasCount, quotaList(quota), 1, recreate)
    quotaExceeded shouldBe Invalid(ResourceQuotaExceededException.fullCluster)
  }
  ...
  test("should not exceed limit when found many quota in namespace") {
    ...
    val quotaExceeded = K8sPodsResourceQuotaChecker.hasReachedQuotaLimit(None, quotaList(quota, anotherQuota), 1, recreate)
    quotaExceeded shouldEqual Valid(())
  }
}
