        ...
        .withValue("k8sDeploymentConfig.spec.template.spec.containers",
          fromIterable(List(
            fromMap(Map(
              ...
              "resources" -> fromMap(
                Map(
                  "requests" -> fromMap(Map("memory" -> "256Mi", "cpu" -> "1").asJava),
                  "limits" -> fromMap(Map("memory" -> "256Mi", "cpu" -> "1").asJava)
                ).asJava
              )
            ).asJava)
          ).asJava)
        ...
        forAll(pods.head.spec.get.containers) { container =>
          container.resources shouldBe Some(
            skuber.Resource.Requirements(
              limits = Map("cpu" -> Quantity("1"), "memory" -> Quantity("256Mi")),
              requests = Map("cpu" -> Quantity("1"), "memory" -> Quantity("256Mi"))
            ))
