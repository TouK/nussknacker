...
class KafkaK8sSupport(k8s: KubernetesClient) extends ExtremelyPatientScalaFutures with LazyLogging with Matchers {
  ...
  def start()(implicit ec: ExecutionContext): Unit = if (k8s.getOption[Pod](kafkaPodName).futureValue.isEmpty) {
    ...
    val srContainer = Container(
      ...
      readinessProbe = Some(Probe(new HTTPGetAction(Left(8081), path = "/subjects"), periodSeconds = Some(1), failureThreshold = Some(60))),
    ...
    Future.sequence(List(k8s.create(kafkaPod), k8s.create(kafkaService), k8s.create(srPod), k8s.create(srService))).futureValue

    def isContainerReady(podName: String) =  k8s.get[Pod](podName).futureValue.status.get.containerStatuses.headOption.get.ready
    eventually {
  ...
  private def runInKafka(command: String, end: String => Boolean, input: Option[String] = None): String = runInPod(kafkaPodName, command, end, input)

  private def runInPod(podName: String, command: String, end: String => Boolean, input: Option[String] = None): String = {
