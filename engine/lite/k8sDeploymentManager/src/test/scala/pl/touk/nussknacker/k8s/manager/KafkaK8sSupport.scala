...
class KafkaK8sSupport(k8s: KubernetesClient)(private implicit val system: ActorSystem)
  ...
  private val kafkaClient =
    new KafkaClient(s"${k8sUtils.localPortForwardHost}:$kafkaPodExposedPort", getClass.getSimpleName)
    ...
    val kafkaContainer = Container(
      ...
      env = List(
        ...
        EnvVar(
          ...
          s"BROKER://$kafkaServiceName:9092,CONTROLLER://localhost:9093,LOCALHOST://${k8sUtils.localPortForwardHost}:$kafkaPodExposedPort"
        ),
