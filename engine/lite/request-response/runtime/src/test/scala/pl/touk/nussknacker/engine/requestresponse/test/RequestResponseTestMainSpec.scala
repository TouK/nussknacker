...
import pl.touk.nussknacker.engine.api.{ContextId, ContextIdPathPart, DisplayJsonWithEncoder, JobData, ProcessVersion}
...
class RequestResponseTestMainSpec extends AnyFunSuite with Matchers with BeforeAndAfterEach {
  ...
  test("scenario with multiple unions should work properly") {
    ...
    def prepareContextId(varValue: String, right: Boolean): ContextId = {
      ...
      val splitPart = if (right) Nil else List(ContextIdPathPart("Split", varValue))
      val contextIdPath = splitPart ++ List(
        ContextIdPathPart(unionName, varValue),
        ContextIdPathPart("unionBeforeEnd", unionName)
      )
      ContextId("unionTest", "start", 0, 0, contextIdPath)
    }
    ...
    leftUnionResults.nodeResults.get("unionRight") should not be defined
    leftUnionResults.nodeResults("$edge-leftVariableA-unionLeft") should have size 1
    leftUnionResults.nodeResults("$edge-leftVariableB-unionLeft") should have size 1
    leftUnionResults.externalInvocationResults("Response").map(withMockedTimestamp).toSet shouldBe Set(
    ...
    rightUnionResults.nodeResults.get("unionLeft") should not be defined
    rightUnionResults.nodeResults("$edge-rightVariableB-unionRight") should have size 1
    rightUnionResults.externalInvocationResults("Response").map(withMockedTimestamp).toSet shouldBe Set(
      ExternalInvocationResult(
