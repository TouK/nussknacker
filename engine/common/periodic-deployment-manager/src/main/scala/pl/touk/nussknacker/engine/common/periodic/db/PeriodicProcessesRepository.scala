package pl.touk.nussknacker.engine.common.periodic.db
...
import pl.touk.nussknacker.engine.common.periodic.model._
import pl.touk.nussknacker.engine.api.ProcessVersion
import pl.touk.nussknacker.engine.api.deployment.{ProcessActionId, ScenarioManager}
...
class SlickPeriodicProcessesRepository(
  ...
  private def findProcesses(
    ...
    query
      ...
      .on { case ((periodicProcess, _), (_, processName, _, _)) =>
        periodicProcess.processName === processName
      }
      .map { case ((periodicProcess, periodicDeployment), (_, _, _, json)) =>
        (periodicProcess, periodicDeployment, json)
      }
      .result
  ...
  private def fetchCanonicalProcess = {
    scenarioManager.fetchCanonicalProcessesQuery
  }
