...
class CustomNodeOutputsMigrationSpec extends AnyFreeSpecLike with Matchers {
  ...
  private def source(id: String)      = FlatNode(Source(NodeId(id), NodeName(id), SourceRef("kafka", List.empty)))
  private def sink(id: String)        = FlatNode(Sink(NodeId(id), NodeName(id), SinkRef("kafka", List.empty)))
  private def filterData(id: String)  = Filter(NodeId(id), NodeName(id), Expression.spel("true"))
  private def dedupData(id: String)   = CustomNode(NodeId(id), NodeName(id), None, "deduplication", List.empty)
  private def otherCustom(id: String) = CustomNode(NodeId(id), NodeName(id), None, "otherTransformer", List.empty)
  ...
  "CustomNodeOutputsMigration" - {
    ...
    "wraps a node nested inside switch and split branches" in {
      val switchData = Switch(NodeId("switch1"), NodeName("switch1"), None, None)
      val splitData  = Split(NodeId("split1"), NodeName("split1"))
    ...
    "wraps a node behind a fragment output" in {
      val fragmentData = FragmentInput(NodeId("frag1"), NodeName("frag1"), FragmentRef("myFragment", List.empty))
      val old = scenario(
        List(
          source("source1"),
    ...
    "scenario with an unnamed output loaded from JSON" - {
      ...
      "rewires the node to the named main output" in {
        ...
        custom.outputs.head.nodes.map(_.id.value) shouldBe List("sink1")
      }
