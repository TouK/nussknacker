...
class ExpressionSuggesterBenchmarkSetup() {
  ...
  def test(expression: String, position: Int): AnyRef = {
    Await.result(expressionSuggester.expressionSuggestions(Expression("spel", expression), CaretPosition2d(0, position), variables)(ExecutionContext.global), ScalaDuration("10s"))
  }
