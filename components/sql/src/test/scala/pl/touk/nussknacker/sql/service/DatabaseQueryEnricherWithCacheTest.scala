...
class DatabaseQueryEnricherWithCacheTest extends BaseHsqlQueryEnricherTest {
  ...
  test("DatabaseQueryEnricher#implementation with cache") {
    ...
    val resultF2 = invoker.invokeService(Context.withInitialId.withVariables(Map("arg1" -> 1)))
    ...
    val resultF3 = invoker.invokeService(Context.withInitialId.withVariables(Map("arg1" -> 1)))
