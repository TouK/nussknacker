...
class DatabaseLookupEnricherWithCacheTest extends BaseHsqlQueryEnricherTest {
  ...
  override val service =
    new DatabaseLookupEnricher(hsqlDbPoolConfig, DbEnricherName("lookupEnricher"))
  ...
  test("DatabaseLookupEnricher#implementation with cache") {
    ...
    val resultF = implementation.invoke(Context.dummy)
    ...
    val resultF2 = implementation.invoke(Context.dummy)
