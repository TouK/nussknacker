...
class DatabaseQueryEnricherTest extends BaseHsqlQueryEnricherTest {
  ...
  test("DatabaseQueryEnricher#implementation without cache") {
    ...
    val implementation = service.createRuntimeLogic(Params(Map("arg1" -> 1)), dependencies = Nil, Some(state))
    ...
    val resultF2 = implementation.apply(Context.withInitialId.withVariables(Map("arg1" -> 1)))
