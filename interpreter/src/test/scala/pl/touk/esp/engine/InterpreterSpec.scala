...
import pl.touk.esp.engine.InterpreterSpec._
...
import pl.touk.esp.engine.api.{Service, _}
...
import pl.touk.esp.engine.compile.{PartSubGraphCompiler, SubprocessResolver, ValidationContext}
import pl.touk.esp.engine.definition.DefinitionExtractor.{ClazzRef, ObjectDefinition, ObjectWithMethodDef}
import pl.touk.esp.engine.definition.{DefinitionExtractor, ServiceDefinitionExtractor}
...
import pl.touk.esp.engine.splittedgraph.part.SinkPart
...
class InterpreterSpec extends FlatSpec with Matchers {
  ...
  def interpretTransaction(node: SourceNode, transaction: Transaction, listeners: Seq[ProcessListener] = listenersDef(), services: Map[String, Service] = servicesDef) = {
    ...
    val interpreter = Interpreter(servicesDefs, 5 seconds, listeners)
