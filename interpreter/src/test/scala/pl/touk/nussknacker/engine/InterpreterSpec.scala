...
class InterpreterSpec extends AnyFunSuite with Matchers {
  ...
  test("interprets fragment output fields") {
    val process = ScenarioBuilder.streaming("test")
      .source("source", "transaction-source")
      .subprocessOneOut("sub", "subProcess1", "output",  "fragmentResult", "toMultiply" -> "2", "multiplyBy" -> "4")
      .buildSimpleVariable("result-sink", resultVariable, "#fragmentResult.result.toString").emptySink("end-sink", "dummySink")
      .toCanonicalProcess
