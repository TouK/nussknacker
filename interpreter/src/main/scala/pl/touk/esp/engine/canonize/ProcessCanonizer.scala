...
object ProcessCanonizer {
  ...
  private def nodesOnTheSameLevel(n: node.Node): List[canonicalnode.CanonicalNode] =
    n match {
      ...
      case node.Aggregate(id, keyExpr, duration, step, next)
        => canonicalnode.Aggregate(id, keyExpr, duration, step) :: nodesOnTheSameLevel(next)
    }
  ...
  private def uncanonize(canonicalNode: List[canonicalnode.CanonicalNode]): ValidatedNel[ProcessUncanonizationError, node.Node] =
    canonicalNode match {
      ...
      case canonicalnode.Aggregate(id, keyExpr, duration, slide)::tail =>
        uncanonize(tail).map(node.Aggregate(id, keyExpr, duration, slide, _))
      case invalidTail =>
