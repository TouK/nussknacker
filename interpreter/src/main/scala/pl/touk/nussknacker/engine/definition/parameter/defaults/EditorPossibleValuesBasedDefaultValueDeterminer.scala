  ...
  override def determineParameterDefaultValue(parameters: DefaultValueDeterminerParameters): Option[Expression] = {
    parameters.determinedEditor
      .flatMap {
        ...
        case FixedValuesPresetParameterEditor(_, firstValue :: _) => Some(firstValue.expression)
        ...
        case DualParameterEditor(FixedValuesPresetParameterEditor(_, firstValue :: _), _) => Some(firstValue.expression)
        case _                                                                            => None
      }
      .map(Expression.spel)
  }
