...
object Validations {
  ...
  def validateParameters(parameterDefinitions: List[Parameter], parameters: List[evaluatedparam.Parameter])(
    ...
    (validatedRedundant, validatedMissing, validatedCustom).mapN { (_, _, _) => () }
  }
  ...
  private def validateWithCustomValidators(
