...
import pl.touk.nussknacker.engine.api.definition.{ParameterEditor, RawParameterEditor, SpelTemplateParameterEditor, SqlParameterEditor, StringParameterEditor}
...
protected object TypeRelatedParameterValueDeterminer extends ParameterDefaultValueDeterminer {
  ...
  private[defaults] def determineTypeRelatedDefaultParamValue(editor: Option[ParameterEditor], className: Class[_]): Option[Expression] = {
    // TODO: use classes instead of class names
    Option(className).map(_.getName).collect {
      case "long" | "short" | "int" | "java.lang.Number" | "java.lang.Long" | "java.lang.Short" | "java.lang.Integer" | "java.math.BigInteger" => Expression.spel("0")
      case "float" | "double" | "java.math.BigDecimal" | "java.lang.Float" | "java.lang.Double" => Expression.spel("0.0")
  ...
  private def defaultStringExpression(editor: Option[ParameterEditor]): Expression =
    editor.collect { // TODO: maybe some better way to specify language like Parameter.language
      ...
      case SqlParameterEditor => Expression.sqlSpelTemplate("SELECT * FROM 42")
