...
import pl.touk.nussknacker.engine.api.process.{ComponentUsage, SourceTestSupport, TestDataGenerator}
...
class ModelDataTestInfoProvider(modelData: ModelData) extends TestInfoProvider with LazyLogging {
  ...
  private lazy val nodeCompiler = new NodeCompiler(modelData.processWithObjectsDefinition, expressionCompiler, modelData.modelClassLoader.classLoader, ProductionServiceInvocationCollector, ComponentUsage.TestDataGeneration)
  ...
  private def prepareSourceObj(source: Source)(implicit metaData: MetaData): Option[process.Source] = {
    ...
    implicit val runNode: ComponentUsage = ComponentUsage.TestDataGeneration
