...
object ProcessDefinitionExtractor {
  ...
  case class ProcessDefinition[T](services: Map[String,T],
    ...
    private def servicesWithIds(componentIdProvider: ComponentIdProvider, processingType: String): List[(ComponentIdWithName, T)] =
    ...
    private def customStreamTransformersWithIds(componentIdProvider: ComponentIdProvider, processingType: String): List[(ComponentIdWithName, (T, CustomTransformerAdditionalData))] =
    ...
    private def sinkFactoriesWithIds(componentIdProvider: ComponentIdProvider, processingType: String): List[(ComponentIdWithName, T)] =
      sinkFactories.map { case (name, obj) =>
        val id = componentIdProvider.createComponentId(processingType, Some(name), ComponentType.Sink)
        ComponentIdWithName(id, name) -> obj
      }.toList

    private def sourceFactoriesWithIds(componentIdProvider: ComponentIdProvider, processingType: String): List[(ComponentIdWithName, T)] =
    ...
    def withComponentIds(componentIdProvider: ComponentIdProvider,
                         processingType: String): ProcessDefinitionWithComponentIds[T] =
  ...
  case class ProcessDefinitionWithComponentIds[T](services: List[(ComponentIdWithName, T)],
    ...
    val allDefinitions: List[(ComponentIdWithName, T)] =
      services ++ sourceFactories ++ sinkFactories ++ customStreamTransformers.map { case (idWithName, (value, _)) => (idWithName, value) }
    ...
    def transform[R](f: T => R): ProcessDefinitionWithComponentIds[R] = copy(
      ...
      customStreamTransformers.map { case (idWithName, (value, additionalData)) => (idWithName, (f(value), additionalData)) },
      expressionConfig.copy(globalVariables = expressionConfig.globalVariables.mapValuesNow(f)))
