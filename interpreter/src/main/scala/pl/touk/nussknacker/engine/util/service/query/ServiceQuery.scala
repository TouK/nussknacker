...
import pl.touk.nussknacker.engine.api.process.{ProcessObjectDependencies, ComponentUsage}
...
class ServiceQuery(modelData: ModelData) {
  ...
  def invoke(serviceName: String,
    ...
    val compiler = new NodeCompiler(definitions, ExpressionCompiler.withoutOptimization(modelData),
      modelData.modelClassLoader.classLoader, collector, ComponentUsage.ServiceQuery)
    ...
    withOpenedService(serviceName, definitions) {
      ...
      implicit val componentUsage: ComponentUsage = ComponentUsage.ServiceQuery
