...
object NodeComponentInfoExtractor {
  ...
  def fromNode(node: Node): NodeComponentInfo = {
    ...
    node match {
      ...
      case _: EndingFragmentDefinition => NodeComponentInfo.forBaseNode(node.id, ComponentType.Sink)
      ...
      case endingCustomNode: EndingCustomNode => NodeComponentInfo(node.id, endingCustomNode.ref, ComponentType.CustomNode)
