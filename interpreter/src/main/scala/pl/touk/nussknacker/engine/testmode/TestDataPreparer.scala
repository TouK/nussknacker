...
import pl.touk.nussknacker.engine.api.process.{ComponentUsage, SourceTestSupport}
...
class TestDataPreparer(modelData: ModelData) {
  ...
  private val nodeCompiler = {
    ...
    new NodeCompiler(modelData.processWithObjectsDefinition,
      expressionCompiler, modelData.modelClassLoader.classLoader, PreventInvocationCollector, ComponentUsage.TestDataGeneration)
  }
