...
class DefaultComponentIdProvider(configs: Map[String, ComponentsUiConfig]) extends ComponentIdProvider {
  ...
  override def createComponentId(processingType: String, name: Option[String], componentType: ComponentType): ComponentId = {
    name match {
  ...
  private def getOverriddenComponentId(processingType: String, componentName: String, defaultComponentId: ComponentId): ComponentId = {
