...
import pl.touk.nussknacker.engine.api.typed.typing.{SingleTypingResult, Typed, TypedDict, TypedFromInstance}
...
case class DictInstance(dictId: String, definition: DictDefinition) extends TypedFromInstance {

  def value(key: String): Any = definition.value(key)

  override def typingResult: typing.TypingResult = TypedDict(dictId, definition.valueType(dictId))
}
