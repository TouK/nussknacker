...
class ClassDefinitionDiscoverySpec
  ...
  test("should skip toString method when HideToString implemented") {
    ...
    forAll(hiddenToStringClasses) {
      new ClassDefinitionExtractor(ClassExtractionSettings.Default).extract(_).methods.keys shouldNot contain(
        "toString"
      )
    }
  }
  ...
  test("should handle generic params in maps") {
    val javaMapDef = new ClassDefinitionExtractor(ClassExtractionSettings.Default).extract(classOf[util.Map[_, _]])
  ...
  private def singleClassDefinition[T: TypeTag](
    ...
    new ClassDefinitionDiscovery(settings)
