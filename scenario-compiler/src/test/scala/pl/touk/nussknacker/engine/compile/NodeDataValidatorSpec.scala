...
class NodeDataValidatorSpec extends AnyFunSuite with Matchers with Inside with TableDrivenPropertyChecks {
  ...
  test("should not allow null values in filter") {
    forAll(ExpressionsTestData.nullExpressions) { filterExpression =>
      validate(Filter("filter", filterExpression.spel), Map.empty) should matchPattern {
  ...
  forAll(valueValidationCases) { (description, component, valueExpression, expectViolation) =>
    test(s"should value-validate a @Min param with $description") {
      ...
      ) { case ValidationPerformed(errors, _, _) =>
        if (expectViolation)
          errors should matchPattern { case LowerThanRequiredParameter(_, _, ParameterName("value"), "tst1") :: Nil =>
          }
        else
  ...
  test("should not value-validate a lazy @Min param whose expression reads context variables") {
    ...
    ) { case ValidationPerformed(Nil, _, _) =>
    }
  }
  ...
  test("should validate empty or blank variable expression") {
    forAll(ExpressionsTestData.emptyOrBlankExpressions) { e =>
      validate(Variable("var1", "specialVariable_2", e.spel), Map.empty) should matchPattern {
        case ValidationPerformed(
              (
                EmptyMandatoryParameter(
