...
class ProcessSplitterSpec extends AnyFunSuite with Matchers with OptionValues {
  ...
  test("split single-output custom node with continuation embedded in node") {
    ...
    singleOutputPart.nextParts.map(_.id.value) shouldBe List("sink")
  ...
  test("split multi-output custom node with connected main output") {
    ...
    output(customPart, "main").nextParts.map(_.id.value) shouldBe List("mainSink")
    output(customPart, "rejected").nextParts.map(_.id.value) shouldBe List("rejectedSink")

    customPart.nextParts.map(_.id.value) should contain theSameElementsAs List("mainSink", "rejectedSink")
  ...
  test("split multi-output custom node keeps duplicate output names for the compiler to reject") {
    ...
    customPart.nextParts.map(_.id.value) shouldBe List("rejectedSink1", "rejectedSink2")
    customPart.ends shouldBe empty
  }
