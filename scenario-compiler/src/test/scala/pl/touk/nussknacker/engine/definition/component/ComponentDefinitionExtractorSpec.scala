...
object ComponentDefinitionExtractorSpec {
  ...
  /** Stands in for the transformation these components would return; extraction fails before it is ever used. */
  object DummyImplementation

  object DuplicateOutputsTransformer extends CustomStreamTransformer {

    @MethodToInvoke(returnType = classOf[AnyRef])
    def execute(@ParamName("stringVal") stringVal: LazyParameter[String]) = DummyImplementation
  ...
  object JoinTransformerWithOutputs extends CustomStreamTransformer {
    ...
    def execute(
      ...
      ContextTransformation.join
        ...
        .implementedBy(DummyImplementation)
    }
  ...
  object MainNameDuplicatedTransformer extends CustomStreamTransformer {
    ...
    @MethodToInvoke(returnType = classOf[AnyRef])
    def execute() = DummyImplementation
