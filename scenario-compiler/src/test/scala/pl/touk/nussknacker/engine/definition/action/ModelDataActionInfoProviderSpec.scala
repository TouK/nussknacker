...
import pl.touk.nussknacker.engine.api.{Context, EagerService, MethodToInvoke, NodeId, ProcessVersion, ServiceInvoker}
...
class ModelDataActionInfoProviderSpec
  ...
  test("collects action parameters of a service sitting in an additional output's branch") {
    ...
    params(testAction).keys.map(_.nodeId) should contain(NodeId("reject-action"))
