...
class CustomNodeValidationSpec extends AnyFunSuite with Matchers with OptionValues {
  ...
  test("invalid scenario - connects an additional output of a component whose implementation cannot route one") {
    ...
    validate(withOutputConnected).result should matchPattern {
      case Invalid(NonEmptyList(CustomNodeError(NodeId("custom1"), message, None), Nil))
  ...
  test("invalid scenario - additional output with a name not declared by the component") {
    ...
    validate(invalidProcess).result should matchPattern {
      case Invalid(NonEmptyList(UnknownCustomNodeOutput("typo", nodeIds), Nil)) if nodeIds == Set(NodeId("custom1")) =>
    }
  }
  ...
  test("invalid scenario - additional output connected on a custom node of an unknown type") {
    ...
    validate(invalidProcess).result should matchPattern {
      case Invalid(NonEmptyList(MissingCustomNodeExecutor("notExisting", NodeId("custom1")), Nil)) =>
    }
  }
  ...
  test(
    ...
    validate(invalidProcess).result should matchPattern {
      case Invalid(NonEmptyList(MissingParameters(params, nodeId), Nil))
          if params == Set(ParameterName("OutputVariable")) && nodeId == NodeId("custom1") =>
    }
  }
  ...
  test("invalid scenario - error inside an additional output is attributed to the node inside it") {
    ...
    errors.collect { case e: ExpressionParserCompilationError => e.nodeId } should contain(NodeId("branchVar"))
  }
  ...
  test("NotBlankValidator rejects an eager branch param whose evaluated value is blank") {
    ...
    validationResult.result should matchPattern {
      case Invalid(
            NonEmptyList(BlankParameter(_, _, ParameterName("label for branch branch1"), NodeId("join1")), Nil)
          ) =>
    }
  }
  ...
  test("compiled part - the main output wired by name is the head of the outputs") {
    ...
    customPart.outputs.head.nextParts.map(_.id.value) shouldBe List("passedOut")
  }
  ...
  test("compiled part - an unwired main output is present as the head, with a DeadEnd") {
    ...
    customPart.outputs.head.ends.map(_.end) shouldBe List(DeadEnd(NodeId("custom1")))
  }
  ...
  test("compiled part - a multi-output component wired through the plain chain compiles as the main output only") {
    ...
    customPart.outputs.head.nextParts.map(_.id.value) shouldBe List("out")
  }
