...
object ComponentImplementationInvoker {
  ...
  def dumbImplementationInvoker[ComponentExecutor: NotNothing: ClassTag]: ComponentImplementationInvoker = {
    (_: Params, _: NodeCompilationDependencies, _: Option[ComponentImplementationSpecificInvocationContext]) =>
      // We create a fake instance, because NodeCompiler even during scenario validation checks what class was returned
      JavaProxy.newProxyInstance(
        getClass.getClassLoader,
        Array(classTag[ComponentExecutor].runtimeClass),
        (_: Any, _: Method, _: Array[AnyRef]) =>
          throw new IllegalAccessException("This is a dumb implementation of component, it shouldn't be used")
      )
  }
