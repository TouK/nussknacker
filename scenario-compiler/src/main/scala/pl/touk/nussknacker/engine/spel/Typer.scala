...
private[spel] class Typer(
  ...
  private def typeNode(
    ...
    def withChildrenOfType[Parts: universe.TypeTag](result: TypingResult) = {
      val w = valid(result)
      withTypedChildren {
        case list if list.forall(_.canBeImplicitlyConvertedTo(Typed.fromDetailedType[Parts])) => w
    ...
    def typeIndexerOnRecord(indexer: Indexer, record: TypedObjectTypingResult) = {
      withTypedChildren {
        ...
        case indexKey :: Nil if indexKey.canBeImplicitlyConvertedTo(Typed[String]) =>
    ...
    catchUnexpectedErrors(node match {
      ...
      case e: OpMinus =>
        withTypedChildren {
          case left :: right :: Nil
              if left.canBeImplicitlyConvertedTo(Typed[Number]) && right.canBeImplicitlyConvertedTo(Typed[Number]) =>
          ...
          case left :: Nil if left.canBeImplicitlyConvertedTo(Typed[Number]) =>
      ...
      case e: OpPlus =>
        withTypedChildren {
          case left :: right :: Nil if left == Unknown || right == Unknown =>
            valid(Unknown)
          case left :: right :: Nil
              if left.canBeImplicitlyConvertedTo(Typed[String]) || right.canBeImplicitlyConvertedTo(Typed[String]) =>
      ...
      case e: Selection =>
        for {
          ...
          result <- typeChildren(validationContext, node, current.pushOnStack(elementType)) {
            case result :: Nil if result.canBeImplicitlyConvertedTo(Typed[Boolean]) =>
              valid(selectionType)
            case other =>
      ...
      case e: Ternary =>
        withTypedChildren {
          case condition :: onTrue :: onFalse :: Nil =>
            for {
              _ <- Option(condition)
                .filter(_.canBeImplicitlyConvertedTo(Typed[Boolean]))
                .map(valid)
    ...
    if (isSingleElementSelection)
      ...
      parentType match {
        case tc: SingleTypingResult
            if tc.runtimeObjType.canBeImplicitlyConvertedTo(Typed[java.util.Collection[_]]) ||
              tc.runtimeObjType.klass.isArray =>
          tc.withoutValue
        case tc: SingleTypingResult if tc.runtimeObjType.canBeImplicitlyConvertedTo(Typed[java.util.Map[_, _]]) =>
