    ...
    invocationResult match {
      ...
      case _ =>
        throw new ClassCastException(
          s"Object returned by [${deps.componentDefinition.id}] component is ${Option(invocationResult).map(ir => s"of ${ir.getClass.getName} class").orNull} which doesn't extend ${classTag[ComponentExecutor].runtimeClass.getName}"
        )
    }
  }
