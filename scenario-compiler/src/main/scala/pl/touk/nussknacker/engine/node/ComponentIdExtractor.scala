...
object ComponentIdExtractor {
  ...
  def fromCompiledNode(node: compilednode.Node): Option[ComponentId] = {
    node match {
      ...
      case compilednode.VariableBuilder(_, _, _, _, _, unsetVariables) if unsetVariables.nonEmpty =>
        Some(BuiltInComponentId.Variable)
      case compilednode.VariableBuilder(_, _, _, Left(_), _, _)  => Some(BuiltInComponentId.Variable)
      case compilednode.VariableBuilder(_, _, _, Right(_), _, _) => Some(BuiltInComponentId.RecordVariable)
