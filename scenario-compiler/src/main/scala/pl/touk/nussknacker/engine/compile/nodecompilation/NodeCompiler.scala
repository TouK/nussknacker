...
class NodeCompiler(
  ...
  private def unwrapContextTransformation[ComponentExecutor: ClassTag](
    ...
    def handleUnexpectedExecutorClass(obj: Any): Nothing = throw new ClassCastException(
      s"Object returned by [${componentDefinition.id}] component is ${Option(obj).map(ir => s"of ${ir.getClass.getName} class").orNull} which doesn't extend ${classTag[ComponentExecutor].runtimeClass.getName}"
    )
    value match {
