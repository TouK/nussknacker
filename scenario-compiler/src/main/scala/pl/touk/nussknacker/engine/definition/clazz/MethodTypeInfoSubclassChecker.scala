...
object MethodTypeInfoSubclassChecker {
  ...
  def check(subclassInfo: MethodTypeInfo, superclassInfo: MethodTypeInfo): ValidatedNel[ParameterListError, Unit] = {
    ...
    val validatedVarArgs = (subclassVarArgOption, superclassVarArgOption) match {
      case (Some(sub), Some(sup)) if sub.refClazz.canBeImplicitlyConvertedTo(sup.refClazz) => ().validNel
    ...
    val validatedNoVarArgs = zippedParameters.zipWithIndex
      .map {
        case ((sub, sup), _) if sub.refClazz.canBeImplicitlyConvertedTo(sup.refClazz) => ().validNel
    ...
    val validatedResult =
      Validated.condNel(
        subclassResult.canBeImplicitlyConvertedTo(superclassResult),
        (),
