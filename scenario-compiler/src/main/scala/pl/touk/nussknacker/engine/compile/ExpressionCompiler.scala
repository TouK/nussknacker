...
import pl.touk.nussknacker.engine.api.{EagerParameterEvaluationResult, JobData, NodeId, NodeName}
      ...
      compiledParams.flatMap { case (typedParam, paramDef) =>
        ...
        evaluatedParamsResults.get(typedParam.name) match {
          ...
          case None if paramDef.validators.nonEmpty && literalValueNotSet && safelyEvaluable =>
            ...
            parameterEvaluator.evaluateEagerParameter(typedParam, paramDef).toOption.map(typedParam.name -> _)
          case None =>
            None
        }
