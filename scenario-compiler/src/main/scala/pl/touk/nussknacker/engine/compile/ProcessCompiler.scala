...
protected trait ProcessCompilerBase {
  ...
  private def findDuplicates(parts: NonEmptyList[SourcePart]): ValidatedNel[ProcessCompilationError, Unit] = {
    ...
    // Duplicated branch ends mean a join is reached twice under one branch key (the source node's id) - typically
    ...
    val duplicatedIdErrors = Option.when(duplicatedIds.nonEmpty)(DuplicatedNodeIds(duplicatedIds.toSet)).toList

    NonEmptyList.fromList(sameJoinErrors ++ duplicatedIdErrors).map(Invalid(_)).getOrElse(valid(()))
  }

  /**
    * The canonical branchId (`BranchEndDefinition.id`) is the source node's id (see the Join case in
    * CanonicalProcessConverter). It resolves to nothing for a resolved fragment, whose branchId FragmentResolver
    * leaves unprefixed, so the fallback is `joinId` - the only id the graph is still guaranteed to contain.
    */
  private def branchSourceNodeIds(
      definition: BranchEndDefinition,
      allNodes: List[splittednode.SplittedNode[_ <: NodeData]]
  ): Set[String] = {
    val branchSourceExists = allNodes.map(_.data).exists {
      case _: BranchEndData => false
      case data             => data.id == definition.id
    }

    if (branchSourceExists) {
      Set(definition.id)
    } else {
      Set(definition.joinId)
    }
  }
      ...
      outputData.map { case (output, outputNode, outputValidation, outputTypesForParts) =>
        CompilationResult.map4(
          ...
          compiledgraph.part.CompiledOutput(
            ...
            output.ends.map(e => TypedEnd(e, outputTypesForParts.getOrElse(e.nodeId, ValidationContext.empty)))
          )
        }
      }.sequence
  ...
  /**
    ...
    */
  private def declaredOutputs(nodeType: String, nodeId: String): NonEmptyList[ComponentOutput] =
    nodeCompiler.declaredOutputs(nodeType).map(_.outputs).getOrElse(missingComponent(nodeType, nodeId))

  private def missingComponent(nodeType: String, nodeId: String): Nothing =
