...
case class LiteTestScenarioRunnerBuilder(components: List[ComponentDefinition], globalProcessVariables: Map[String, WithCategories[AnyRef]], config: Config, testRuntimeMode: Boolean)
  ...
  override def withGlobalProcessVariables(globalProcessVariables: Map[String, WithCategories[AnyRef]]): LiteTestScenarioRunnerBuilder =
  ...
  override def build(): LiteTestScenarioRunner = new LiteTestScenarioRunner(components, globalProcessVariables, config, componentUseCase(testRuntimeMode))
 ...
 */
class LiteTestScenarioRunner(components: List[ComponentDefinition], globalProcessVariables: Map[String, WithCategories[AnyRef]], config: Config, componentUseCase: ComponentUseCase) extends ClassBasedTestScenarioRunner {
  ...
  def runWithDataReturningDetails[T: ClassTag](scenario: CanonicalProcess, data: List[T]): SynchronousResult = {
    ...
    ModelWithTestComponents.withTestComponents(config, testSource :: testSink :: components, globalProcessVariables) { modelData =>
