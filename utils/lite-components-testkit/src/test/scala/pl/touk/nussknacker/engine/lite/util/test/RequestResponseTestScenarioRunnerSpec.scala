...
import pl.touk.nussknacker.engine.api.{MethodToInvoke, NodeId, NodeName, ParamName, Service}
  ...
  test("runs failure handling") {
    ...
    val runResults = runner.runWithRequests(scenario) { invoker =>
      ...
      firstError.nodeComponentInfo shouldBe Some(
        NodeComponentInfo(NodeId("fail"), NodeName("fail"), Some(ComponentId(ComponentType.Service, failingComponent)))
      )
