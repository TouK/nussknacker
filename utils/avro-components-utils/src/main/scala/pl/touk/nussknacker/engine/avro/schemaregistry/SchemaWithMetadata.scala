...
object SchemaWithMetadata{
  def apply(schemaMetadata: SchemaMetadata): SchemaWithMetadata = {
    schemaMetadata.getSchemaType match {
      case AvroSchema.TYPE => SchemaWithMetadata(new AvroSchema(schemaMetadata.getSchema), schemaMetadata.getId)
      case JsonSchema.TYPE => SchemaWithMetadata(new JsonSchema(schemaMetadata.getSchema), schemaMetadata.getId)
