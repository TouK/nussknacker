...
import pl.touk.nussknacker.engine.avro.schemaregistry.confluent.client.AvroWithJsonPayloadSchema
  ...
  override def determineSchemaUsedInTyping: Validated[SchemaDeterminerError, RuntimeSchemaData[AvroSchema]] = {
    ...
    schemaRegistryClient
      ...
      .andThen(withMetadata => withMetadata.schema match {
        ...
        case s: AvroWithJsonPayloadSchema => Valid(RuntimeSchemaData(s.rawSchema(), Some(withMetadata.id)))
...
class ParsedSchemaDeterminer(schemaRegistryClient: SchemaRegistryClient,
                             topic: String,
                             versionOption: SchemaVersionOption,
                             isKey: Boolean) {
