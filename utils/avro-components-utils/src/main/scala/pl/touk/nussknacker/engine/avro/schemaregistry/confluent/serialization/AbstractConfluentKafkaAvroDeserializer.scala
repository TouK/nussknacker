...
abstract class AbstractConfluentKafkaAvroDeserializer extends AbstractKafkaAvroDeserializer with DatumReaderWriterMixin with RecordDeserializer {
  ...
  protected def read(buffer: ByteBuffer, expectedSchemaData: Option[RuntimeSchemaData]): AnyRef = {
    ...
    try {
      ...
      val writerSchemaData = RuntimeSchemaData(AvroSchema(ConfluentUtils.extractSchema(parsedSchema)), Some(schemaId))
      val readerSchemaData = expectedSchemaData.getOrElse(writerSchemaData)
      // HERE we create our DatumReader
      val reader = createDatumReader(writerSchemaData.avroSchema.schema, readerSchemaData.avroSchema.schema, useSchemaReflection, useSpecificAvroReader)
      val bufferDataStart = 1 + AbstractKafkaSchemaSerDe.idSize
      deserializeRecord(readerSchemaData.avroSchema, reader, buffer, bufferDataStart)
    } catch {
