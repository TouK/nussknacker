...
trait UniversalKafkaBaseTransformer[T] extends SingleInputGenericNodeTransformation[T] with WithCachedTopicsExistenceValidator {
  ...
  def schemaRegistryProvider: KafkaSerDeSchemaRegistryProvider
  ...
  protected def prepareValueSchemaDeterminer(preparedTopic: PreparedKafkaTopic, version: SchemaVersionOption): UniversalSchemaDeterminer = {
    new BasedOnVersionSchemaDeterminer(schemaRegistryClient, preparedTopic.prepared, version, isKey = false)
  }

  //TODO: add schema versioning for key schemas
  protected def prepareKeySchemaDeterminer(preparedTopic: PreparedKafkaTopic): UniversalSchemaDeterminer = {
    new BasedOnVersionSchemaDeterminer(schemaRegistryClient, preparedTopic.prepared, LatestSchemaVersion, isKey = true)
  }
