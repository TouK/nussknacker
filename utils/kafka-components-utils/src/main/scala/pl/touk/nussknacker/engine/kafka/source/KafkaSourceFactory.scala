 ...
 *
 * @tparam K - type of key of kafka event that is produced by raw source (SourceFunction).
 * @tparam V - type of value of kafka event that is produced by raw source (SourceFunction).
 * */
