...
import AvroSinkValueParameter.FieldName
...
object AvroSinkValueParameter {
  ...
  private def getDefaultValue(fieldSchema: Schema.Field, paramName: Option[String])(implicit nodeId: NodeId): ValidatedNel[ProcessCompilationError, Option[Expression]] =
    if (fieldSchema.hasDefaultValue) {
      val defaultValueJsonStr = new ObjectMapper().writeValueAsString(fieldSchema.defaultVal())
      val defaultValueJson = io.circe.parser.parse(defaultValueJsonStr).valueOr(throw _)
      new AvroDefaultExpressionExtractor(fieldSchema.schema(), defaultValueJson, handleNotSupported = true).toExpression
        .leftMap(_.map(err => CustomNodeError(err.getMessage, paramName)))
    } else {
      Valid(None).toValidatedNel
    }
