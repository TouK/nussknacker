...
trait KafkaSourceFactory[K, V]
  ...
  protected def deserializationSchemaFactory: KafkaDeserializationSchemaFactory[ConsumerRecord[K, V]]

  protected def formatterFactory: RecordFormatterFactory

  protected def processObjectDependencies: ProcessObjectDependencies
  ...
  protected lazy val keyTypingResult: TypedClass = Typed.typedClass[K](keyClassTag)

  protected lazy val valueTypingResult: TypedClass = Typed.typedClass[V](valueClassTag)
  ...
  override def implementation(params: Map[String, Any], dependencies: List[NodeDependencyValue], finalState: Option[State]): Source[ConsumerRecord[K, V]] = {
    ...
    val formatter = formatterFactory.create(kafkaConfig, deserializationSchema)(keyClassTag, valueClassTag)
