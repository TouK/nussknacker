...
object JsonSchemaBasedParameter {
  ...
  def apply(schema: Schema, defaultParamName: FieldName, validationMode: ValidationMode)(implicit nodeId: NodeId): ValidatedNel[ProcessCompilationError, SchemaBasedParameter] =
    ParameterRetriever(schema, defaultParamName, validationMode).toSchemaBasedParameter(schema, paramName = None, defaultValue = None, isRequired = None)
  ...
  private case class ParameterRetriever(rootSchema: Schema, defaultParamName: FieldName, validationMode: ValidationMode)(implicit nodeId: NodeId) {
    ...
    def toSchemaBasedParameter(schema: Schema, paramName: Option[String], defaultValue: Option[Expression], isRequired: Option[Boolean]): ValidatedNel[ProcessCompilationError, SchemaBasedParameter] = {
      schema match {
        case objectSchema: ObjectSchema if !objectSchema.hasOnlyAdditionalProperties =>
          objectSchemaToSchemaBasedParameter(objectSchema, paramName, isRequired = None) //ObjectSchema doesn't use property required
        case _ =>
    ...
    private def createJsonSinkSingleValueParameter(schema: Schema,
                                                   ...
                                                   isRequired: Option[Boolean]): SingleSchemaBasedParameter = {
      ...
      SingleSchemaBasedParameter(parameter, new JsonSchemaOutputValidator(validationMode).validate(_, schema, Some(rootSchema)))
    }

    private def objectSchemaToSchemaBasedParameter(schema: ObjectSchema, paramName: Option[String], isRequired: Option[Boolean]): ValidatedNel[ProcessCompilationError, SchemaBasedParameter] = {
      import cats.implicits.{catsStdInstancesForList, toTraverseOps}

      val listOfValidatedParams: List[Validated[NonEmptyList[ProcessCompilationError], (String, SchemaBasedParameter)]] = schema.getPropertySchemas.asScala.map {
        case (fieldName, fieldSchema) =>
          ...
          val schemaBasedValidated = getDefaultValue(fieldSchema, paramName).andThen { defaultValue =>
            toSchemaBasedParameter(schema = fieldSchema, paramName = Option(concatName), defaultValue = defaultValue, isRequired = isRequired)
          }
