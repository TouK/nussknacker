...
object SwaggerTyped {
  ...
  def typingResult(swaggerTyped: SwaggerTyped): TypingResult = swaggerTyped match {
    ...
    case SwaggerEnum(values) =>
      ...
      Typed.fromIterableOrNullType(values.map(Typed.fromInstance))
    case SwaggerBool =>
    ...
    case SwaggerUnion(types) =>
      // TODO: Is it ok? Or we should skip this element?
      Typed.fromIterableOrNullType(types.map(typingResult))
    case SwaggerAny =>
      Unknown
    case SwaggerNull =>
      TypedNull
    ...
    if (elementType.isEmpty) {
      ...
      additionalProperties match {
        ...
        case AdditionalPropertiesDisabled =>
          // TODO: Is it ok? Or we should skip this element?
          typedStringKeyMap(Typed.fromIterableOrNullType(patternPropertiesTypesSet))
