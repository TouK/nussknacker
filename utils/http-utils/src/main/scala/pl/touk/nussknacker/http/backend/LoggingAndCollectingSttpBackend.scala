...
class LoggingAndCollectingSttpBackend[+P](delegate: SttpBackend[Future, P], baseLoggerName: String)(
    ...
    serviceInvocationCollector: ServiceInvocationCollector
  ...
  override def send[T, R >: P with Effect[Future]](request: Request[T, R]): Future[Response[T]] = {
    ...
    lazy val response = delegate
      .send(withLoggedStringResponseBody)
      .transform(
        response => handleResponse(correlationId, request, reqTime, response),
        error => {
          responseLogger.debug(
            s"[$correlationId]: request failed in ${System.currentTimeMillis() - reqTime}ms: ${requestLog.pretty}"
          )
          error
        }
      )
  ...
  private def withStringResponseBody[T, R >: P with Effect[Future]](
      responseAs: ResponseAs[T, R]
  ): ResponseAs[WithStringResponseBody[T], R] = {
    responseAs match {
      case ResponseAsByteArray =>
        ResponseAsByteArray.map(byteArray =>
          WithStringResponseBody(byteArray, () => Some(new String(byteArray, StandardCharsets.UTF_8)))
        )
  ...
  private def handleResponse[T, R >: P with Effect[Future]](
      correlationId: UUID,
      request: Request[T, R],
      requestTimestamp: Long,
      response: Response[WithStringResponseBody[T]]
  ): CollectableAction[Response[T]] = {
    def responseLog = RequestResponseLog(request, response, resBody = response.body.stringResponseBody())

    responseLogger.debug(
      s"[$correlationId][took ${System.currentTimeMillis() - requestTimestamp}ms]: ${responseLog.pretty}"
    )
