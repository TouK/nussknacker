...
class LazyKafkaAdminClientTest extends AnyFunSuiteLike with Matchers {

  test("should create admin client only once") {
    ...
    val lazyClient = new LazyKafkaAdminClient(createClient)
  ...
  test("should create admin client again after previous failure") {
    var createClientInvokedTimes = 0
    val client                   = mock[Admin]
    def createClient = {
      createClientInvokedTimes += 1
  ...
  test("should close admin client") {
    val client       = mock[Admin]
  ...
  test("should not try to close never used admin client") {
    var createClientInvokedTimes = 0
    val client                   = mock[Admin]
    def createClient = {
      createClientInvokedTimes += 1
