...
object AzureTokenCredentialFactory extends LazyLogging {
  ...
  def createCredential(configuration: Configuration): TokenCredential = {
    logger.debug("Property AZURE_CLIENT_ID value from configuration: " + configuration.get(Configuration.PROPERTY_AZURE_CLIENT_ID))
    logger.debug("Property AZURE_CLIENT_ID value from environment: " + System.getenv(Configuration.PROPERTY_AZURE_CLIENT_ID))
    logger.debug("Property AZURE_CLIENT_ID value from java props: " + System.getProperty(Configuration.PROPERTY_AZURE_CLIENT_ID))
    ...
    val chain = clientSecretCredentialOpt.toList :+ new DefaultAzureCredentialBuilder().configuration(configuration).build()
