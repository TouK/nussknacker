...
import pl.touk.nussknacker.engine.schemedkafka.schemaregistry.SchemaWithMetadata
...
class SchemaRegistryCaches(cacheConfig: SchemaRegistryCacheConfig) extends LazyLogging {
  ...
  val schemaByIdCache = new DefaultCache[Int, SchemaWithMetadata](CacheConfig(maximumSize, parsedSchemaAccessExpirationTime, Option.empty))
