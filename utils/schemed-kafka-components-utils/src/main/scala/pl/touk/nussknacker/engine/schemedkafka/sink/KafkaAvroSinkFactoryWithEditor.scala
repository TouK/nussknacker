...
object KafkaAvroSinkFactoryWithEditor {
  ...
  case class TransformationState(
      ...
      schemaBasedParameter: SchemaBasedParameter
  )

}

class KafkaAvroSinkFactoryWithEditor(
  ...
  private val restrictedParamNames: Set[ParameterName] =
    Set(SchemaVersionParamName, SinkKeyParamName, topicParamName, SinkValidationModeParameterName)
  ...
  private def valueParamStep(context: ValidationContext)(implicit nodeId: NodeId): NodeTransformationDefinition = {
    case TransformationStep(
      ...
      validatedSchema
        .andThen { schemaData =>
          AvroSchemaBasedParameter(schemaData.schema.rawSchema(), restrictedParamNames).map { valueParam =>
  ...
  protected def finalParamStep(context: ValidationContext)(implicit nodeId: NodeId): NodeTransformationDefinition = {
    case TransformationStep(
          (`topicParamName`, _) :: (SchemaVersionParamName, _) :: (SinkKeyParamName, _) :: valueParams,
          state
