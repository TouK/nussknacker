...
object AvroSchemaSupport extends ParsedSchemaSupport[AvroSchema] {
  ...
  override def parameterValidator(schema: ParsedSchema, mode: ValidationMode): SinkValueValidator = new AvroSchemaOutputValidator(schema.cast().rawSchema(), mode)
...
object JsonSchemaSupport extends ParsedSchemaSupport[OpenAPIJsonSchema] {
  ...
  override def parameterValidator(schema: ParsedSchema, mode: ValidationMode): SinkValueValidator = {
    new JsonSchemaOutputValidator(schema.cast().rawSchema(), mode)
  }
}
