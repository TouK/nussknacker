...
import pl.touk.nussknacker.engine.schemedkafka.schemaregistry.{SchemaId, SchemaIdFromMessageExtractor, SchemaRegistryClient, SchemaRegistryClientFactory}
...
/**
  ...
  */
class AvroToJsonFormatterFactory(schemaRegistryClientFactory: SchemaRegistryClientFactory,
                                 schemaIdFromMessageExtractor: SchemaIdFromMessageExtractor,
                                 serializerFactory: SchemaRegistryBasedSerializerFactory) extends RecordFormatterFactory {

  override def create[K: ClassTag, V: ClassTag](kafkaConfig: KafkaConfig,
                                                kafkaSourceDeserializationSchema: KafkaDeserializationSchema[ConsumerRecord[K, V]]): RecordFormatter = {
    ...
    def createSerializer(isKey: Boolean) = serializerFactory.createSerializer(schemaRegistryClient, kafkaConfig, None, isKey)
    ...
    new AvroToJsonFormatter(kafkaConfig, schemaRegistryClient, keyMessageReader, valueMessageReader, kafkaSourceDeserializationSchema, schemaIdFromMessageExtractor)
  }
...
/**
  ...
  */
class AvroToJsonFormatter[K: ClassTag, V: ClassTag](protected val kafkaConfig: KafkaConfig,
  ...
  override protected def readValueMessage(schemaOpt: Option[ParsedSchema], topic: String, jsonObj: Json): Array[Byte] = {
    val avroSchema = AvroUtils.extractSchema(schemaOpt.getOrElse(throw new IllegalArgumentException("Error reading value schema: empty schema id")))
