...
class UniversalToJsonFormatter[K: ClassTag, V: ClassTag](protected val kafkaConfig: KafkaConfig,
  ...
  override protected def deserialize(record: ConsumerRecord[Array[Byte], Array[Byte]], valueSchemaIdOpt: Option[SchemaId]): ConsumerRecord[K, V] = {
  ...
  protected def formatMessage(schemaIdOpt: Option[SchemaId], data: Any): Json = {
    ...
    val support = schemaIdOpt
      ...
      .map(recordFormatterSupportDispatcher.forSchemaType).getOrElse(JsonPayloadRecordFormatterSupport)
