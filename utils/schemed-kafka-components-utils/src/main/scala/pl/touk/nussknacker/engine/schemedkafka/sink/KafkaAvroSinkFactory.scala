...
object KafkaAvroSinkFactory {
  ...
  case class KafkaAvroSinkFactoryState(
      schema: RuntimeSchemaData[AvroSchema],
      runtimeSchema: Option[RuntimeSchemaData[AvroSchema]]
  )
...
class KafkaAvroSinkFactory(
  ...
  private val outputValidatorErrorsConverter = new OutputValidatorErrorsConverter(
    KafkaUniversalComponentTransformer.SinkValueParamName
  )

  override def contextTransformation(context: ValidationContext, dependencies: List[NodeDependencyValue])(
      implicit nodeId: NodeId
  ): NodeTransformationDefinition = topicParamStep orElse schemaParamStep orElse {
    case TransformationStep(
          (`topicParamName`, DefinedEagerParameter(topic: String, _)) ::
          (KafkaUniversalComponentTransformer.SchemaVersionParamName, DefinedEagerParameter(version: String, _)) ::
          (
            KafkaUniversalComponentTransformer.SinkValidationModeParameterName,
            DefinedEagerParameter(mode: String, _)
          ) ::
          (KafkaUniversalComponentTransformer.SinkKeyParamName, _: BaseDefinedParameter) ::
          (KafkaUniversalComponentTransformer.SinkValueParamName, value: BaseDefinedParameter) :: Nil,
          _
        ) =>
      ...
      val validationResult = validatedSchema
        .andThen { schema =>
          new AvroSchemaOutputValidator(extractValidationMode(mode))
            .validate(value.returnType, schema.rawSchema())
        ...
        .toList
        .flatMap(_.toList)
      val finalState = determinedSchema.toOption.map(schema =>
        KafkaAvroSinkFactoryState(schema, schemaDeterminer.toRuntimeSchema(schema))
      )
    ...
    case TransformationStep(
          ...
          (KafkaUniversalComponentTransformer.SinkValueParamName, _) :: Nil,
          _
        ) =>
  ...
  override def implementation(
    ...
    val serializationSchema = schemaBasedMessagesSerdeProvider.serializationSchemaFactory.create(
      ...
      finalState.runtimeSchema.map(_.toParsedSchemaData),
      kafkaConfig
    )
    val validationMode = extractValidationMode(
      params(KafkaUniversalComponentTransformer.SinkValidationModeParameterName).asInstanceOf[String]
    )
