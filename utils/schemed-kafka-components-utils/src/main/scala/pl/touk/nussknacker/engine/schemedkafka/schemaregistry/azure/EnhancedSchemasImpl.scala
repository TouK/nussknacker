...
import pl.touk.nussknacker.engine.schemedkafka.schemaregistry.azure.model.{SchemaVersions, Schemas}
import reactor.core.publisher.Mono

class EnhancedSchemasImpl(endpoint: String, apiVersion: String, httpPipeline: HttpPipeline, serializerAdapter: SerializerAdapter) {
  ...
  @ServiceInterface(name = "AzureSchemaRegistryS") trait SchemasService {
    ...
    def getSchemas(@HostParam("endpoint") endpoint: String,
                   @PathParam("groupName") groupName: String,
                   @QueryParam("api-version") apiVersion: String,
                   @HeaderParam("Accept") accept: String,
                   context: Context): Mono[Response[Schemas]]

    @Get("/$schemaGroups/{groupName}/schemas/{schemaName}")
    @ExpectedResponses(Array(200))
    @UnexpectedResponseExceptionType(classOf[ErrorException])
    def getSchema(@HostParam("endpoint") endpoint: String,
                  @PathParam("groupName") groupName: String,
                  @PathParam("schemaName") schemaName: String,
                  @QueryParam("api-version") apiVersion: String,
                  @HeaderParam("Accept") accept: String,
                  context: Context): Mono[SchemasGetByIdResponse]

    @Get("/$schemaGroups/{groupName}/schemas/{schemaName}/versions")
    @ExpectedResponses(Array(200))
    @UnexpectedResponseExceptionType(classOf[ErrorException])
    def getSchemaVersions(@HostParam("endpoint") endpoint: String,
                          @PathParam("groupName") groupName: String,
                          @PathParam("schemaName") schemaName: String,
                          @QueryParam("api-version") apiVersion: String,
                          @HeaderParam("Accept") accept: String,
                          context: Context): Mono[Response[SchemaVersions]]
  ...
  @ServiceMethod(returns = ReturnType.SINGLE)
  def getSchemaByNameWithResponseAsync(groupName: String, schemaName: String, context: Context): Mono[SchemasGetByIdResponse] = {
    val accept = "application/json"
    service.getSchema(endpoint, groupName, schemaName, apiVersion, accept, context)
  }

  @ServiceMethod(returns = ReturnType.SINGLE)
  def getVersionsWithResponseAsync(groupName: String, schemaName: String, context: Context): Mono[Response[SchemaVersions]] = {
    val accept = "application/json"
