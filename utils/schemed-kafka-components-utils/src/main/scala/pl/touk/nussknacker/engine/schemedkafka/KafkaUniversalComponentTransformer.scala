...
trait KafkaUniversalComponentTransformer[T] extends SingleInputGenericNodeTransformation[T] with WithCachedTopicsExistenceValidator {
  ...
  protected def getVersionParam(versions: List[Integer]): Parameter = {
    ...
    Parameter[String](KafkaUniversalComponentTransformer.SchemaVersionParamName).copy(editor = Some(FixedValuesParameterEditor(versionValues)))
  }
  ...
  protected def parseVersionOption(params: Map[String, Any]): SchemaVersionOption =
    SchemaVersionOption.byName(params(SchemaVersionParamName).asInstanceOf[String])
