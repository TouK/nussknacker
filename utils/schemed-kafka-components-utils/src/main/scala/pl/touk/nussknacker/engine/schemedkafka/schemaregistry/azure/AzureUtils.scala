...
object AzureUtils {
  ...
  def extractSchemaId(headers: Headers): SchemaId = {
    extractSchemaIdOpt(headers).getOrElse(
      throw new IllegalArgumentException(contentTypeHeader + " header not available")
    )
  }
  ...
  def extractSchemaIdOpt(headers: Headers): Option[SchemaId] = {
    ...
    Option(contentTypeHeaderOpt).map { contentTypeHeader =>
      ...
      require(
        contentTypeHeaderString.startsWith("avro/binary+"),
        s"$contentTypeHeader header: '$contentTypeHeaderString' should start with avro/binary"
      )
