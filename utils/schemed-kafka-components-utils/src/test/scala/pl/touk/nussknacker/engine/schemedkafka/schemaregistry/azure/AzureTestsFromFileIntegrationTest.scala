...
@Network
class AzureTestsFromFileIntegrationTest extends AnyFunSuite with Matchers with OptionValues
  with EitherValuesDetailedMessage with LazyLogging {
  ...
  private def wrapWithConsumerRecord(topic: String, key: String, schemaId: SchemaId, serializedValue: Array[Byte]) = {
    ...
    new ConsumerRecord(topic, 0, 0, 0, TimestampType.CREATE_TIME,
      keyBytes.length, serializedValue.length, keyBytes, serializedValue, inputHeaders, Optional.empty[Integer]())
  }
