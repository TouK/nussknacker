...
import pl.touk.nussknacker.engine.schemedkafka.schemaregistry.confluent.schemaid.SchemaIdFromNuHeadersPotentiallyShiftingConfluentPayload.{
  KeySchemaIdHeaderName,
  ValueSchemaIdHeaderName
}
...
class SchemaIdFromMessageExtractorTest extends AnyFunSuite with Matchers with OptionValues {
  ...
  test("extract schema id from nu specific headers") {
    extractor
      .getSchemaId(toHeaders(KeySchemaIdHeaderName -> "123"), Array.empty, isKey = true)
      .value
      .value shouldEqual SchemaId.fromInt(123)
    extractor
      .getSchemaId(toHeaders(ValueSchemaIdHeaderName -> "123"), Array.empty, isKey = false)
      .value
      .value shouldEqual SchemaId.fromInt(123)
  }
  ...
  test("schema id from headers is more important than schema id from payload") {
    ...
    bos.flush()
    extractor
      .getSchemaId(headersWithSchemaIdAvailable, bos.toByteArray, isKey = false)
      .value
      .value shouldEqual schemaIdFromHeader
    extractor
      .getSchemaId(headersWithSchemaIdAvailable, Array.empty, isKey = false)
      .value
      .value shouldEqual schemaIdFromHeader
  }

  test(
    "even if schema id appear in headers, schema id from payload should be read and position in buffer should be moved"
  ) {
    ...
    result.buffer
      .array() shouldEqual Array(ConfluentUtils.MagicByte, 0, 0, 0, schemaIdInBothHeaderAndPayload.asInt.byteValue())
