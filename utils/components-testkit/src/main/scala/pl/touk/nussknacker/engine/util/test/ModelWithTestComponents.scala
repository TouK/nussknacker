...
object ModelWithTestComponents {
  ...
  def withTestComponents[T](config: Config, components: List[ComponentDefinition], globalProcessVariables: Map[String, WithCategories[AnyRef]])(action: ModelData => T): T = {
    val testComponentHolder = TestComponentsHolder.registerTestComponents(components, globalProcessVariables)
