...
case class TestComponentsHolder(runId: TestRunId) extends Serializable {
  ...
  def globalProcessVariables: Map[String, WithCategories[AnyRef]] = TestComponentsHolder.expressionConfigForId(runId)
...
object TestComponentsHolder {
  ...
  def expressionConfigForId(id: TestRunId): Map[String, WithCategories[AnyRef]] = variables(id)

  def registerTestComponents(componentDefinitions: List[ComponentDefinition], globalProcessVariables: Map[String, WithCategories[AnyRef]]): TestComponentsHolder = synchronized {
    val runId = TestRunId(UUID.randomUUID().toString)
    components += (runId -> componentDefinitions)
    variables += (runId -> globalProcessVariables)
