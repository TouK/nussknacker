...
case class TestComponentsHolder(runId: TestRunId) extends Serializable {
  ...
  def globalVariables: Map[String, AnyRef] = TestComponentsHolder.variables(runId)
...
object TestComponentsHolder {
  ...
  private var variables  = Map[TestRunId, Map[String, AnyRef]]()
  ...
  def globalVariables(id: TestRunId): Map[String, AnyRef] = variables(id)
