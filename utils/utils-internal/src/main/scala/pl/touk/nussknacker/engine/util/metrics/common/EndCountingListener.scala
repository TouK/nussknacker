...
private[engine] class EndCountingListener(allNodes: Iterable[NodeData]) extends EmptyProcessListener with WithMetrics {
  ...
  override def open(context: EngineRuntimeContext): Unit = {
    super.open(context)
    endRateMeters = new Meters("end", {
      case e:EndingNodeData => e.id
    })
    deadEndRateMeters = new Meters("dead_end", {
      case e: DeadEndingData => e.id
    })
  }
  ...
  private class Meters(name: String, nodeIds: PartialFunction[NodeData, String]) {
    val meters: Map[String, RateMeter] = allNodes.collect(nodeIds).map {
      nodeId => nodeId -> metricsProvider.instantRateMeterWithCount(MetricIdentifier(NonEmptyList.of(name), Map(nodeIdTag -> nodeId)))
    }.toMap
    def mark(nodeId: String): Unit = meters.getOrElse(nodeId, throw new IllegalArgumentException(s"Unknown node $nodeId in $name known: ${meters.keySet}")).mark()
  }
