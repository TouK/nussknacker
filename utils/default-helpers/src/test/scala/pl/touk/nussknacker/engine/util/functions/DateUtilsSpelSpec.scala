...
class DateUtilsSpelSpec extends AnyFunSuite with BaseSpelSpec with Matchers {
  ...
  test("conversions between various time classes") {
    ...
    evaluateExpr[Long](
      "#DATE.toEpochMilli(#localDateTime, #DATE.zuluTimeZone)"
    ) shouldEqual fixedZoned.toInstant.toEpochMilli
    evaluateExpr[Long](
      "#DATE.toEpochMilli(#localDateTime, #DATE.UTCOffset)"
    ) shouldEqual fixedZoned.toInstant.toEpochMilli
    ...
    evaluateExpr[OffsetDateTime](
      "#DATE.toInstant(#epoch).atOffset(#DATE.UTCOffset)"
    ) shouldEqual fixedZoned.toOffsetDateTime
    ...
    evaluateExpr[LocalDateTime](
      "#DATE.toInstant(#epoch).atZone(#DATE.zuluTimeZone).toLocalDateTime"
    ) shouldEqual fixedZoned.toLocalDateTime
    evaluateExpr[LocalDateTime](
      "#instant.atZone(#DATE.zuluTimeZone).toLocalDateTime"
    ) shouldEqual fixedZoned.toLocalDateTime
  ...
  test("some shortcut conversions using default time zone") {
    ...
    evaluateExpr[ZonedDateTime]("#DATE.nowAtDefaultTimeZone") shouldEqual evaluateExpr[ZonedDateTime](
      "#DATE.nowAtZone(#DATE.zuluTimeZone)"
    )

    evaluateExpr[Instant]("#DATE.toInstantAtDefaultTimeZone(#localDateTime)") shouldEqual evaluateExpr[Instant](
      "#localDateTime.atZone(#DATE.zuluTimeZone).toInstant"
    )

    evaluateExpr[ZonedDateTime]("#instant.atZone(#DATE.defaultTimeZone)") shouldEqual evaluateExpr[ZonedDateTime](
      "#instant.atZone(#DATE.zuluTimeZone)"
    )
    evaluateExpr[ZonedDateTime]("#localDateTime.atZone(#DATE.defaultTimeZone)") shouldEqual evaluateExpr[ZonedDateTime](
      "#localDateTime.atZone(#DATE.zuluTimeZone)"
    )
  }

  test("operation on week days") {
    val someMonday                                = LocalDateTime.of(2021, 10, 11, 0, 0).atZone(ZoneOffset.UTC)
    val variables                                 = Map[String, Any]("zoned" -> someMonday)
    def evaluateExpr[T: TypeTag](expr: String): T = evaluate(expr, variables)
  ...
  test("period and duration computations between two dates") {
    ...
    val variables                                 = Map[String, Any]("zoned" -> someDay)
    def evaluateExpr[T: TypeTag](expr: String): T = evaluate(expr, variables)
  ...
  test("check if date is in given range") {
    val variables                                 = Map[String, Any]("date" -> LocalDate.of(2020, 6, 1))
    def evaluateExpr[T: TypeTag](expr: String): T = evaluate(expr, variables)
  ...
  test("check if date time is in given range") {
    val variables                                 = Map[String, Any]("date" -> LocalDateTime.of(2020, 6, 1, 11, 0))
    def evaluateExpr[T: TypeTag](expr: String): T = evaluate(expr, variables)
    ...
    evaluateExpr[Boolean](
      "#date.isAfter('2020-05-01T11:00:00') AND #date.isBefore('2020-07-01T11:00:00')"
    ) shouldEqual true
  }

  test("check if day of week is in given range") {
    val variables = Map[String, Any]("monday" -> DayOfWeek.MONDAY, "friday" -> DayOfWeek.FRIDAY)
    def evaluateExpr[T: TypeTag](expr: String): T = evaluate(expr, variables)
  ...
  test("check if month is in given range") {
    val variables = Map[String, Any]("february" -> Month.FEBRUARY, "december" -> Month.DECEMBER)
    def evaluateExpr[T: TypeTag](expr: String): T = evaluate(expr, variables)
