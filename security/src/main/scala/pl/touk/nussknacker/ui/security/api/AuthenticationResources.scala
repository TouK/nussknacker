...
trait AuthenticationResources extends Directives with FailFastCirceSupport with AnonymousAccessSupport {
  ...
  // does. When we migrate fully to Tapir, we will get rid of Pekko HTTP and the `authenticate` directive won't be needed.
  // Currently, in the implementation of `authenticate(authCredentials: AuthCredentials)` we use Pekko HTTP classes,
  // so before we throw away Pekko HTTP, we should migrate to some other implementations (e.g. from the Tapir's server
