...
class BasicAuthenticationProvider extends AuthenticationProvider with Directives {
  ...
  override def createAuthenticationResources(config: Config, classLoader: ClassLoader)(implicit ec: ExecutionContext, sttpBackend: SttpBackend[Future, Any]): AuthenticationResources = {
