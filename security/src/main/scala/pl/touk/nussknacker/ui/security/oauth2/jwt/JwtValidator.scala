...
class JwtValidator(keyProvider: JwtHeader => Key) {
  ...
  def introspect[TokenClaims: Decoder](token: String): Validated[OAuth2ErrorHandler.OAuth2JwtError, TokenClaims] = {
    (for {
      ...
      parsedTokenClaims <- tokenClaimsJson
        .as[TokenClaims]
        .leftMap(OAuth2JwtDecodeClaimsJsonError(parsedToken, key, tokenClaimsJson, _))
  ...
  private def parsedTokenWithoutSignature(token: String) = {
    JwtCirce
      .decodeAll(token, JwtOptions.DEFAULT.copy(signature = false))
      .toEither
      .bimap(OAuth2JwtDecodeRawError(RawJwtToken(token), _), ParsedJwtToken.apply _ tupled)
  }
