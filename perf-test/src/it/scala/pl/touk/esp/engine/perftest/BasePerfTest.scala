...
import pl.touk.esp.engine.perftest.util.JmxClient.JmxConfig
import pl.touk.esp.engine.perftest.util.{JmxMetricsCollector, MeasureTime}
import pl.touk.esp.engine.perftest.util.JmxMetricsCollector.StoppedMetricsCollector
...
//    ConfigFactory.load("application-local.conf")
    ConfigFactory.load()
      .withValue(
        ...
        ConfigValueFactory.fromAnyRef(s"pl.touk.esp.engine.perftest.sample.$configCreatorClassName")
      )
  }
