    ...
    (from, to) match {
      ...
      case (Unknown(_), _) =>
        conversionStrategy match {
          case ConversionStrategy.Strict =>
            s"${from.display} cannot be assigned to ${to.display}".invalidNel
          case ConversionStrategy.Loose =>
            NoConvertionIsNeeded.validNel
          case ConversionStrategy.NoConversion =>
            NoConvertionIsNeeded.validNel
        }
