...
describe("validating comments on deploy", () => {
  ...
  it("is valid if comment is empty and comment is not required", () => {
    const validated = ValidateDeployComment("", {validationPattern: "", subtitutionPattern: undefined})
    expect(validated).toEqual({ isValid: true, toolTip: undefined })
  })

  it("is invalid if comment is empty and comment is required", () => {
    const validated = ValidateDeployComment("", {validationPattern: undefined, subtitutionPattern: undefined})
    expect(validated).toEqual({ isValid: false, toolTip: "Comment is required." })
  })

  it("is valid if match pattern is not provided", () => {
    const validated = ValidateDeployComment("Some comment", { validationPattern: undefined, subtitutionPattern: undefined })
    expect(validated).toEqual({ isValid: true, toolTip: undefined })
  })

  it("is invalid if comment does not match provided pattern", () => {
    const validated = ValidateDeployComment("Some comment jiraz-123", { validationPattern: pattern, subtitutionPattern: pattern })
  ...
  it("is valid if comment matches provided pattern", () => {
    const validated = ValidateDeployComment("Some comment jira-123", { validationPattern: pattern, subtitutionPattern: pattern })
    expect(validated).toEqual({ isValid: true, toolTip: undefined })
  })

  it("is valid if comment matches provided pattern more than once", () => {
    const validated = ValidateDeployComment("Some comment jira-123 jira-234", { validationPattern: pattern, subtitutionPattern: pattern })
    expect(validated).toEqual({ isValid: true, toolTip: undefined })
  })
})
