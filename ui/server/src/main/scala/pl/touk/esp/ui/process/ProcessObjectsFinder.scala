...
import pl.touk.esp.ui.api.SignalDefinition
...
object ProcessObjectsFinder {
  ...
  def findSignals(processes: List[DisplayableProcess],
  ...
  def findQueries(processes: List[DisplayableProcess],
  ...
  private def findProcessesWithTransformers(processList: List[DisplayableProcess], transformers: Set[String]): List[String] = {
