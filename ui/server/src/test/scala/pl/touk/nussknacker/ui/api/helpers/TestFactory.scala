...
import pl.touk.nussknacker.engine.{EmptyCustomActionsProvider, ModelData, ProcessingTypeConfig}
...
object TestFactory extends TestPermissions{
  ...
  class MockDeploymentManager(val defaultProcessStateStatus: StateStatus) extends
    FlinkDeploymentManager(ModelData(ProcessingTypeConfig.read(ConfigWithScalaVersion.streamingProcessTypeConfig)), shouldVerifyBeforeDeploy = false, mainClassName = "UNUSED", EmptyCustomActionsProvider){
