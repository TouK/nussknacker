...
class NodeResourcesSpec extends FunSuite with ScalatestRouteTest with FailFastCirceSupport
  ...
  test("validates filter nodes") {
    ...
    saveProcess(testProcess) {
      ...
      val request = NodeValidationRequest(
        ...
        variableTypes = Map("existButString" -> Typed[String], "longValue" -> Typed[Long]),
  ...
  test("handles global variables in NodeValidationRequest") {
    ...
    saveProcess(testProcess) {
      ...
      val request = NodeValidationRequest(
        nodeData = data,
        processProperties = ProcessProperties(typeSpecificProperties = StreamMetaData()),
        variableTypes = Map.empty,
        branchVariableTypes = Some(
          Map(
