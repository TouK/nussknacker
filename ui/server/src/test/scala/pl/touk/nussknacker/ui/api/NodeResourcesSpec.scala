...
class NodeResourcesSpec extends AnyFunSuite with ScalatestRouteTest with FailFastCirceSupport
  ...
  private val processValidation = ProcessValidation(typeToConfig.mapValues(_.modelData),
    typeToConfig.mapValues(_.additionalPropertiesConfig), new SubprocessResolver(subprocessRepository))
