...
class ComponentResourcesSpec extends FunSpec with ScalatestRouteTest with FailFastCirceSupport
  ...
  private val componentService = new DefaultComponentService(testConfig, testProcessingTypeDataProvider, fetchingProcessRepository, subprocessRepository, processCategoryService)
