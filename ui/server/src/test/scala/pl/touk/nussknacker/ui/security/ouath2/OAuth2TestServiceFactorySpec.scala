...
import pl.touk.nussknacker.ui.security.oauth2.{OAuth2ErrorHandler, OAuth2ServiceFactory}
...
import pl.touk.nussknacker.ui.security.ouath2.OAuth2TestServiceFactory.{TestAccessTokenResponse, TestPermissionResponse, TestProfileClearanceResponse, TestProfileResponse}
...
class OAuth2TestServiceFactorySpec extends FlatSpec with Matchers with ScalaFutures with Suite  {
  ...
  def createErrorOAuth2Service(uri: URI, code: StatusCode): TestOAuth2Service = {
    ...
    OAuth2TestServiceFactory(config)
  }

  def createDefaultServiceMock(body: Json, uri: URI): TestOAuth2Service = {
    implicit val testingBackend = SttpBackendStub
      .asynchronousFuture
      .whenRequestMatches(_.uri.equals(Uri(uri)))
