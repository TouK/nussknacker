...
class ServiceRoutesSpec extends FunSuite with Matchers with ScalatestRouteTest with FailFastCirceSupport with TestPermissions{
  ...
  private implicit val user = LoggedUser("1", Option.apply("admin"), category1Deploy)
