...
object MockFetchingProcessRepository {
  def apply[T: ProcessShapeFetchStrategy](processes: List[BaseProcessDetails[T]])(implicit ec: ExecutionContext): FetchingProcessRepository[Future] with BasicRepository = new FetchingProcessRepository[Future] with BasicRepository {
    ...
    override def fetchProcessingType(processId: ProcessId)(implicit loggedUser: LoggedUser, ec: ExecutionContext): Future[String] =
      getUserProcesses.map(_.find(p => p.processId == processId).map(_.processingType).get)
