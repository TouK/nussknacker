...
class BasicHttpAuthenticatorSpec extends FunSpec with Matchers {
  class DummyConfiguration(users: List[ConfiguredUser], backend: AuthenticationBackend.Value = AuthenticationBackend.BasicAuth, usersFile: String = "")
    extends BasicAuthConfiguration(backend: AuthenticationBackend.Value, usersFile: String) {
    override def loadUsers(): List[ConfiguredUser] = users
  }

  it("should authorize using plain password") {
    val authenticator = new BasicHttpAuthenticator(new DummyConfiguration(List(ConfiguredUser("foo", Some("password"), None, Map.empty))))
  ...
  it("should authorize using bcrypt password") {
    val authenticator = new BasicHttpAuthenticator(new DummyConfiguration(List(ConfiguredUser("foo", None,
